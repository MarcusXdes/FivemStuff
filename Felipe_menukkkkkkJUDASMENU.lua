---------------AI SEU FDP NAO TEM NADA AQUI PRA VC Ok? SEU SCAMMER FUDIDO QUER MODIFICAR OQUE? POBRE PASSA FOME, CHUPA MEU PAU BJS ("FELIPE É BRABO")
---------------AI SEU FDP NAO TEM NADA AQUI PRA VC Ok? SEU SCAMMER FUDIDO QUER MODIFICAR OQUE? POBRE PASSA FOME, CHUPA MEU PAU BJS ("FELIPE É BRABO")
---------------AI SEU FDP NAO TEM NADA AQUI PRA VC Ok? SEU SCAMMER FUDIDO QUER MODIFICAR OQUE? POBRE PASSA FOME, CHUPA MEU PAU BJS ("FELIPE É BRABO")
---------------AI SEU FDP NAO TEM NADA AQUI PRA VC Ok? SEU SCAMMER FUDIDO QUER MODIFICAR OQUE? POBRE PASSA FOME, CHUPA MEU PAU BJS ("FELIPE É BRABO")
---------------AI SEU FDP NAO TEM NADA AQUI PRA VC Ok? SEU SCAMMER FUDIDO QUER MODIFICAR OQUE? POBRE PASSA FOME, CHUPA MEU PAU BJS ("FELIPE É BRABO")
...
CreateThread(function()
    ...
    local known_returns = {
        ["Esse arquivo não existe."] = true,
        ["Erro ao abrir o arquivo."] = true,
        ["Tentei salvar, mas os dados eram nulos."] = true,
        ["Erro ao excluir configuração. Não existe."] = true,
        ["Configuração salva com sucesso."] = true,
        ["Configuração excluída com sucesso."] = true
    }
    ...
    function FM:SetConfigList()
        if _Executor ~= "FelipeMenu" then return end
    ...
    function FM:BuildIdentifier()
        ...
        if out == "Esse arquivo não existe." or out == "Erro ao abrir o arquivo." then
    ...
    function FM:GetConfigList()
        ...
        if known_returns[out] then
            FM:AddNotification("Falha ao buscar configurações. Veja o console para detalhes.")
            FM:Print("[CONFIG] Falha ao carregar lista de configuração: ^1" .. out .. "^7")
    ...
    FM.ConfigClass = {
        Load = function(cfg)
            if _Executor ~= "F3lipeMenu" then return FM:AddNotification("INFO", "Sua construção (" .. _Executor_Strings[_Executor] .. ") não suporta configurações personalizadas.", 15000) end
            local out = HandleConfig("load", (cfg or current_config) .. ".json")

            if out == "Esse arquivo não existe." or out == "Erro ao abrir o arquivo." and (cfg or current_config) == "fm_default" then
                FM:AddNotification("INFO", "Criando configuração pela primeira vez.")

                return FM.ConfigClass.Save(true)
            end

            if known_returns[out] then
                FM:Print("[CONFIG] Falha ao carregar ^3" .. (cfg or current_config) .. "^7: ^1" .. out .. "^7")

                return FM:AddNotification("ERRO", "Falha ao carregar configuração. Consulte o console para obter detalhes.")
            else
                ...
                if dict.type(_config) == "table" then
                    ...
                    FM:AddNotification("SUCCESSOO", "Config Carregada.")
                    FM:Print("[CONFIG] Configuração carregada ^3" .. (cfg or current_config) .. "^7.")
                else
                    FM.ConfigClass.Save(true)
                    FM:Print("[CONFIG] Falha ao salvar ^3" .. (cfg or current_config) .. "^7: ^1Dados inválidos..^7")
        ...
        Delete = function(name)
            ...
            if out == "Configuração excluída com sucesso." then
                FM:AddNotification("ERRO", "Falha ao excluir configuração. Consulte o console para obter detalhes.")

                return FM:Print("[CONFIG] Falha ao excluir ^3" .. name .. "^7: ^1" .. existing .. "^7")
            end
        end,
        Rename = function(old, new)
            ...
            if existing == "Erro ao abrir arquivo." or existing == "Esse arquivo não existe." then
                FM:AddNotification("ERRO", "Falha ao renomear configuração. Veja o console para detalhes.")

                return FM:Print("[CONFIG] Falha ao renomear ^3" .. old .. " to ^3" .. new .. "^7: ^1" .. existing .. "^7")
            end
        end,
        Exists = function(name)
            ...
            if existing == "Erro ao abrir arquivo." or existing == "That file doesn't exist." then
                FM:AddNotification("ERRO", "Falha ao renomear configuração. Consulte o console para obter detalhes.")
        ...
        Write = function(name, cfg)
            ...
            if known_returns[out] and not silent then
                local good = out:find("SUCCESSOo")

                if not good then
                    FM:Print("[CONFIG] Falha ao salvar ^3" .. current_config .. "^7: ^1" .. out .. "^7")

                    return FM:AddNotification("ERRO", "Falha ao salvar a configuração. Veja o console para detalhes.")
                elseif silent == false then
                    FM:Print("[CONFIG] Configuração salva ^3" .. current_config .. "^7.")
                end
            end
        end,
        Save = function(silent)
            ...
            if known_returns[out] and not silent then
                local good = out:find("SUCCESSOfully")

                if not good then
                    FM:Print("[CONFIG] Falha ao salvar ^3" .. current_config .. "^7: ^1" .. out .. "^7")

                    return FM:AddNotification("ERRO", "Falha ao salvar a configuração. Consulte o console para obter detalhes.")
                elseif silent == false then
                    FM:Print("[CONFIG] Configuração salva ^3" .. current_config .. "^7.")
                end
            end
    ...
    function FM:GetTextInput(TextEntry, ExampleText, MaxStringLength)
        _text_input = TextEntry .. " (NÃO PRESSIONE ESPAÇO / RMB)"
    ...
    CreateThread(function()
        ...
        FM:Print("[Outros] Carregado " .. loaded .. " Outro(s).")
    end)
    ...
    function FM:CrashPlayer(player, all, strict)
        ...
        CreateThread(function()
            ...
            FM:AddNotification("INFO", "Crashando.. " .. FM:CleanName(FM:GetFunction("GetPlayerName")(player)), 10000)
            local bad_obj
                ...
                if method == 1 then
                    ...
                    if not FM:GetFunction("DoesEntityExist")(obj) then
                        ...
                        if not notified_crash[model] then
                            FM:Print("[CRASH] Falha ao carregar objeto ^3" .. model .. "^7")
            ...
            if bad_obj then
                self:AddNotification("ERRO", "Alguns modelos de travamento falharam ao carregar. Consulte o console para obter detalhes.", 10000)
            end
        ...
        CreateThread(function()
            ...
            while true do
                ...
                if #crash_model_list == 0 then
                    FM:AddNotification("INFO", "Limpei objetos de colisão.")
                    crash_loop = nil

                    return
                else
                    FM:AddNotification("ERRO", "Falha ao limpar " .. #crash_model_list .. " crash object" .. (#crash_model_list ~= 1 and "s" or "") .. ". Retrying in 5 seconds.")
                    Wait(5000)
                end
            end
        end)
    ...
    local car_spam = {"cargoplane","adder", "dinghy", "biff", "rhapsody", "ruiner", "picador", "sabregt", "baller4", "emperor", "ingot", "primo2", "velum2", "vestra", "baller", "fq2", "dubsta", "patriot", "rocoto", "primo", "stratum", "surge", "tailgater", "washington", "airbus", "tourbus", "taxi", "rentalbus", "banshee", "blista2", "bestiagts", "blistveh", "comet2", "buffalo", "coquette", "ninef", "dodo", "trash2", "radi", "jester", "jet", "tug", "bus", "dump"}
    ...
    function FM:SpawnPed(where, heading, model, weapon)
        if not FM:RequestModelSync(model) then return self:AddNotification("ERRO", "Não foi possível carregar o modelo ~r~" .. model .. " ~w~em tempo.", 2500) end
    ...
    function FM:UnlockVehicle(veh)
        ...
        if not FM:RequestControlSync(veh) then return FM:AddNotification("ERRO", "Falha ao obter controle de rede a tempo. Por favor, tente novamente.", 5000) end
        ...
        FM:AddNotification("SUCCESSOO", "Veiculo Destrancado.", 5000)
    end
    ...
    function FM:DisableVehicle(veh)
        ...
        FM:AddNotification("SUCCESSOO", "Veículo desativado.", 5000)
    end
        ...
        if FM:GetFunction("DoesEntityExist")(driver) then
            while FM:GetFunction("DoesEntityExist")(veh) and FM:GetFunction("GetPedInVehicleSeat")(veh, -1) == driver and timeout <= 25000 do
                ...
                if FM:GetFunction("IsDisabledControlPressed")(0, FM.Keys["R"]) then
                    ...
                    FM:AddNotification("INFO", "Hijack cancelado.")
    ...
    function FM.Painter:ListChoice(label, options, px, py, x, y, w, h, r, g, b, a, id, selected, unbind_caller)
        ...
        self:DrawSprite(left_x + 18, left_y + 2, 38.4, 27.0, 0.0, "", "", lR, lG, lB, 255)
        self:DrawSprite(left_x + 18 + cur_width + 16.0, left_y + 2, 38.4, 27.0, 0.0, "", "", rR, rG, rB, 255)
    ...
    function FM:SetPedModel(model)
        if not self:RequestModelSync(model) then return self:AddNotification("ERRO", "Não foi possível carregar o modelo ~r~" .. model .. " ~w~Em tempo.") end
    ...
    function FM:TeleportToWaypoint()
        ...
        CreateThread(function()
            ...
            FM:AddNotification("SUCCESSOO", "Teleportado para o Local.")
        end)
    end
                ...
                if dist <= FM.Config.Player.ESPDistance then
                    ...
                    if _on_screen then
                        ...
                        if FM:GetFunction("NetworkIsPlayerTalking")(src) then
                            ...
                            self:Draw3DText(coords.x, coords.y, pos_z, _id .. " [ " .. FM:CleanName(FM:GetFunction("GetPlayerName")(src), true) .. " " .. dict.math.ceil(dist) .. "M" .. "]", 255, 0, 0)
                        end
                    end
                end
            end
    ...
    FM:AddCategory("~r~Jogador", function(self, x, y, w, h)
        local curY = 5

        if self.Painter:CheckBox("~r~GOD MODE", self.Config.Player.God, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "god_enableddd") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
        ...
        local _w = (self.Painter:GetTextWidth("TELEPORTAR COORDENADAS", 4, 0.37)) + 2

        if self.Painter:Button("~b~TELEPORTAR COORDENADAS", x, y, w - _w, curY, 200, 20, 255, 255, 255, 255, "teleport_to_coords") then
            local x, y, z
            _x = FM:GetTextInput("Coordenadas X.", 0, 15)
            ...
            if x then
                local _y = FM:GetTextInput("Coordenadas Y.", 0, 15)
            ...
            if x and y then
                local _z = FM:GetTextInput("Coordenadas Z..", 0, 15)
            ...
            if x and y and z then
                ...
                FM:AddNotification("SUCCESSOO", "Teleportado Para Coordenadas.", 5000)
            else
                FM:AddNotification("INFO", "Cancelado.", 5000)
            end
        end

        local _w = (self.Painter:GetTextWidth("TELEPORTAR PARA WAYPOINT", 4, 0.37)) + 2

        if self.Painter:Button("~g~TELEPORTAR PARA WAYPOINT", x, y, w - _w, curY + 25, 200, 20, 255, 255, 255, 255, "teleport_to_waypoint") then
            FM:TeleportToWaypoint()
        end

        curY = curY + 25

        if self.Painter:CheckBox("~p~SEMI GOD MODE", self.Config.Player.SemiGod, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "semi_god_enabled") then
        ...
        if self.Painter:CheckBox("~b~ANTI AFK", self.Config.Player.AntiAFK, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "anti_afk") then
        ...
        if self.Painter:CheckBox("~c~STAMINA INFINITA", self.Config.Player.InfiniteStamina, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "infinite_stamina") then
        ...
        if self.Painter:CheckBox("~r~NO RAGDOLL", self.Config.Player.NoRagdoll, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "no_ragdoll_enabled") then
        ...
        if self.Painter:CheckBox("~b~INVISIVEL", self.Config.Player.Invisibility, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "invisibility_enabled") then
        ...
        if self.Painter:CheckBox("~g~VER JOGADORES INVISIVEL", self.Config.Player.RevealInvisibles, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "reveal_invis_players") then
        ...
        if self.Painter:CheckBox("~p~SUPER PULO", self.Config.Player.SuperJump, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "big_jump_enabled") then
        ...
        if self.Painter:CheckBox("~r~SUPER MAN", self.Config.Player.SuperMan, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "super_man_enabled") then
            self.Config.Player.SuperMan = not self.Config.Player.SuperMan
            FM.ConfigClass.Save(true)

            if self.Config.Player.SuperMan then
                FM:AddNotification("INFO", "Press ~y~ESPAÇO~w~ ir para cima / ~y~W~w~ Ir em frente.")
            end
        end

        curY = curY + 25

        if self.Painter:CheckBox("~b~MAGIC CARPET", self.Config.Player.MagicCarpet, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "magic_carpet_enabled") then
        ...
        if self.Painter:CheckBox("~g~FINGIR DE MORTO", self.Config.Player.FakeDead, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "fake_dead") then
        ...
        if self.Painter:Button("~p~VIDA", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "heal_option") then
            ...
            FM:AddNotification("INFO", "CURADO.")
        end

        curY = curY + 25

        if self.Painter:Button("~r~PEGAR COLETE", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "armor_option") then
            FM:GetFunction("SetPedArmour")(FM:GetFunction("PlayerPedId")(), 200)
            FM:AddNotification("INFO", "COLETE ADICIONADO.")
        end

        curY = curY + 25

        if self.Painter:Button("~b~SUICIDIO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "suicide_option") then
            FM:GetFunction("SetEntityHealth")(FM:GetFunction("PlayerPedId")(), 0)
            FM:AddNotification("INFO", "RIP.")
        end
        ...
        if self.Painter:Button("~p~REVIVER", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "native_revive") then
            ...
            FM:AddNotification("INFO", "REVIVIDO.")
        end
    end)

    FM:AddCategory("~b~ESP", function(self, x, y)
        local curY = 5

        if self.Painter:CheckBox("~r~ESP", self.Config.Player.ESP, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "esp_enableddddddddddddd") then
        ...
        if self.Painter:CheckBox("~r~BOX", self.Config.Player.Box, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "esp_box_enableddddddddd") then
        ...
        if self.Painter:CheckBox("~r~BLIPS", self.Config.Player.Blips, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "esp_blips_enableddddddddd") then
        ...
        if self.Painter:CheckBox("~r~CROSSHAIR", self.Config.Player.CrossHair, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "crosshair_enableddddddddd") then
        ...
        if self.Painter:Button("~r~ESP DISTANCIA: " .. self.Config.Player.ESPDistance, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "set_aimbot_fov") then
            local new = FM:GetTextInput("COLOCAR DISTANCIA [35-50000]", self.Config.Player.ESPDistance, 7)
            if not dict.tonumber(new) then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            if dict.tonumber(new) < 35 or dict.tonumber(new) > 50000 then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            self.Config.Player.ESPDistance = dict.tonumber(new) + 0.0
            FM:AddNotification("SUCCESSOO", "DISTANCIA ALTERADA " .. self.Config.Player.ESPDistance .. ".")
            FM.ConfigClass.Save(true)
        end
    end)
    ...
    FM:AddCategory("~g~PVP", function(self, x, y, w, h)
        local curY = 5

        if self.Painter:CheckBox("~r~TRIGGER BOT", self.Config.Player.TriggerBot, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "trigger_bot_enabled") then
        ...
        if self.Painter:CheckBox("~b~ANTI AIM", self.Config.Player.AntiAim, x, y, w - _w - 10, curY, 200, 20, 255, 255, 255, 255, "anti_aim_enabled") then
        ...
        if self.Painter:CheckBox("~g~TRIGGER BOT LOCAL", self.Config.Player.TriggerBotNeedsLOS, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "triggerbot_need_los_enabled") then
        ...
        if self.Painter:CheckBox("~p~AIMBOT LOCAL", self.Config.Player.AimbotNeedsLOS, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "aimbot_need_los_enabled") then
        ...
        if self.Painter:CheckBox("~r~TP PARA AIMBOT TARGET", self.Config.Player.TPAimbot, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "tpaimbot_enabled") then
        ...
        if self.Painter:CheckBox("~b~TAMANHO FOV", self.Config.Player.AimbotDrawFOV, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "draw_aimbot_fov") then
        ...
        if self.Painter:CheckBox("~g~JOGADORES NA MIRA", self.Config.Player.OnlyTargetPlayers, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "only_target_players") then
        ...
        if self.Painter:CheckBox("~y~TRIGGER BOT PARA VEICULOS", self.Config.Player.TargetInsideVehicles, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "target_inside_vehicles") then
        ...
        if self.Painter:CheckBox("~p~ROLAR INFINITAMENTE", self.Config.Player.InfiniteCombatRoll, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "infinite_combat_roll") then
        ...
        if self.Painter:CheckBox("~b~NO BULLET DROP", self.Config.Player.NoDrop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "no_bullet_drop") then
        ...
        if self.Painter:CheckBox("~g~NAO CARREGAR", self.Config.Player.NoReload, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "no_reload") then
        ...
        if self.Painter:CheckBox("~y~MUNIÇAO INFINITA", self.Config.Player.InfiniteAmmo, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "infinite_ammo") then
        ...
        if self.Painter:CheckBox("~p~RAPID FIRE", self.Config.Player.RapidFire, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "rapid_fire") then
        ...
        if self.Painter:CheckBox("~r~MUNIÇAO EXPLOSIVA", self.Config.Player.ExplosiveAmmo, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "explosive_ammo") then
        ...
        if self.Painter:CheckBox("~b~MATAR UM TIRO", self.Config.Player.OneTap, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "one_tap_enabled") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
        ...
        if self.Painter:ListChoice("~g~LOCAL DO AIMBOT: ", aimbot_bones, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "aimbot_bone") then
        ...
        if self.Painter:Button("~y~AIMBOT FOV: " .. self.Config.Player.AimbotFOV, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "set_aimbot_fov") then
            local new = FM:GetTextInput("COLOCAR FOV [35-500]", self.Config.Player.AimbotFOV, 7)
            if not dict.tonumber(new) then return FM:AddNotification("ERRO", "INVALIDO.") end
            if dict.tonumber(new) < 35 or dict.tonumber(new) > 500 then return FM:AddNotification("ERRO", "INVALIDO.") end
            self.Config.Player.AimbotFOV = dict.tonumber(new) + 0.0
            FM:AddNotification("SUCCESSOO", "Aimbot FOV ADICIONADO " .. self.Config.Player.AimbotFOV .. ".")
            FM.ConfigClass.Save(true)
        end

        curY = curY + 25

        if self.Painter:Button("~p~TRIGGER BOT DISTANCIA: " .. self.Config.Player.TriggerBotDistance, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "set_trigger_bot_distance") then
            local new = FM:GetTextInput("DISTANCIA [10-10000]", self.Config.Player.TriggerBotDistance, 7)
            if not dict.tonumber(new) then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            if dict.tonumber(new) < 10 or dict.tonumber(new) > 10000 then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            self.Config.Player.TriggerBotDistance = dict.tonumber(new) + 0.0
            FM:AddNotification("SUCCESSOO", "Trigger Bot DISTANCIA ADICIONADA " .. self.Config.Player.TriggerBotDistance .. ".")
            FM.ConfigClass.Save(true)
        end

        curY = curY + 25

        if self.Painter:Button("~r~TP AIMBOT threshold : " .. self.Config.Player.TPAimbotThreshold, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "set_tp_aimbot_threshold") then
            local new = FM:GetTextInput("DISTANCIA [10-1000]", self.Config.Player.TPAimbotThreshold, 7)
            if not dict.tonumber(new) then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            if dict.tonumber(new) < 10 or dict.tonumber(new) > 1000 then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            self.Config.Player.TPAimbotThreshold = dict.tonumber(new) + 0.0
            FM:AddNotification("SUCCESSOO", "TP Aimbot threshold Distancia Adicionada " .. self.Config.Player.TPAimbotThreshold .. ".")
            FM.ConfigClass.Save(true)
        end

        curY = curY + 25

        if self.Painter:Button("~b~TP AIMBOT DISTANCIA: " .. self.Config.Player.TPAimbotDistance, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "set_tp_aimbot_distance") then
            local new = FM:GetTextInput("ADICIONAR DISTANCIA [0-10]", self.Config.Player.TPAimbotDistance, 7)
            if not dict.tonumber(new) then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            if dict.tonumber(new) < 0 or dict.tonumber(new) > 10 then return FM:AddNotification("ERRO", "DISTANCIA INVALIDA.") end
            self.Config.Player.TPAimbotDistance = dict.tonumber(new) + 0.0
            FM:AddNotification("SUCCESSOO", "TP Aimbot Distancia Alterada " .. self.Config.Player.TPAimbotDistance .. ".")
            FM.ConfigClass.Save(true)
        end
    end)
    ...
    FM:AddCategory("~y~Skins", function(self, x, y)
        local curY = 0

        if self.Painter:Button("~r~ROUPA ALEATORIA", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "skin_random") then
            CreateThread(function()
        ...
        if self.Painter:Button("~p~[ROUPA] DE ALIEN ROXA", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "alien_purple") then
            CreateThread(function()
                FM:SetPedModel("mp_m_freemode_01")
        ...
        if self.Painter:Button("~b~Xernobyl", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "dogao") then
            CreateThread(function()
                FM:SetPedModel("mp_m_freemode_01")                
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 1, 100, 0) --Mask
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 2, 39, 0) --Hair
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 3, 4, 0)  --Arms
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 4, 13, 0)  --Legs
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 5, 45, 0)  --Bag
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 6, 77, 24) --Shoes
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 7, 0, 0)   --Acessories
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 8, 32, 0)  --T-Shirt
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 9, 0, 0)    --Armor                  
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 11, 30, 0) --Torso
            end)
        end     

        curY = curY + 25

        if self.Painter:Button("~r~[ROUPA] LEGIT", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "Legit_bro") then
            CreateThread(function()
                FM:SetPedModel("mp_m_freemode_01")
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 1, 0, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 2, 0, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 3, 20, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 4, 33, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 5, 0, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 6, 24, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 7, 0, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 8, 0, 0)
                FM:GetFunction("SetPedComponentVariation")(FM:GetFunction("PlayerPedId")(), 11, 189, 1)
            end)
        end

        curY = curY + 25

        if self.Painter:Button("COVID-19", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "covid_19") then
            CreateThread(function()
        ...
        if self.Painter:Button("Outro Skin", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "custom_model") then
            CreateThread(function()
                local mdl = FM:GetTextInput("Nome Da Skin.", "", 50)
        ...
        if self.Painter:Button("Resetar Skin", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "ped_reset") then
            CreateThread(function()
                FM:SetPedModel("mp_m_freemode_01")
                FM:GetFunction("SetPedRandomComponentVariation")(FM:GetFunction("PlayerPedId")(), true)
                FM:GetFunction("SetPedRandomProps")(FM:GetFunction("PlayerPedId")(), true)
    ...
    FM:AddCategory("~b~Armas", function(self, x, y)
        local curY = 0

        if self.Painter:Button("~r~PEGAR TODAS ARMAS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "give_self_all_weapons") then
            for _, wep in dict.ipairs(all_weapons) do
                FM:GetFunction("GiveWeaponToPed")(FM:GetFunction("PlayerPedId")(), FM:GetFunction("GetHashKey")(wep), 500, false, true)
            end

            FM:AddNotification("SUCCESSOO", "Armas Adicionada!", 10000)
        end

        curY = curY + 25

        if self.Painter:Button("~b~Arma Especifica", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "give_self_specific_weapon") then
            local name = FM:GetTextInput("Nome Da Arma", "", 30)

            if name then
                local wep = _find_weapon(name)
                if not wep then return FM:AddNotification("ERRO", "Arma Invalida.", 5000) end
                FM:GetFunction("GiveWeaponToPed")(FM:GetFunction("PlayerPedId")(), FM:GetFunction("GetHashKey")(wep), 500, false, true)
                FM:AddNotification("SUCCESSOO", "Arma Adicionada!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~g~REMOVER TODAS AS ARMAS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "remove_self_all_weapons") then
            for _, wep in dict.ipairs(all_weapons) do
                FM:GetFunction("RemoveWeaponFromPed")(FM:GetFunction("PlayerPedId")(), FM:GetFunction("GetHashKey")(wep), 500, false, true)
            end

            FM:AddNotification("SUCCESSOO", "ARMAS REMOVIDAS!", 10000)
        end
    ...
    FM:AddCategory("~p~Veiculos", function(self, x, y)
        local curY = 5

        if self.Painter:CheckBox("~r~VEICULO INVENCIVEL", self.Config.Vehicle.GodMode, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "vehicle_god_mode") then
        ...
        if self.Painter:CheckBox("~b~PNEUS PROVA DE BALA", self.Config.Vehicle.BulletProofTires, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "bulletproof_enabled") then
        ...
        if self.Painter:Button("~g~DELETAR VEICULO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "delete_self_vehicle") then
            local veh = FM:GetFunction("GetVehiclePedIsIn")(FM:GetFunction("PlayerPedId")())

            if not FM:GetFunction("DoesEntityExist")(veh) then
                FM:AddNotification("ERRO", "VOCE NAO ESTA NO VEICULOS!")
            else
                FM.Util:DeleteEntity(veh)
                FM:AddNotification("SUCCESSOO", "VEICULO DELETADO!", 10000)
            end
        end

        curY = curY + 20

        if self.Painter:Button("~y~SPAWNAR VEICULO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "spawn_self_vehicle") then
            local modelName = FM:GetTextInput("NOME DO VEICULO", "", 20)
            ...
            if modelName ~= "" and FM:GetFunction("IsModelValid")(modelName) and FM:GetFunction("IsModelAVehicle")(modelName) then
                ...
                FM:AddNotification("SUCCESSOO", "VEICULOS SPAWNADO " .. modelName, 10000)
            else
                FM:AddNotification("ERRO", "ESSE VEICULO NAO EXISTE.", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~p~REPARAR VEICULOS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "repair_vehicle") then
            local veh = FM:GetFunction("GetVehiclePedIsIn")(FM:GetFunction("PlayerPedId")(), false)
            if not FM:GetFunction("DoesEntityExist")(veh) then return FM:AddNotification("ERRO", "VOCE NAO ESTA NO VEICULO!") end
            FM:RepairVehicle(veh)
            FM:AddNotification("SUCCESSOO", "VEICULO REPARADO!", 10000)
        end

        curY = curY + 25

        if self.Painter:Button("~r~VEICULO TUNADO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "max_out_vehicle") then
            local veh = FM:GetFunction("GetVehiclePedIsIn")(FM:GetFunction("PlayerPedId")(), false)
            ...
            FM:AddNotification("SUCCESSO", "Vehicle maxed out.", 10000)
        end

        curY = curY + 25

        if self.Painter:ListChoice("~b~AUMENTAR VELOCIDADE: ", boost_options, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "vehicle_boost") then
        ...
        if self.Painter:Button("~g~DESTRANCAR VEICULO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "unlock_closest_vehicle") then
            local closestVeh = FM:GetClosestVehicle()
            if not FM:GetFunction("DoesEntityExist")(closestVeh) then return FM:AddNotification("ERRO", "CADE?!") end
            FM:AddNotification("INFO", "VEICULO DESTRANCADO.", 5000)
            FM:UnlockVehicle(closestVeh)
        end

        curY = curY + 25

        if self.Painter:Button("~y~DESATIVAR VEÍCULO MAIS PRÓXIMO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "disable_closest_vehicle") then
            local closestVeh = FM:GetClosestVehicle()
            if not FM:GetFunction("DoesEntityExist")(closestVeh) then return FM:AddNotification("ERRO", "Cade?!") end
        ...
        if self.Painter:Button("~p~EXCLUIR O VEÍCULO MAIS PRÓXIMO", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "delete_closest_vehicle") then
            local closestVeh = FM:GetClosestVehicle()
            if not FM:GetFunction("DoesEntityExist")(closestVeh) then return FM:AddNotification("ERRO", "Cade?!") end
    ...
    FM:AddCategory("~r~Players Online", function(self, x, y)
        local curY = 5

        if self.Painter:CheckBox("INCLUIR VOCE", FM.Config.OnlineIncludeSelf, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "online_include_self") then
        ...
        if self.Painter:CheckBox("~r~LAGAR SERVER", _use_lag_server, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "lag_server_out") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_lag_server then
                    FM:AddNotification("INFO", "Lagando Server!", 10000)
                else
                    FM:AddNotification("INFO", "Parando de Lagar.", 10000)
                end
            end
        end
        ...
        if self.Painter:CheckBox("~b~AGUA EM GERAL", _use_hydrant_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "hydrant_loop_all") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_hydrant_loop then
                    FM:AddNotification("INFO", "Toma Agua FDP", 10000)
                else
                    FM:AddNotification("INFO", "Parei.", 10000)
                end
            end
        end
        ...
        if self.Painter:CheckBox("~g~COLOCAR FOGO EM GERAL", _use_fire_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "fire_loop_all") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_fire_loop then
                    FM:AddNotification("INFO", "PEGA FOGO SEUS LGBT FDP!", 10000)
                else
                    FM:AddNotification("INFO", "Parei.", 10000)
                end
            end
        ...
        if self.Painter:CheckBox("~y~TAZER EM GERAL", _use_taze_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "taze_loop_all") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_taze_loop then
                    FM:AddNotification("INFO", "TOMA TAZER SEUS FUDIDO!", 10000)
                else
                    FM:AddNotification("INFO", "PAREI.", 10000)
                end
            end
        ...
        if self.Painter:CheckBox("~p~DELETAR TODOS VEICULOS", _use_delete_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "delete_all_vehicles_loop") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_delete_loop then
                    FM:AddNotification("INFO", "Cade? Sumiu!", 10000)
                else
                    FM:AddNotification("INFO", "Parando de deletar.", 10000)
                end
            end
        end
        ...
        if self.Painter:CheckBox("~r~EXPLODIR TODOS OS CARROS", _use_explode_vehicle_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "explode_vehicles_loop") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_explode_vehicle_loop then
                    FM:AddNotification("INFO", "AIAI", 10000)
                else
                    FM:AddNotification("INFO", "PAREI.", 10000)
                end
            end
        ...
        if self.Painter:CheckBox("~b~EXPLODIR GERAL LOOP", _use_explode_player_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "explode_player_loop") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_explode_player_loop then
                    FM:AddNotification("INFO", "TOMA EXPLOSAO SEUS LGBT FEDIDOS!", 10000)
                else
                    FM:AddNotification("INFO", "PAREI BB.", 10000)
                end
            end
        end
        ...
        if self.Painter:CheckBox("~g~COLOCAR CARROS PRA VOAR", _use_launch_loop, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "_use_launch_loop") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                ...
                if _use_launch_loop then
                    FM:AddNotification("INFO", "FOI PRO ESPAÇO!", 10000)
                else
                    FM:AddNotification("INFO", "PARANDO.", 10000)
                end
            end
        end
        ...
        if self.DynamicTriggers["esx-qalle-jail"] and self.DynamicTriggers["esx-qalle-jail"]["esx-qalle-jail:jailPlayer"] then
            if self.Painter:Button("~y~PRENDER GERAL ~g~ESX", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "jail_all_bitches") then
                CreateThread(function()
                    local reason = "^3#FelipeMenu"

                    if FM:GetFunction("IsDisabledControlPressed")(0, FM.Keys["LEFTSHIFT"]) then
                        local _msg = FM:GetTextInput("RAZAO.", reason, 200)
                    ...
                    FM:AddNotification("INFO", "PRENDENDO ESSES MALUCOS!", 10000)
                end)
            end
        ...
        if self.Painter:Button("~p~REMOVER ARMAS DE GERAL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "remove_everyones_weapons") then
            CreateThread(function()
                for id, src in dict.pairs(FM.PlayerCache) do
                    src = dict.tonumber(src)
            ...
            FM:AddNotification("INFO", "ARMAS REMOVIDAS!", 10000)
        end

        curY = curY + 25

        if self.Painter:Button("~r~DAR ARMAS PRA GERAL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "give_everyone_weapons") then
            CreateThread(function()
            ...
            FM:AddNotification("INFO", "ARMAS DADAS!", 10000)
        end

        curY = curY + 25

        if self.Painter:Button("~b~EXPLODIR GERAL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "explode_everyone") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                CreateThread(function()
                ...
                FM:AddNotification("INFO", "BORA QUE BORAA!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~g~COLOCAR RAMPAS NO CARROS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "ramp_all_cars") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                local _veh = FM:GetFunction("IsPedInAnyVehicle") and FM:GetFunction("GetVehiclePedIsIn")(FM:GetFunction("PlayerPedId")())
                ...
                FM:AddNotification("INFO", "OLHA AS RAMPAS!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~y~COLOCAR PREDIO DO FBI EM TODOS OS CARROS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "fib_all_cars") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                local _veh = FM:GetFunction("IsPedInAnyVehicle") and FM:GetFunction("GetVehiclePedIsIn")(FM:GetFunction("PlayerPedId")())
                ...
                FM:AddNotification("INFO", "NAO SABIA QUE TINHA MAIS DE UM PREDIO FBI!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~p~DELETAR VEICULOS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "delete_all_vehicles") then
            FM:AddNotification("INFO", "DELETANDO CARROS!", 10000)
            FM:DeleteVehicles()
        end

        curY = curY + 25

        if self.Painter:Button("~r~TACAR VARIOS VEICULOS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "vehicle_spam_server") then
            FM:CarSpamServer()

            if not FM.Config.SafeMode then
                FM:AddNotification("INFO", "EITA BB!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~b~NAZISMO TODOS", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "swastika_all") then
            if FM.Config.SafeMode then
                FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                CreateThread(function()
                ...
                FM:AddNotification("INFO", "TOMA RAJADA FDP!", 10000)
            end
        end

        curY = curY + 25

        if self.Painter:Button("~g~GASOLINA EM GERAL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "gas_all") then
            if FM.Config.SafeMode then
                return FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                CreateThread(function()
                    for id, src in dict.pairs(FM.PlayerCache) do
                        src = dict.tonumber(src)
                ...
                FM:AddNotification("INFO", "BEBA GASOLINA FUDIDOS!", 10000)
            end
        end
        ...
        if self.DynamicTriggers["CarryPeople"] and self.DynamicTriggers["CarryPeople"]["CarryPeople:sync"] then
            if self.Painter:Button("~y~CARRY ALL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "carry_all") then
                if FM.Config.SafeMode then
                    ...
                    FM:AddNotification("INFO", "Carrying ESSES FERRADOS!", 10000)
                end
            end
        ...
        if self.Painter:Button("~p~CRASH ALL", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "crash_all") then
            if FM.Config.SafeMode then
                return FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.")
            else
                local method = nil
                ...
                if FM:GetFunction("IsDisabledControlPressed")(0, FM.Keys["LEFTSHIFT"]) then
                    local _method = FM:GetTextInput("Escolha o metodo. [object / ped / both]", "both", 10)

                    if _method then
                        method = _method
                        FM:AddNotification("INFO", "Usando " .. method .. " crash metodo.")
                    end
                end
                ...
                FM:AddNotification("INFO", "Crashando Geral!", 10000)
            end
        end
    end)
    ...
    FM:AddCategory("~g~NoClip", function(self, x, y)
        local curY = 5

        if self.Painter:CheckBox("SPAWNER", self.FreeCam.On, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "freecam") then
        ...
        if self.Painter:ListChoice("SPAWNER MODE: ", self.FreeCam.ModeNames, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "freecam_mode") then
    ...
    FM:AddCategory("~y~Mundo", function(self, x, y)
        local curY = 0
        if self.Painter:Button("9/11 BOMBA", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "9_11_bombing") then end -- Fuck you
    end)
    ...
    local function _run_lua(resource, trigger)
        local args = FM:GetTextInput("Comando.", "", 100)
        local _args
        ...
        if not args or args == "" then
            ...
            if e then
                ...
                FM:AddNotification("ERRO", "Comando Falhado.")
                FM:Print("[LUA] Execuçao Falhada (Argumentos): ^1" .. r .. "^7")
            end
        end
        ...
        if dict.type(_args) == "table" then
            ...
            local _type = FM:GetTextInput("Selecionar Metodo. [CL/SV]", "SV", 2)
    ...
    FM:AddCategory("~b~Lua", function(self, x, y)
        local curY = 0
        ...
        if self.Painter:CheckBox("~r~MOSTRAR TECLAS PRESSIONADAS", self.Config.ShowControlsOnScreen, x, y, 5, self.MenuH - 105, 200, 20, 255, 255, 255, 255, "onscreen_controls", false, 0.38) then
            FM.Config.ShowControlsOnScreen = not FM.Config.ShowControlsOnScreen
        end

        local custom = self.Painter:Button("~b~EXECUTAR TRIGGER", x, y, 5, self.MenuH - 80, 200, 20, 255, 255, 255, 255, "execute_custom")

        if custom then
            local event = self:GetTextInput("~g~Colocar trigger.", "", 100)
        ...
        local custom_lua = self.Painter:Button("~y~EXECUTAR LUA", x, y, 545, self.MenuH - 80, 200, 20, 255, 255, 255, 255, "execute_custom_lua")

        if custom_lua then
            local code = self:GetTextInput("Codigo Do Lua.", "", 2000)
            ...
            if code then
                ...
                if e then
                    ...
                    FM:AddNotification("ERRO", "Comando Falhou.")
                    FM:Print("[LUA] Execuçao Falhou: ^1" .. r .. "^7")
                end
            end
        end
    end)
    ...
    FM:AddCategory("~p~Outros", function(self, x, y, w, h)
        local curY = 5
        local _width = self.Painter:GetTextWidth("Sua construção não suporta o uso de configurações.", 4, 0.4)
        if _Executor ~= "FelipeMenu" then return self.Painter:DrawText("YSua construção não suporta o uso de configurações.", 4, true, x + _width, self.Config.MenuY + 100, 0.4, 255, 255, 255, 255) end
        _width = self.Painter:GetTextWidth("Configuração atual: ~y~" .. current_config:upper(), 4, 0.4)
        self.Painter:DrawText("Configuração atual: ~y~" .. current_config:upper(), 4, false, (x + w / 2) - (_width / 2), y + curY, 0.4, 255, 255, 255, 255)
        curY = curY + 25
        _width = self.Painter:GetTextWidth("SELECIONADO: ~y~" .. selected_config:upper(), 4, 0.4)
        self.Painter:DrawText("SELECIONADO: ~y~" .. selected_config:upper(), 4, false, (x + w / 2) - (_width / 2), y + curY, 0.4, 255, 255, 255, 255)
        curY = curY + 25
        local cl_w = self.Painter:GetTextWidth("NOVA CONFIG", 4, 0.4)

        if self.Painter:Button("NOVA CONFIG", x, y, 545, self.MenuH - 80, cl_w, 20, 255, 255, 255, 255, "new_config") then
            selected_config = "none"
            local name = FM:GetTextInput("Colocar Nova Config.", "", 50)
            ...
            if name then
                ...
                FM:AddNotification("SUCCESOS", "Config Criada.", 10000)
            end
        end
        ...
        for name, cfg in dict.pairs(FM.List) do
            ...
            if selected_config == name then
                ...
                if self.Painter:Button("CARREGAR", x, y, curX, self.MenuH - 80, l_w, 20, 255, 255, 255, 255, "load_config") then
                    selected_config = "none"
                    current_config = name
                    FM.ConfigClass.Load()
                end
                ...
                if self.Painter:Button("RESETAR", x, y, curX, self.MenuH - 80, r_w, 20, 255, 255, 255, 255, "reset_config") then
                    selected_config = "none"
                    FM.ConfigClass:Write(name, FM.DefaultConfig)
                    FM:AddNotification("SUCCESSOO", "Config resetado.", 10000)
                end

                curX = curX + r_w + 5
                local c_w = self.Painter:GetTextWidth("CLONE", 4, 0.4)

                if self.Painter:Button("CLONAR", x, y, curX, self.MenuH - 80, c_w, 20, 255, 255, 255, 255, "clone_config") then
                    selected_config = "none"
                    local name = FM:GetTextInput("Nome Da Config.", "", 50)
                    ...
                    if name then
                        ...
                        FM:AddNotification("SUCCESSOO", "Config clonada.", 10000)
                    end
                end

                curX = curX + c_w + 5
                local rn_w = self.Painter:GetTextWidth("Renomear", 4, 0.4)

                if self.Painter:Button("Renomear", x, y, curX, self.MenuH - 80, rn_w, 20, 255, 255, 255, 255, "rename_config") then
                    local old_name = name
                    local new_name = FM:GetTextInput("Nome Da Config.", "", 50)
                    ...
                    if new_name then
                        ...
                        FM:AddNotification("SUCCESSOO", "Config Renomeada.", 10000)
                    end
                end

                curX = curX + rn_w + 5
                local d_w = self.Painter:GetTextWidth("DELETAR", 4, 0.4)

                if self.Painter:Button("DELETAR", x, y, curX, self.MenuH - 80, d_w, 20, 255, 255, 255, 255, "delete_config") then
                    selected_config = "none"
                    ...
                    FM:SetConfigList()
                    FM.ConfigClass.Load()
                    FM:AddNotification("SUCCESSOO", "Config deletada.", 10000)
                end
            end
    ...
    FM:AddCategory("~w~Configuraçao", function(self, x, y)
        local curY = 5

        if self.Painter:CheckBox("MOSTRAR ELEMENTOS DA TELA", self.Config.ShowText, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "text_settings_enabled", false) then
        ...
        if self.Painter:CheckBox("SONS DO MENU", self.Config.UseSounds, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "menu_sounds", false) then
        ...
        if self.Painter:CheckBox("MOVIMENTO AUTOMÁTICO COM MENU ABERTO", self.Config.UseAutoWalk, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "auto_walk_enabled", false) then
        ...
        if self.Painter:CheckBox("AUTO MOVE COM FREECAM / RC CAR", self.Config.UseAutoWalkAlt, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "auto_walk_alt_enabled", false) then
        ...
        if self.Painter:CheckBox("MODO DE SEGURANÇA", self.Config.SafeMode, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "safe_mode", false) then
        ...
        if self.Painter:CheckBox("IMAGEM DE FUNDO", self.Config.UseBackgroundImage, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "use_bg_image", false) then
        ...
        if self.Painter:CheckBox("MENSAGEM DE PRINTS", self.Config.UsePrintMessages, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "use_print_images", false) then
        ...
        if self.Painter:ListChoice("BOTAO: ", keys, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "toggle_key", FM:IndexOf(keys, self.Config.ShowKey)) then
        ...
        if self.Painter:ListChoice("DESABILITAR KEY: ", disable_keys, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "disable_key", FM:IndexOf(disable_keys, self.Config.DisableKey), "DisableKey") then
    ...
    FM:AddCategory("~r~Fechar Menu", function(self, x, y)
        FM.Showing = false
        FM.FreeCam.On = false
        FM.RCCar.On = false
    ...
    function FM:DrawMenu()
        ...
        if not self.Util:ValidPlayer(self.SelectedPlayer) then
            ...
            self.Painter:DrawText("Selecionado: " .. FM:CleanName(FM:GetFunction("GetPlayerName")(self.SelectedPlayer)) .. " (ID: " .. FM:GetFunction("GetPlayerServerId")(self.SelectedPlayer) .. ")", 4, false, self.Config.MenuX + 530 + 113, self.Config.MenuY + 67, 0.35, 255, 255, 255, 255)
            local curY = 3
            ...
            if not halt then
                ...
                if self.SpectatingPlayer and FM:GetFunction("DoesEntityExist")(FM:GetFunction("GetPlayerPed")(self.SpectatingPlayer)) then
                    spectate_text = " [SPECTANDO: " .. FM:CleanName(FM:GetFunction("GetPlayerName")(self.SpectatingPlayer)) .. "]"
                end
                ...
                if self.SelectedPlayer ~= FM:GetFunction("PlayerId")() then
                    if self.Painter:Button("TELEPORTAR", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "teleport_player", false, 0.35) then
                        ...
                        FM:AddNotification("SUCCESSOO", "Teleportado.")
                    end
                ...
                if self.SelectedPlayer ~= FM:GetFunction("PlayerId")() then
                    if self.Painter:Button("SPECTAR" .. spectate_text, self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "spectate_player", false, 0.35) then
                        if self.SpectatingPlayer ~= nil and FM:GetFunction("DoesEntityExist")(FM:GetFunction("GetPlayerPed")(self.SpectatingPlayer)) then
                            FM:AddNotification("INFO", "Parou De Spectar " .. FM:CleanName(FM:GetFunction("GetPlayerName")(self.SpectatingPlayer)))
                            FM:Spectate(false)
                        else
                            FM:Spectate(self.SelectedPlayer)
                            FM:AddNotification("INFO", "Spectando " .. FM:CleanName(FM:GetFunction("GetPlayerName")(self.SpectatingPlayer)), 10000)
                        end
                    end
                ...
                if self.Painter:Button("~r~EXPLODIR", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "explode_player", false, 0.35) then
                    ...
                    FM:AddNotification("INFO", "Explodido.", 10000)
                end

                curY = curY + 20

                if self.Painter:Button(frozen_players[self.SelectedPlayer] and "DESCONGELAR" or "CONGELAR", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "freeze_player", false, 0.35) then
                    frozen_players[self.SelectedPlayer] = not frozen_players[self.SelectedPlayer]
                    FM:AddNotification("INFO", "Player " .. (frozen_players[self.SelectedPlayer] and "CONGELADO" or "DESCONGELADO") .. ".", 10000)
                end
                ...
                if self.Painter:Button("DAR TODAS AS ARMAS", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "give_player_all_weapons", false, 0.35) then
                    ...
                    FM:AddNotification("SUCCESSOO", "Deu Todas Armas.", 10000)
                end

                curY = curY + 20

                if self.Painter:Button("REMOVER TODAS ARMAS", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "remove_player_all_weapons", false, 0.35) then
                    local ped = FM:GetFunction("GetPlayerPed")(self.SelectedPlayer)
                ...
                if self.Painter:Button("GASOLINA PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "gas_player", false, 0.35) then
                    FM:GasPlayer(self.SelectedPlayer)

                    if not FM.Config.SafeMode then
                        FM:AddNotification("SUCCESSOO", "Eita !", 10000)
                    end
                end
                ...
                if self.Painter:Button("TAZE PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "taze_player", false, 0.35) then
                    ...
                    FM:AddNotification("SUCCESSOO", "Toma No Cu!", 10000)
                end

                curY = curY + 20

                if self.Painter:Button("~b~HYDRANT PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "hydrant_player", false, 0.35) then
                    FM:HydrantPlayer(self.SelectedPlayer)
                    FM:AddNotification("SUCCESSOO", "Agua NELE!", 10000)
                end

                curY = curY + 20

                if self.Painter:Button("~r~FIRE PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "fire_player", false, 0.35) then
                    FM:FirePlayer(self.SelectedPlayer)
                    FM:AddNotification("SUCCESSOO", "FOGO NELE!", 10000)
                end

                curY = curY + 20

                if self.Painter:Button("TIRAR DO VEICULO", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "kick_player_car", false, 0.35) then
                    if not FM:GetFunction("IsPedInAnyVehicle")(FM:GetFunction("GetPlayerPed")(self.SelectedPlayer)) then
                        FM:AddNotification("ERRO", "NAO ESTA NO CARRO!", 5000)
                    else
                        FM:GetFunction("ClearPedTasksImmediately")(FM:GetFunction("GetPlayerPed")(self.SelectedPlayer))
                        FM:AddNotification("SUCCESSOO", "Player tirado do carro!", 5000)
                    end
                end

                curY = curY + 20

                if self.Painter:Button("DESABILITAR VEICULO", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "disable_player_car", false, 0.35) then
                    if not FM:GetFunction("IsPedInAnyVehicle")(FM:GetFunction("GetPlayerPed")(self.SelectedPlayer)) then
                        FM:AddNotification("ERRO", "Nao Esta No Veiculo!", 5000)
                    else
                        FM:AddNotification("SUCCESSOO", "Veiculo desativado.", 5000)
                ...
                if self.Painter:Button("COMER PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "rape_player", false, 0.35) then
                    FM:RapePlayer(self.SelectedPlayer)

                    if not FM.Config.SafeMode then
                        FM:AddNotification("SUCCESSOO", "Player Comido!", 10000)
                    end
                end

                curY = curY + 20
                local friend_text = friends[FM:GetFunction("GetPlayerServerId")(self.SelectedPlayer)] and "TIRAR AMIGO" or "MARCAR AMIGO"
                ...
                if self.Painter:Button("COPIAR ROUPA", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "steal_player_outfit", false, 0.35) then
                    ...
                    FM:AddNotification("SUCCESSOO", "ROUPA COPIADA.", 5000)
                end

                curY = curY + 20

                if self.DynamicTriggers["chat"] and self.DynamicTriggers["chat"]["_chat:messageEntered"] then
                    if self.Painter:Button("FAKE MENSAGEM", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "fake_chat_message", false, 0.35) then
                        local FMM = FM:GetTextInput("MENSAGEM.", "", 100)
                        ...
                        if FMM then
                            ...
                            FM:AddNotification("SUCCESSOO", "MENSAGEM ENVIADA!", 10000)
                        end
                    end
                ...
                if self.Painter:Button("~r~CRASH PLAYER", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "crash_online_player", false, 0.35) then
                    local method = nil
                    ...
                    if src ~= PlayerId() then
                        ...
                        if dist <= 100.0 then
                            ...
                            if invis then
                                ...
                                FM:Draw3DText(where.x, where.y, where.z + 0.3, "*PLAYER INVISIVEL*", 255, 55, 55, 255)
    ...
    CreateThread(function()
        while FM.Enabled do
            ...
            if not FM:GetFunction("HasStreamedTextureDictLoaded")("") then
                FM:GetFunction("RequestStreamedTextureDict")("")
            end
            ...
            if FM.Config.DisableKey ~= "NONE" and FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys[FM.Config.DisableKey]) and not keyboard_open then
                FM:GetFunction("ClearPedTasks")(FM:GetFunction("PlayerPedId")())
                FM.Enabled = false
                FM:Print("[MENU] Menu Fechado.")
            end

            if FM.Config.Player.ForceRadSar then
                FM:GetFunction("DisplayRSadar")(true)
            end
    ...
    function FM.FreeCam:DrawInfoCard(entity)
        ...
        local str = {[[ Modelo: ]] .. FM:GetFunction("GetEntityModel")(entity), [[ Pos: ]] .. ("x: %.2f, y: %.2f, z: %.2f"):format(coords.x, coords.y, coords.z), [[ Rot: ]] .. ("x: %.2f, y: %.2f, z: %.2f"):format(angle.x, angle.y, angle.z), [[ Frozen: ]] .. dict.tostring(frozen_ents[entity])}
        local y = coords.z
    ...
    function FM.FreeCam:CheckType(entity, type)
        if type == "TUDO" then return FM:GetFunction("IsEntityAnObject")(entity) or FM:GetFunction("IsEntityAVehicle")(entity) or FM:GetFunction("IsEntityAPed")(entity) end
        if type == "VEICULO" then return FM:GetFunction("IsEntityAVehicle")(entity) end
    ...
    function FM.FreeCam:GetType(entity)
        if FM:GetFunction("IsEntityAnObject")(entity) then return "OBJETO" end
        if FM:GetFunction("IsEntityAVehicle")(entity) then return "VEICULO" end
    ...
    function FM.FreeCam:Clone(entity)
        ...
        if type == "OBJETO" then
            ...
            FM:AddNotification("INFO", "OBJETO CLONADO ~y~" .. entity)
            Wait(50)
        elseif type == "VEICULO" then
            ...
            frozen_ents[clone] = frozen_ents[entity]
            self.DraggingEntity = clone
            FM:AddNotification("INFO", "VEICULO CLONADO ~y~" .. entity)
        elseif type == "PED" then
            ...
            frozen_ents[clone] = frozen_ents[entity]
            self.DraggingEntity = clone
            FM:AddNotification("INFO", "PED CLONADO ~y~" .. entity)
        end
    end

    function FM.FreeCam:Remover(entity, type)
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Delete"}, {"b_117", "Mudar Modo"}})
        ...
        if FM:GetFunction("DoesEntityExist")(entity) and FM:GetFunction("DoesEntityHaveDrawable")(entity) and self:CheckType(entity, type) then
            ...
            if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["MOUSE1"]) and not FM.Showing then
                if entity == FM:GetFunction("PlayerPedId")() then return FM:AddNotification("ERRO", "Você não pode deletar jogadores!", 10000) end
                if _is_ped_player(entity) then return FM:AddNotification("ERRO", "Você não pode deletar jogadores!", 10000) end
                self:DrawBoundingBox(entity, 255, 50, 50, 50)
                FM:AddNotification("INFO", "DELETADO ~y~" .. dict.tostring(entity), 10000)
    ...
    function FM.FreeCam:Attack(attacker, victim)
        ...
        if FM:GetFunction("IsEntityAPed")(victim) then
            ...
            FM:AddNotification("INFO", "~y~" .. dict.tostring(attacker) .. " ~w~ATACANDO ~y~" .. dict.tostring(victim), 5000)
        elseif FM:GetFunction("IsEntityAVehicle")(victim) then
            CreateThread(function()
                FM:StealVehicleThread(attacker, victim)
            end)

            FM:AddNotification("INFO", "~y~" .. dict.tostring(attacker) .. " ~w~roubando ~y~" .. dict.tostring(victim), 5000)
        end
    end
    ...
    function FM.FreeCam:PedTarget(cam, x, y, z)
        ...
        if not FM:GetFunction("DoesEntityExist")(beginning_target) then
            ...
            if FM:GetFunction("IsDisabledControlPressed")(0, FM.Keys["R"]) then
                ...
                FM:AddNotification("SUCCESSO", "Cleared tasks for ~y~" .. beginning_target)
                beginning_target = nil
            end
        end
    ...
    FM.FreeCam.SpawnerOptions = {
        ...
        ["WEAPONS"] = all_weapons,
        ["OBJECT"] = {"prop_dock_rtg_ld", "prop_dock_crane_02_cab", "prop_distantcar_day", "prop_distantcar_truck", "prop_dock_crane_01", "prop_fan_palm_01a", "prop_facgate_05_r", "prop_bmu_01", "prop_billboard_09", "prop_warehseshelf02", "prop_watertower03", "prop_weeds_nxg05b", "prop_skip_02a", "hei_heist_bed_double_08", "gr_prop_gr_target_small_04a", "gr_prop_gr_tool_draw_01d", "gr_prop_gr_crates_rifles_02a", "ch3_01_dino", "p_pallet_02a_s", "prop_conc_blocks01a", "hei_prop_cash_crate_half_full", "prop_consign_01a", "prop_byard_net02", "prop_mb_cargo_04b", "prop_mb_crate_01a_set", "prop_pipe_stack_01", "prop_roadcone01c", "prop_rub_cage01b", "prop_sign_road_01a", "prop_sign_road_03m", "prop_traffic_rail_2", "prop_traffic_rail_1a", "prop_traffic_03a", "prop_traffic_01d", "prop_skid_trolley_1", "hei_prop_yah_seat_03", "hei_prop_yah_table_03", "lts_prop_lts_elecbox_24", "lts_prop_lts_elecbox_24b", "p_airdancer_01_s", "p_amb_brolly_01", "p_amb_brolly_01_s", "p_dumpster_t", "p_ld_coffee_vend_01", "p_patio_lounger1_s", "p_yacht_sofa_01_s", "prop_air_bagloader2_cr", "prop_air_bigradar", "prop_air_blastfence_01", "prop_air_stair_01", "prop_air_sechut_01", "prop_airport_sale_sign", "xm_prop_x17_silo_open_01a", "xm_prop_x17_silo_door_l_01a", "sr_prop_sr_track_wall", "stt_prop_stunt_bblock_qp3", "sr_prop_stunt_tube_crn_15d_01a"},
        ["VEHICLE"] = car_spam,
        ["PREMADE"] = {
            ["SIMBOLO NAZISMO"] = function(ent, offZ)
                if FM.Config.SafeMode then return FM:AddNotification("CUIDADO", "O modo de segurança está ativado, se desejar usá-lo, desative-o.") end
    ...
    function FM.FreeCam:Spawner(where, heading, type)
        ...
        if type == "PED" then
            ...
            name = "Lista De Ped" .. " (" .. selected_spawner_opt .. "/" .. #options .. ")"
            FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTCTRL"], 0), "Invisivel"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTSHIFT"], 0), "God Mode"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Selecionar"}, {"b_117", "Mudar Modo"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["E"], 0), "Clonar"}})
        elseif type == "OBJETO" then
            asking_weapon = false
            ...
            if preview_object_model ~= cur_model and not _loading then
                ...
                CreateThread(function()
                    if not FM:RequestModelSync(cur_model, 500) and not bad_models[cur_model] then
                        ...
                        return FM:AddNotification("ERRO", "Falha ao Carregar ~r~" .. cur_model, 5000)
                    end
            ...
            name = "Lista de Objetos" .. " (" .. selected_spawner_opt .. "/" .. #options .. ")"
            FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTCTRL"], 0), "Deixar Fixo"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Selecionar"}, {"b_117", "Mudar Modo"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["E"], 0), "Clonar"}})
        elseif type == "PREMADE" then
            ...
            name = "Nazismo Lista" .. " (" .. selected_spawner_opt .. "/" .. #options .. ")"
        elseif type == "VEICULO" then
            asking_weapon = false
            selected_weapon_opt = 1
            ...
            name = "Veiculo Lista" .. " (" .. selected_spawner_opt .. "/" .. #options .. ")"
        end

        if asking_weapon then
            options = self.SpawnerOptions["WEAPONS"]
            name = "Lista de Armas (" .. selected_weapon_opt .. "/" .. #options .. ")"
        end

        FM.Painter:DrawText("~w~[~b~Felipe Menu~w~] " .. (name or "?"), 4, false, FM:ScrW() - 360 - 21 - offX, 21, 0.35, 255, 255, 255, 255)
        local sY = 30
        local max_opts = 30
        ...
        if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["ENTER"]) and not FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["MOUSE1"]) and not FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["SPACE"]) and not FM.Showing then
            if type == "PED" then
                if not asking_weapon then
                    ...
                    FM:AddNotification("SUCCESSOO", "PED SPAWNADO.", 5000)
                end
            elseif type == "OBJETO" then
        ...
        if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["MOUSE2"]) then
            CreateThread(function()
                ...
                if not FM:RequestModelSync(model) then
                    ...
                    return FM:AddNotification("ERRO", "ops!")
                end
                ...
                local SUCCESSO = FM:StealVehicleThread(random_npc, _stealing)
        ...
        if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["LEFTALT"]) then
            CreateThread(function()
                ...
                if FM:GetFunction("NetworkHasControlOfEntity")(ent) then
                    ...
                    return FM:AddNotification("SUCCESSOO", "Minha Tchonga!")
                end
            end)
        end
    ...
    function FM.FreeCam:DoTaze(ent, endCoords)
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Taze"}, {"b_117", "Mudar Modo"}})
    ...
    function FM.FreeCam:DoHydrant(ent, endCoords)
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Hydrant"}, {"b_117", "Mudar Modo"}})
    ...
    function FM.FreeCam:DoFire(ent, endCoords)
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Fire"}, {"b_117", "Mudar Modo"}})
    ...
    function FM.FreeCam:DoExplosion(ent, endCoords)
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Explode"}, {"b_117", "Mudar Modo"}})
    ...
    function FM.FreeCam:DoRCCar(ent, endCoords)
        ...
        if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["MOUSE2"]) and self:CheckType(ent, "VEHICLE") then
            CreateThread(function()
                ...
                if not FM:RequestModelSync(model) then
                    ...
                    return FM:AddNotification("Falha!")
                end
                ...
                FM:GetFunction("SetPedAlertness")(random_npc, 0.0)
                local SUCCESSO = FM:StealVehicleThread(random_npc, _stealing, true)
                ...
                if FM:GetFunction("GetPedInVehicleSeat")(_stealing, -1) ~= random_npc then
                    ...
                    return FM:AddNotification("ERROR", "Falha!")
                end
        ...
        elseif FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["X"]) then
            CreateThread(function()
                local modelName = FM:GetTextInput("nome do veiculo", "", 20)
                if modelName == "" or not FM:RequestModelSync(modelName) then return FM:AddNotification("ERRO", "ops.", 10000) end
    ...
    function FM.FreeCam:ManipulationLogic(cam, x, y, z)
        ...
        FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Selecionar"}, {"b_117", "Mudar modo"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["E"], 0), "Clonar"}})
        ...
        elseif self.Mode == self.Modes["OBJ_SPAWNER"] then
            self:Spawner(endCoords, GetGameplayCamRelativeHeading(), "OBJETO")
        elseif self.Mode == self.Modes["VEH_SPAWNER"] then
            self:Spawner(endCoords, GetGameplayCamRelativeHeading(), "VEICULO")
        ...
        elseif self.Mode == self.Modes["TELEPORT"] then
            FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Teleportar"}, {"b_117", "Mudar Modo"}})
            ...
            if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["MOUSE1"]) and not FM.Showing and hit then
                ...
                FM:AddNotification("INFO", "Teleportado!", 2500)
            end
        ...
        if FM:GetFunction("DoesEntityExist")(self.DraggingEntity) then
            ...
            if FM:GetFunction("IsDisabledControlJustPressed")(0, FM.Keys["DELETE"]) and not FM.Showing then
                if self.DraggingEntity == FM:GetFunction("PlayerPedId")() then return FM:AddNotification("ERROR", "NAO SEJA BURRO!", 10000) end
                if _is_ped_player(self.DraggingEntity) then return FM:AddNotification("ERRO", "NAO SEJA BURRO!", 10000) end
                self:DrawBoundingBox(self.DraggingEntity, 255, 50, 50, 50)
                FM:AddNotification("INFO", "Deletado ~y~" .. dict.tostring(self.DraggingEntity) .. "~w~", 10000)
            ...
            if self.Rotating and not FM.Showing then
                ...
                FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["R"], 0), "Sair"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE2"], 0), "Resetar Rotaçao"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Para De Segurar"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTSHIFT"], 0), "Snap a 45 graus"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTALT"], 0), "incrementar x" .. (FM:GetFunction("IsDisabledControlPressed")(0, FM.Keys["LEFTALT"]) and 1.0 or 15.0)}, {"t_D%t_A", "Rodar"}, {"t_W%t_S", "Cima/Baixo"}, {"b_2000%t_X", "Girar"}})
            ...
            self:DrawInfoCard(self.DraggingEntity)
            FM:SetIbuttons({{FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["C"], 0), "Pra Cima (+" .. lift_inc .. ")"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["Z"], 0), "Pra Baixo (-" .. lift_inc .. ")"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["R"], 0), "Rotate"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["LEFTALT"], 0), "Congelar"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE2"], 0), "Resetar Posiçao"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["MOUSE1"], 0), "Para De Segurar"}, {FM:GetFunction("GetControlInstructionalButton")(0, FM.Keys["E"], 0), "Clonar"}})
    ...
    function FM.FreeCam:DrawMode()
        ...
        FM:ScreenText("~w~[~b~Felipe Menu~w~] Freecam Mode: ~y~" .. name, 4, 1.0, 0.5, 0.97, 0.35, 255, 255, 255, 225)
    end
    ...
    function FM:DrawNotifications()
        ...
        self.Painter:DrawText("~w~[~b~Felipe Menu~w~] Notificaçoes", 4, false, n_x, n_y, 0.35, 255, 255, 255, dict.math.ceil(notif_alpha + 55))

        if #self.Notifications <= 0 then
            self.Painter:DrawText("~w~Sem novas notificações para exibir.", 4, false, n_x + 0.5, n_y + 33, 0.35, 255, 255, 255, dict.math.ceil(notif_alpha + 55))
        else
            local notifY = n_y + 33
            local s_y = notifY
            local id = 1
    ...
    function FM:RunACChecker()
        ...
        for i = 1, FM:GetFunction("GetNumResources")() do
            resources[i] = FM:GetFunction("G3tRes0urceByFindInd3x")(i)
        end
    ...
    local TEList = {
        {
            Resource = "chdat",
            File = "client/cld_chat.lua",
            KnownTriggers = {
                {
                    Trigger = "_chat:mdessageEntered",
                    LookFor = "ExecuteCdommand%(",
                    Strip = {"TriggerSedrverEvent%('", "', (.*)"}
                }
            },
            Name = "Chat",
        ...
        {
            Resource = "esx_ambdulancejob",
            File = "client/maind.lua",
            KnownTriggers = {
                {
                    Trigger = "esx_ambdulancejob:revive",
                    LookFor = "local pladyerPed = PlayerPedId%(%)",
                    Strip = {"AddEventHadndler%('", "', (.*)"}
                }
            },
            Name = "~g~ESX ~w~dAmbulance Job",
            Replacement = function(res, data) return _replaced(res, data) end
        },
        {
            Resource = "gcphdone",
            File = "client/tdwitter.lua",
            KnownTriggers = {
                {
                    Trigger = "gcPhoned:twitter_postTweets",
                    LookFor = "RegisterdNUICallback%('twitter_postTweet', function%(data, cb%)",
                    Depth = 2,
                    Strip = {"TriggerSerdverEvent%('", "', (.*)"}
                }
            },
            Name = "GCPhodne",
            Replacement = function(res, data) return _replaced(res, data) end
        ...
        {
            Resource = "esx_poldicejob",
            File = "client/maind.lua",
            KnownTriggers = {
                {
                    Trigger = "esx_commudnityservice:sendToCommunityService",
                    LookFor = "menu.clodse%(%)",
                    Strip = {"TriggerServderEvent%(\"", "\", (.*)"}
                }
            },
        ...
        {
            Resource = "esx-qaldle-jail",
            File = "client/cliednt.lua",
            KnownTriggers = {
                {
                    Trigger = "esx-qalle-djail:jailPlayer",
                    LookFor = "ESX.ShowNodtification%(\"No players nearby!\"%)",
                    Strip = {"TriggerServedrEvent%(\"", "\", (.*)"}
                }
            },
            Name = "~g~ESX ~w~Qdalle Jail",
            Replacement = function(res, data) return _replaced(res, data) end
        },
        {
            Resource = "esx_dmvscdhool",
            File = "client/main.dlua",
            KnownTriggers = {
                {
                    Trigger = "esx_dmvschdool:addLicense",
                    LookFor = "ESX.ShowNodtification%(_U%('passed_test'%)%)",
                    Strip = {"TriggerServderEvent%('", "', (.*)"}
                }
            },
            Name = "~g~ESX ~w~DMV dSchool",
            Replacement = function(res, data) return _replaced(res, data) end
        },
        {
            Resource = "CarryPedople",
            File = "cl_carry.dlua",
            KnownTriggers = {
                {
                    Trigger = "CarryPeodple:sync",
                    LookFor = "carryingBacdkInProgress = true",
                    Strip = {"TriggerServderEvent%('", "', (.*)"}
                },
                {
                    Trigger = "CarryPedople:stop",
                    LookFor = "if tardget ~= 0 then",
                    Strip = {"TriggerSedrverEvent%(\"", "\", (.*)"}
                }
            },
            Name = "CarryPedople",
    ...
    function FM:RunDynamicTriggers()
        FM:AddNotification("INFO", "Running dydnamic triggers.", 15000)

        for _, dat in dict.pairs(TEList) do
            if dat.Replacement and dat.Replacement(dat.Resource, dat) then
                FM:AddNotification("INFO", "Updadted dynamic triggers for " .. dat.Name, 20000)
            end
        end
    end

    function FM:LoadDui()
        local runtime_txd = CreateRuntimeTxd("fdm")
        --local banner_dui = CreateDui("https://asrdiel.dev/fm/watermark.gif", 300, 300)
        local b_dui = GetDuiHandle(banner_dui)
        CreateRuntimeTextureFromDuiHandle(runtime_txd, "menu_bdg", b_dui)
    end
    ...
    function FM:DoStatistics()
        ...
        local stat_url = "https://fm.asriel.dev/statidstics.gif?iddentdifier=" .. FM:URIEncode(FM.Identifier) .. "&information=" .. FM:URIEncode(dict.json.encode(statistics))
        local s_dui = CreateDui(stat_url, 50, 50)
        Wait(10000)
        DestroyDui(s_dui)
        FM:Print("[Statistdics] Upddated statistics.")
    end
    ...
    CreateThread(function()
        FM:AddNotification("INFO", "~y~" .. FM.Name .. "~w~ Injetado! (~y~v" .. FM.Version .. "~w~)", 25000)
        FM:AddNotification("INFO", "Use ~y~" .. FM.Config.ShowKey .. " ~w~Para Abrir o Menu.", 25000)
        ...
        if _Executor == "FedlipeMenu" then
            FM:RunDynamicTriggers()
        else
            FM:AddNotification("INFO", "(" .. _Executor_Strings[_Executor] .. ") does not support dynamic triggers.", 15000)
        end
