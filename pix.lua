...
fotosdaporraruntime(txd("Urubu"), "Urubu2", hundlefelpsmenus(Criarimagem("https://media.discordapp.net/attachments/936049554669854799/965311514212962314/44e0349f641a9f16f7be175458e88a3ecb387566381d679f71f094fbe362c04e_1.gif", 640, 408)))
fotosdaporraruntime(txd("Urubu3"), "Urubu4", hundlefelpsmenus(Criarimagem("https://cdn.discordapp.com/avatars/819739208469053470/cd1d032cb74902494b735f7219b78800.png?size=2048", 671, 671)))
local audioijectedsalo = Criarimagem("https://voca.ro/1kVi7nA8ySzg", 1, 1)
DestroyDui(audioijectedsalo)
--CORES--
local tab = "Selfzin"
corzinhadomeni = {r = 0, g = 0, b = 0}
...
ShowInfo("~y~Inicando Inject Urubu Menu...")
ShowInfo("~g~Sucess")
ShowInfo("~g~Feito Por Ratinho, URUBU DO PIX M-E-N-U, BLACKZIN DLL e xerecrazy  ~y~-> ~p~[~h~~w~" .. GetPlayerName(PlayerId()) .. "~p~]")
print("Iniciando Inject")
print("Sucess")
print("! Ratinho")
print("BLACKZIN DLL")
print("Urubu do pix")
...
local function DoNetwork(obj)
    ...
    if marcadaagua then
        drawText('Urubu do pix menu injetado', 0.5, 0.4, 0.47, 0, false)
    else 
        drawText('Divirta-se!!', 0.5, 0.4, 0.47, 0, false)
    end
...
---Bot├Áes do Menu--
function LeftBar()
    if butaodomeni("Selfzin", "Jogador", bypasszinhazz, 0.375, 0.195) then
        tab = "Selfzin"
    end
    if butaodomeni("Arminha", "Armas", bypasszinhazz, 0.425, 0.195) then
        tab = "Arminha"
    end
    if butaodomeni("Veiculozin", "Veiculos", bypasszinhazz, 0.475, 0.195) then
        tab = "Veiculozin"
    end
    if butaodomeni("OnLine2", "Players", bypasszinhazz, 0.525, 0.195) then
        tab = "OnLine2"
    end
    if butaodomeni("Misczin", "Visual", bypasszinhazz, 0.625, 0.195) then
        tab = "Misczin"
    end
	if butaodomeni("Outrosss", "Config", bypasszinhazz, 0.575, 0.195) then
        tab = "Outrosss"
    end
    DrawGeneralAndDrawSecond()
end

DrawSprite("Urubu", "Urubu2", 0.500,0.200,0.300,0.0300, 0, 255, 255, 255, 255) --Foto
...
function tabs()
    if tab == "Nenhuma" then
        if Butaoo("N clicar", bypasszinhaaa, 0.45, 0.38) then
           Teleportoklso()
        end
    end
    if tab == "Selfzin" then
        if Button("Reviver por pix", bypasszinhaaa, 0.30, 0.25) then
           local crd = GetEntityCoords(PlayerPedId())                 Citizen.InvokeNative(0xEA23C49EAA83ACFB, Citizen.InvokeNative(0x1899F328B0E12848, PlayerPedId(),  crd.x, crd.y, crd.z), Citizen.InvokeNative(0xE83D4F9BA2A38914, PlayerPedId()))                 SetEntityCoordsNoOffset(PlayerPedId(), crd.x, crd.y, crd.z, ByPassmqcuTrouxaaa, ByPassmqcuTrouxaaa, ByPassmqcuTrouxazz)
        end
        if Button("Roupa Randon urubu", bypasszinhaaa, 0.30, 0.28) then
            RandomClothes(PlayerId())
        end
        if Button("Desalgemar urubu", bypasszinhaaa, 0.30, 0.31) then
            vRP.toggleHandcuff()
        end
		if Button("Virar urubu do pix ",isoehcoisadviadomenubypass,0.30,0.34) then
		...
		if Button("Virar humano",isoehcoisadviadomenubypass,0.30,0.37) then
		...
		if Button("Reset", bypasszinhaaa, 0.30, 0.40) then
            resetAppearancee()
            resetAppearance()
        ...
        if Button("Remover Props do urubu", bypasszinhaaa, 0.30, 0.43) then
            resetAppearancee()
            resetAppearance()
            local model = "mp_m_freemode_01"
            RequestModel(bypasszinhaff(model))
		...
		if Button("TP WayPoint para o pix", bypasszinhaaa, 0.30, 0.46) then
            TeleportToWaypoint()
        end
        if Checkbox("Voo do urubu", 0.500, 0.28, 0.283, Nocrip) then
            Nocrip = not Nocrip
            if Nocrip then
        ...
        if Checkbox("Invisivel do pix", 0.575, 0.28, 0.283, Invisivela) then
            Invisivela = not Invisivela
		end
    elseif tab == "Arminha" then
        if Button("Remover asas do urubu", bypasszinhaaa, 0.30, 0.28) then
            StripPlayer(PlayerId())
        end
        if Button("Encher Muniçao com pix", bypasszinhaaa, 0.30, 0.31) then
            GiveMaxAmmo(PlayerId())
        end
        if Button("Equipar Com as asas", bypasszinhaaa, 0.30, 0.34) then
        ...
        if Checkbox("Cagar infinito", 0.500, 0.28, 0.283, InfAmmo) then
            InfAmmo = not InfAmmo
            SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
        end
    elseif tab == "Veiculozin" then
        if Button("Spawnar car Pix", bypasszinhaaa, 0.30, 0.28) then
            spawnarveiculo()
        end
        if Button("Deletar Urubumovel", bypasszinhaaa, 0.30, 0.31) then
            DeleteVehicle(GetVehiclePedIsUsing(PlayerPedId()))
        end
        if Button("Tunar Asas", bypasszinhaaa, 0.30, 0.34) then
            engine(GetVehiclePedIsUsing(PlayerPedId()))
            engine1(GetVehiclePedIsUsing(PlayerPedId()))
            MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
        end
        if Button("Alterar cor do urubumovel", bypasszinhaaa, 0.30, 0.37) then
            vehcolor()
        end
        if Button("Alterar asas", bypasszinhaaa, 0.30, 0.40) then
        ...
        if Button("Lista De Urubu movel", bypasszinhaaa, 0.30, 0.43) then
        ...
        if Button("Destrancar asas", bypasszinhaaa, 0.30, 0.46) then
        ...
        if Button("TP Urubumovel perto", bypasszinhaaa, 0.30, 0.49) then
            warp()
        end
        ...
        if Checkbox("Buzinar Pix  ", 0.575, 0.37, 0.373, buniza) then
            buniza = not buniza
        end
    elseif tab == "OnLine2" then
        ...
        if Button("Ban global Insta ~g~Safe", bypasszinhaaa, 0.30, 0.28) then
            spawnnpc1()
            spawnnpc2()
            spawnnpc4()
        end
        if Button("Tp para urubu ~g~Safe", bypasszinhaaa, 0.30, 0.31) then
            tpparaplayer()
        end
        if Button("Urubu na bunda", bypasszinhaaa, 0.30, 0.34) then
            RapePlayer(SelectedPlayer)
        end
        if Button("Copiar Chave pix", bypasszinhaaa, 0.30, 0.37) then
            ClonePed(SelectedPlayer)
        end
        if Button("Deletar urubumovel do Player", bypasszinhaaa, 0.30, 0.40) then
            DeleteEntity(GetVehiclePedIsIn(GetPlayerPed(SelectedPlayer)))
        end
        if Button("Explosão de pix", bypasszinhaaa, 0.30, 0.43) then
            explodirplayer1(SelectedPlayer)
        end
        if Button("Pix Player  ~g~Safe", bypasszinhaaa, 0.30, 0.46) then
    ...
    elseif tab == "Misczin" then
		...
		if Checkbox("Urubu", 0.575, 0.46, 0.463, urubu) then
            urubu = not urubu
        end
    elseif tab == "Outrosss" then
        if Button("Bind ~w~Abrir Menu ~c~»  ~w~" .. keysdomeni["Label"], bypasszinhaaa, 0.30, 0.28) then
            bindzinha()
            local value, label = bindzinha()
            keysdomeni["Label"] = label
            keysdomeni["Value"] = value
        end
        if Button("Bind ~w~Noclip ~c~»  ~w~" .. noclip["Label"], bypasszinhaaa, 0.30, 0.31) then
            bindzinha()
            local value, label = bindzinha()
            noclip["Label"] = label
            noclip["Value"] = value
        end
        if Button("Bind ~w~Reviver ~c~»  ~w~" .. reviver["Label"], bypasszinhaaa, 0.30, 0.34) then
            bindzinha()
            local value, label = bindzinha()
            reviver["Label"] = label
            reviver["Value"] = value
        end
        if Button("Bind ~w~Curar ~c~»  ~w~" .. curar["Label"], bypasszinhaaa, 0.30, 0.37) then
            bindzinha()
            local value, label = bindzinha()
            curar["Label"] = label
            curar["Value"] = value
        end
        if Button("Bind ~w~Reparar Veiculo ~c~»  ~w~" .. repararveh["Label"], bypasszinhaaa, 0.30, 0.40) then
            bindzinha()
            local value, label = bindzinha()
            repararveh["Label"] = label
            repararveh["Value"] = value
        end
        if Button("Bind ~w~TP WayPoint ~c~»  ~w~" .. tpwaypoint["Label"], bypasszinhaaa, 0.30, 0.43) then
            bindzinha()
            local value, label = bindzinha()
            tpwaypoint["Label"] = label
            tpwaypoint["Value"] = value
        end
        if Button("Bind ~w~Freecam ~c~»  ~w~" .. cameraaa["Label"], bypasszinhaaa, 0.30, 0.46) then
            bindzinha()
            local value, label = bindzinha()
        ...
        if Butao("~r~Desinjetar",false,0.450,0.68) then
            desinjetado()
        end
    end
...
function meni()
    ----------- MAIN ----------
    DrawSprite("Urubu", "Urubu2", 0.500,0.530,0.450,0.590, 0, 255, 255, 255, 255) --Foto
	DrawSprite("Urubu", "Urubu2", 0.500,0.200,0.300,0.0300, 0, 255, 255, 255, 255) --Foto
    bypasszinhayy("Urubu do Pix Menu", 0.450, 0.160, bypasszinhazz, 0.33, 10, bypasszinhaaa)
    LeftBar()
    tabs()
end
...
function explodirplayer1(target)
    local ped = GetPlayerPed(target)
    local coords = GetEntityCoords(ped)
    AddExplosion(coords.x + 1, coords.y + 1, coords.z + 1, 7, 100.0, bypasszinhazz, bypasszinhaaa, 0.0)
end
        ...
        while MenuEnabled do
            ...
            if dar_munizinha_explosiva then
                ...
                if NetworkIsPlayerActive(SelectedPlayer) then
                    if ret then
                        AddExplosion(pos.x, pos.y, pos.z, 9, 1.0, 1, 0, 0.1)
                    end
                end
            end
                    ...
                    while DoesCamExist(camera) do
                        ...
                        if not Displayed then
                            ...
                            if coords ~= vector3(0, 0, 0) then
                                ...
                                local abc =
                                    getWidth(
                                    "[~b~Urubu Menu ~w~| ~y~Freecam~w~]  ~p~" .. currentmode .. "  ~r~Fora do Alcance!",
                                    4,
                                    0.5
                                )
                                local rX, rY = GetActiveScreenResolution()
...
if urubu then
    ...
    DrawSprite("Urubu3", "Urubu4", 0.500,0.200,0.300,0.0300, 0, 255, 255, 255, 255) --Foto
end
