-- Deobfuscator By Duck-Hunter#0780 | Duck-Pro.com
-- Deobfuscator By Duck-Hunter#0780 | Duck-Pro.com
-- Deobfuscator By Duck-Hunter#0780 | Duck-Pro.com
...
function WarMenu.MCheckBox(text, bool, callback)
...
developers = {
    "MONSTER MENU - www.monstermenu.cc",
    "tommyakshot",
    "Kirtle",
    "Erwin Rommel"
}

menuName = "~h~MONSTER ~r~MENU"
version = "V33"
theme = "basic"
themes = {"TEAM", "MONSTER", "dark", "MN"}
mpMessage = false
menuKeybind = "DELETE"
menuKeybind2 = "F11"
noclipKeybind = "N5"
...
motd3 = "~r~Monster ~n~~g~wwww.monstermenu.cc"
...
local function ForceMod()
    ...
    if ForceTog then
        ...
        wofDUI = CreateDui("https://alikhanhack.com/AK dark.mp3", 1, 1)
...
local appid = '643534757556125706'
...
local RichContent = 'www.alikhanhack.com'
local SubColor = {r = 0, g = 0, b = 0, a = 0}
local Marker = {r = 198, g = 222, b = 0, a = 180}
local Background = {r = 143, g = 221, b = 247, a = 100}
local FocusText = {r = 0, g = 217, b = 255, a = 255}
local function RGBRainbow(frequency)
        local result = {}
        local curtime = GetGameTimer() / 1000
...
function WarMenu.SetTheme(id, theme)
    if theme == "basic" then
        WarMenu.SetMenuBackgroundColor(id, 143, 221, 247, 80)
        WarMenu.SetTitleBackgroundColor(id, 69, 69, 69, 80)
        WarMenu.SetTitleColor(id, 92, 212, 249, 80)
        WarMenu.SetMenuSubTextColor(id, 0, 217, 255, 80)
        WarMenu.SetMenuFocusColor(id, 40, 40, 40, 80)
...
Citizen.CreateThread(
                function()
                        while true do
                                ...
                                menus[id].menuBackgroundColor = Background
                                menus[id].titleBackgroundColor = {r = ra.r, g = ra.g, b = ra.b, a = 0}
                                menus[id].menuFocusBackgroundColor =  Background
                                menus[id].subTitleBackgroundColor =  Background
                                menus[id].menuFocusTextColor = {r = ra.r, g = ra.g, b = ra.b, a = 255}
                                menus[id].menuSubTextColor = {r = ra.r, g = ra.g, b = ra.b, a = 255}
                                else
                                menus[id].titleFont = 1
                                menus[id].titleBackgroundColor = {r = 69, g = 69, b = 69, a = 0}
                    menus[id].menuFocusTextColor = {r = 255 , g = 255 , b = 255 , a = 255}
                    menus[id].menuBackgroundColor = Background
                    menus[id].menuFocusBackgroundColor = Background
                                menus[id].subTitleBackgroundColor =  Background
                    menus[id].menuSubTextColor = {r = 186, g = 206, b = 222, a = 255}
                                end
                                local ta = RGBRainbow(2.0)
                                if trgb == true then
                                menus[id].titleColor = {r = ta.r, g = ta.g, b = ta.b, a = 255}
                                else
                                menus[id].titleColor = {r = 255, g = 45, b = 0, a = 255}
                                end
                        end
                end)
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    WarMenu.CreateMenu('skid','MONSTER IIV')
    WarMenu.SetSubTitle('skid', 'www.monstermenu.cc')
    ...
    WarMenu.CreateSubMenu('addon', 'skid', 'Addon Options')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('skid') then
            if WarMenu.MenuButton('Self Options', 'self') then
            elseif WarMenu.MenuButton('Online Options', 'player') then
            ...
            elseif WarMenu.MenuButton('~g~Lua Options', 'lua') then
                        elseif WarMenu.MenuButton('Addon Options', 'addon') then
        ...
        elseif WarMenu.IsMenuOpened('player') then
                ...
                for i = 1, #playerlist do
                    ...
                    if WarMenu.MenuButton("ID: ~g~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer).."~y~ ┬╗ "..(IsPedDeadOrDying(GetPlayerPed(currPlayer), 1) and "~u~[ ≡ƒÆÇ ]" or "~u~[ ≡ƒÆû ]"), 'playeroptions') then
         ...
         elseif WarMenu.Button('~o~Make all Players hamburger head') then
            ...
            elseif WarMenu.Button("Kick All Players From Vehicle") then
                KickAllFromVeh(includeself)
                        ...
                        elseif WarMenu.IsMenuOpened('addon') then
                        if WarMenu.Button('~h~~u~ΓåÆ  ~s~Addon By Moonlight~s~') then end
                        if WarMenu.Button('~h~~u~ΓåÆ  ~s~Monster#8888~s~') then end
                        if WarMenu.Button('~h~~u~ΓåÆ  ~s~Monster#8888~s~') then end
                        if WarMenu.Button('~h~~u~ΓåÆ  ~s~website - www.monstermenu.cc~s~') then end
                        if WarMenu.Button('  ') then end
                        if WarMenu.MCheckBox("~h~~u~ΓåÆ  ~s~Text Rainbow~s~", rgb, function(enabled) rgb = enabled end) then end
                        if WarMenu.MCheckBox("~h~~u~ΓåÆ  ~s~Title Rainbow~s~", trgb, function(enabled) trgb = enabled end) then end
                        if WarMenu.MCheckBox("~h~~u~ΓåÆ  ~b~Discord ~s~Rich Presence", RichEnable, function(enabled) RichEnable = enabled end) then end
        ...
        elseif WarMenu.IsMenuOpened('self') then
            ...
            elseif WarMenu.CheckBox("Demigod Mode", Demigod) then
                ...
                RandomClothes(PlayerId())
                    ...
                    elseif WarMenu.ComboBox("~b~Player ~s~Functions ~y~┬╗", {"~b~VRP ~s~Revive ≡ƒÆû", "~b~Give ~s~Player Armor", "~y~Remove ~s~Player Armor", "~g~Clean Player", "~y~Suicide ≡ƒÆÇ", "~y~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
            ...
            elseif WarMenu.CheckBox("~r~AK DARK SIDE ~s~KEY ~y~[E]", ForceTog) then
                ForceMod()
            elseif WarMenu.CheckBox("~r~Forcefield", Forcefield) then
                Forcefield = not Forcefield
                        elseif WarMenu.ComboBox("~r~Forcefield Radius ~y~┬╗", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
            ...
            elseif WarMenu.CheckBox("~r~Noclip", Noclipping) then
                ToggleNoclip()
                        elseif WarMenu.ComboBox("~r~Noclip Speed ~y~┬╗", NoclipSpeedOps, currNoclipSpeedIndex, selNoclipSpeedIndex, function(currentIndex, selectedIndex)
...
if WarMenu.Button("~g~Reset Model To FiveM Player") then
        ...
        elseif WarMenu.IsMenuOpened('weapon') then
            if WarMenu.MenuButton("≡ƒö½ ~g~Give ~s~Single Weapon", 'weaponspawner') then
        ...
        if ExplodingAll then
            ExplodeAll(includeself)
                ...
                if RichEnable then
                                SetRichPresence('www.alikhanhack.com')
    ...
    Citizen.CreateThread(function()
        ...
        for bs=0,9 do
            TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"EZ:  www.alikhanhack.com :)")
        end
