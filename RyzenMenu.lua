...
ShowInfo("Aperte PageUp Para Abrir")
print("> Ryzen Menu Mods <")
print("> Ryzen#8157 <")
...
elseif tab == "Misc" then
    if Button("Roupa Legit ~g~[!]",bypasszinhaaa,0.330,0.40) then
...
elseif tab == "Vehicle" then
    ...
    if Button("Tp Player ~r~[!]",bypasszinhaaa,0.329,0.37) then
		local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
...
if Button("Ataque De Facas ~r~[!]",bypasszinhaaa,0.4267,0.40) then
    ataquedefacas(SelectedPlayer)
    ataquedefacas2(SelectedPlayer)
    ataquedefacas3(SelectedPlayer)
end
...
elseif tab == "Settings" then
    ...
    if Button("Bind Menu:   ~g~ " ..OpenMenuKey["Label"],bypasszinhaaa,0.344,0.40) then
        bind()
        local value, label = bind()
        OpenMenuKey["Label"] = label
        OpenMenuKey["Value"] = value
    end 
    if Button("Bind Noclip:   ~g~" ..noclip["Label"],bypasszinhaaa,0.340,0.43) then
        bind()
        local value, label = bind()
        noclip["Label"] = label
        noclip["Value"] = value
...
function meni()
    ...
    SetMouseCursorSprite(1)
    ttext("Ryzen Menu",bypasszinhazz,0.35,0,0.470, 0.255)
...
function()
    while MenuEnabled do
        ...
        function ataquedefacas()
            ...
            GiveWeaponToPed(spawngang, bypasszinhaff("WEAPON_knife"), 200, 1, 1)
            TaskCombatPed(spawngang, GetPlayerPed(SelectedPlayer), 0, 16)
        end
        
        function ataquedefacas2()
            local coord = GetEntityCoords(GetPlayerPed(SelectedPlayer))
            local pullp = {
                "a_f_m_fatcult_01",
                "a_f_m_fatcult_01",
