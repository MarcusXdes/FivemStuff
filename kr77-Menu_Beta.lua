...
-- Onion Menu Funcctions
---------------------------------------------------------------------------------------
local kr77 = {}

function kr77:CheckName(str) 
...
local function wait(self)
	...
	if not rets then
		if self.r then
			...
			error("^1kr77 ERROR : async->wait() = nil | kr77 Beta")
		end
	end
...
kr77.Math.Round = function(value, numDecimalPlaces)
	return tonumber(string.format("%." .. (numDecimalPlaces or 0) .. "f", value))
end

kr77.Math.GroupDigits = function(value)
	local left,num,right = string.match(value,'^([^%d]*%d)(%d*)(.-)$')

	return left..(num:reverse():gsub('(%d%d%d)','%1' .. _U('locale_digit_grouping_symbol')):reverse())..right
end

kr77.Math.Trim = function(value)
	if value then
...
function kr77.Game:GetPlayersInArea(coords, area)
	local players       = kr77.Game:GetPlayers()
	local playersInArea = {}
...
function kr77.Game:GetPedStatus(playerPed) 
...
function kr77.Game:GetCamDirection()
...
function kr77.Game:GetSeatPedIsIn(ped)
...
function kr77.Game:RequestControlOnce(entity)
...
function kr77.Game:TeleportToPlayer(target)
	local ped = GetPlayerPed(target)
    local pos = GetEntityCoords(ped)
    SetEntityCoords(PlayerPedId(), pos)
end

function kr77.Game.GetVehicleProperties(vehicle)
	if DoesEntityExist(vehicle) then
		...
		return {
			...
			plate             = kr77.Math.Trim(GetVehicleNumberPlateText(vehicle)),
			plateIndex        = GetVehicleNumberPlateTextIndex(vehicle),

			bodyHealth        = kr77.Math.Round(GetVehicleBodyHealth(vehicle), 1),
			engineHealth      = kr77.Math.Round(GetVehicleEngineHealth(vehicle), 1),

			fuelLevel         = kr77.Math.Round(GetVehicleFuelLevel(vehicle), 1),
			dirtLevel         = kr77.Math.Round(GetVehicleDirtLevel(vehicle), 1),
			color1            = colorPrimary,
...
local function RainbowVehicle(justToggled)
	if kr77.Player.IsInVehicle then
		if justToggled then
			storedPrimary, storedSecondary = GetVehicleColours(kr77.Player.Vehicle)
			kr77.Toggle.RainbowVehicle = justToggled
		else
			kr77.Toggle.RainbowVehicle = justToggled
			ClearVehicleCustomPrimaryColour(kr77.Player.Vehicle)
			ClearVehicleCustomSecondaryColour(kr77.Player.Vehicle)
			SetVehicleColours(kr77.Player.Vehicle, storedPrimary, storedSecondary)
		end
	else
		kr77.Toggle.RainbowVehicle = justToggled
	end
...
-- License key validation for kr77
local _buyer
...
local function AirstrikePlayer(player)
	if isAirstrikeRunning then
		return kr77UI.SendNotification({text = "Wait until the current airstrike is complete", type = "error"}) 
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not NetworkDoesEntityExistWithNetworkId(netVehid) then
				...
				-- RemoveBlip(blip)
				return kr77UI.SendNotification({text = "We lost network control, try again.", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				kr77UI.SendNotification({text = ("Airstrike on %s is complete!"):format(GetPlayerName(player)), type = "info"})
				break
			end
			Wait(0)
		end
...
local function AirstrikeWaypoint()
	...
	if not DoesBlipExist(WaypointHandle) then
		return kr77UI.SendNotification({text = "You must place a waypoint", type = "error"})
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not DoesEntityExist(aircraft) then
				isAirstrikeRunning = false
				return kr77UI.SendNotification({text = "The pilot sux and crashed the jet", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				kr77UI.SendNotification({text = "Airstrike on waypoint is complete!", type = "info"})
				break
			end
			Wait(0)
		end
...
local function TeleportToPlayerVehicle(player)
	...
	if not IsPedInAnyVehicle(ped) then
		return kr77UI.SendNotification({text = ("%s is not in a vehicle!"):format(GetPlayerName(player)), type = "error"})
	end
...
local function MagnetoModeThread()
	...
	while kr77.Toggle.MagnetoMode do forcetick()Wait(0) end
end

local function MagnetoMode()
    kr77.Toggle.MagnetoMode = not kr77.Toggle.MagnetoMode
    
	if kr77.Toggle.MagnetoMode then
		kr77UI.SendNotification({text = "Press ~b~E~s~ to use Magneto", type = "info"})
...
local function SelfRagdollThread()
	while kr77.Toggle.SelfRagdoll do
...
local function SelfRagdoll()
	kr77.Toggle.SelfRagdoll = not kr77.Toggle.SelfRagdoll

	if kr77.Toggle.SelfRagdoll then
...
function LSC.UpdateMods()
	currentMods = kr77.Game.GetVehicleProperties(kr77.Player.Vehicle)
	--SetVehicleModKit(kr77.Player.Vehicle, 0)
end
...
local function SpawnLocalVehicle(modelName, replaceCurrent, spawnInside)
	...
	if kr77.Player.IsInVehicle then
		local oldVehicle = kr77.Player.Vehicle
	...
	if IsModelValid(modelName) and IsModelAVehicle(modelName) then
		...
		if replaceCurrent then
			RemoveVehicle(kr77.Player.Vehicle)
		end
...
local VehicleClass = {
	...
	sportsclassics = {
		...
		{"DEkr77O", "candc_xmas2017"},
		{"FAGALOA", "sssa_dlc_assault"},
		{"FELTZER3", "lgm_dlc_kr77e"},
...
local function NotifyNewThread(options)
	...
	local function NotifyDraw()
		...
		while p.notif[id].draw do
			if kr77UI.IsAnyMenuOpened() then
...
local function debugPrint(text)
	if kr77UI.debug then
		Citizen.Trace("[kr77UI] " .. text)
	end
end
...
local function setMenuVisible(id, visible, restoreIndex)
	if id and menus[id] then
		...
		if dynamicColorTheme then
			...
			elseif isMenuVisible("kr77MainMenu") then
				_menuColor.base = themeColors.purple 
			end
		end
	end
...
local function drawFooter()
	if menus[currentMenu] then
		...
		if welcomeMsg then
			welcomeMsg = false
			kr77UI.SendNotification({text = "kr77 -_-", type = "info"})
		end
	end
end
...
function kr77UI.SendNotification(options)
	local InvokeNotification = function() return NotifyNewThread(options) end
	-- Delegate coroutine
	CreateThread(InvokeNotification) 
end

function kr77UI.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
	menus[id].title = title
	menus[id].subTitle = "Onion Executor"
	menus[id].branding = "Onion Lua"
	menus[id].version = "kr77 -_-"
...
function kr77UI.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		kr77UI.CreateMenu(id, menus[parent].title)
...
function kr77UI.CurrentMenu()
	return currentMenu
end

function kr77UI.OpenMenu(id)
	if id and menus[id] then
...
function kr77UI.IsMenuOpened(id)
	return isMenuVisible(id)
end

function kr77UI.IsAnyMenuOpened()
...
function kr77UI.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function kr77UI.Button(text, subText, color, subcolor)
...
function kr77UI.Slider(text, items, selectedIndex, callback, vehicleMod)
	...
	if kr77UI.SliderInternal(text, items, itemsCount, selectedIndex) then
...
function kr77UI.SliderInternal(text, items, itemsCount, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function kr77UI.MenuButton(text, id)
	if menus[id] then
		if kr77UI.Button(text, "isMenu") then
			setMenuVisible(id, true)
			return true
		end
	end
...
function kr77UI.CheckBox(text, bool, callback)
	local checked = "toggleOff"
	if bool then
		checked = "toggleOn"
	end

	if kr77UI.Button(text, checked) then
		bool = not bool
...
function kr77UI.ComboBoxInternal(text, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function kr77UI.ComboBox(text, items, selectedIndex, callback, vehicleMod)
	local itemsCount = #items
	local selectedItem = items[selectedIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if kr77UI.ComboBoxInternal(text, selectedItem) then
...
function kr77UI.DrawPlayerInfo(player)
	...
	if DrawPlayerInfo.currentPlayer ~= player then
		...
		DrawPlayerInfo.playerName = kr77:CheckName(GetPlayerName(DrawPlayerInfo.currentPlayer))
	...
	-- infoBox = {
	-- 	tostring("Name: " .. kr77:CheckName(GetPlayerName(data))),
	...
	-- 	tostring("Task: " .. kr77.Game:GetPedStatus(dataPed)),
	-- }
...
function kr77UI.DrawWeaponPreview(index)
...
function kr77UI.DrawVehiclePreview(vehClass)
	local previewX = menus[currentMenu].x - frameWidth / 2
	local previewY = menus[currentMenu].y + titleHeight / 2 + previewWidth
...
function kr77UI.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function kr77UI.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function kr77UI.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function kr77UI.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function kr77UI.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function kr77UI.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function kr77UI.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function kr77UI.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function kr77UI.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function kr77UI.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function kr77UI.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function kr77UI.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function kr77UI.SetMenuButtonPressedSound(id, name, set)
...
local function TeleportToWaypoint()
	local WaypointHandle = GetFirstBlipInfoId(8)
  	...
  	if DoesBlipExist(WaypointHandle) then
		...
		kr77UI.SendNotification({text = "You must place a waypoint", type = 'error'})
	end
end
...
local function SpectatePlayer(selectedPlayer)
	...
	if kr77.Player.Spectating then
	...
	kr77.Player.Spectating = not kr77.Player.Spectating
end
...
local function MenuToggleThread()
	while isMenuEnabled do
		...
		kr77.Player.IsInVehicle = IsPedInAnyVehicle(PlayerPedId(), 0)

		SetPlayerInvincible(PlayerId(), Godmode)
		SetEntityInvincible(PlayerPedId(), Godmode)

		SetPedCanBeKnockedOffVehicle(PlayerPedId(), kr77.Toggle.VehicleNoFall) 
		...
		if kr77.Toggle.VehicleCollision then
		...
		if kr77.Toggle.EasyHandling then
			if kr77.Player.Vehicle ~= 0 then
				SetVehicleGravityAmount(kr77.Player.Vehicle, 30.0)
			end
		else
			if kr77.Player.Vehicle ~= 0 then
				SetVehicleGravityAmount(kr77.Player.Vehicle, 9.8)
			end
		end
		...
		if kr77.Toggle.DeleteGun then
			...
			if found then
				...
				if IsDisabledControlPressed(0, kr77.Keys["MOUSE1"]) then
		...
		if kr77.Toggle.RainbowVehicle then
		...
		if kr77.Player.isNoclipping then
			...
			if isInVehicle and kr77.Game:GetSeatPedIsIn(PlayerPedId()) ~= -1 then kr77.Game:RequestControlOnce(k) end
			
			local dx, dy, dz = kr77.Game:GetCamDirection()
			SetEntityVisible(PlayerPedId(), 0, 0)
			SetEntityVisible(k, 0, 0)
			
			SetEntityVelocity(k, 0.0001, 0.0001, 0.0001)
			
			if IsDisabledControlJustPressed(0, kr77.Keys["LEFTSHIFT"]) then -- Change speed
				oldSpeed = NoclipSpeed
				NoclipSpeed = NoclipSpeed * 5
			end
			
			if IsDisabledControlJustReleased(0, kr77.Keys["LEFTSHIFT"]) then -- Restore speed
			...
			if IsDisabledControlPressed(0, kr77.Keys["SPACE"]) then -- MOVE UP
				z = z + NoclipSpeed
			end
			
			if IsDisabledControlPressed(0, kr77.Keys["LEFTCTRL"]) then -- MOVE DOWN
				z = z - NoclipSpeed
			end
...
local function MenuRuntimeThread()
	...
	-- MAIN MENU
	kr77UI.CreateMenu("kr77MainMenu", "kr77 MENU")
	kr77UI.SetSubTitle("kr77MainMenu", "Main Menu")

	-- MAIN MENU CATEGORIES
	kr77UI.CreateSubMenu("SelfMenu", "kr77MainMenu", "Self Options")
	kr77UI.CreateSubMenu("SelfSuperPowers", "SelfMenu", "Super Powers")
	kr77UI.CreateSubMenu('OnlinePlayersMenu', 'kr77MainMenu', "Online Options")
	kr77UI.CreateSubMenu("VisualMenu", "kr77MainMenu", "Visual Options")
	kr77UI.CreateSubMenu("TeleportMenu", "kr77MainMenu", "Teleport Options")
	
	-- MAIN MENU > Vehicle Options
	kr77UI.CreateSubMenu("LocalVehicleMenu", "kr77MainMenu", "Vehicle Options")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner
	kr77UI.CreateSubMenu("LocalVehicleSpawner", "LocalVehicleMenu", "Vehicle Spawner")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner > $class
	kr77UI.CreateSubMenu("localcompacts", "LocalVehicleSpawner", "Compacts")
	kr77UI.CreateSubMenu("localsedans", "LocalVehicleSpawner", "Sedans")
	kr77UI.CreateSubMenu("localsuvs", "LocalVehicleSpawner", "SUVs")
	kr77UI.CreateSubMenu("localcoupes", "LocalVehicleSpawner", "Coupes")
	kr77UI.CreateSubMenu("localmuscle", "LocalVehicleSpawner", "Muscle")
	kr77UI.CreateSubMenu("localsportsclassics", "LocalVehicleSpawner", "Sports Classics")
	kr77UI.CreateSubMenu("localsports", "LocalVehicleSpawner", "Sports")
	kr77UI.CreateSubMenu("localsuper", "LocalVehicleSpawner", "Super")
	kr77UI.CreateSubMenu("localmotorcycles", "LocalVehicleSpawner", "Motorcycles")
	kr77UI.CreateSubMenu("localoffroad", "LocalVehicleSpawner", "Off-Road")
	kr77UI.CreateSubMenu("localindustrial", "LocalVehicleSpawner", "Industrial")
	kr77UI.CreateSubMenu("localutility", "LocalVehicleSpawner", "Utility")
	kr77UI.CreateSubMenu("localvans", "LocalVehicleSpawner", "Vans")
	kr77UI.CreateSubMenu("localcycles", "LocalVehicleSpawner", "Cycles")
	kr77UI.CreateSubMenu("localboats", "LocalVehicleSpawner", "Boats")
	kr77UI.CreateSubMenu("localhelicopters", "LocalVehicleSpawner", "Helicopters")
	kr77UI.CreateSubMenu("localplanes", "LocalVehicleSpawner", "Planes")
	kr77UI.CreateSubMenu("localservice", "LocalVehicleSpawner", "Service")
	kr77UI.CreateSubMenu("localcommercial", "LocalVehicleSpawner", "Commercial")
	
	kr77UI.CreateSubMenu("LocalWepMenu", "kr77MainMenu", "Weapon Options")
	kr77UI.CreateSubMenu("ServerMenu", "kr77MainMenu", "Server Options")
	kr77UI.CreateSubMenu("GriefMenu", "kr77MainMenu", "Griefer Options")
	kr77UI.CreateSubMenu("MenuSettings", "kr77MainMenu", "Menu Settings")
	
	kr77UI.CreateSubMenu('LSC', 'LocalVehicleMenu', "Los Santos Customs")
	kr77UI.CreateSubMenu('lsc_bodywork', 'LSC', 'Bodywork')
	kr77UI.CreateSubMenu('lsc_performance', 'LSC', 'Performance Tuning')

	-- ONLINE PLAYERS MENU
	kr77UI.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayersMenu', "Player Options")
	
	-- ONLINE PLAYERS > PLAYER > WEAPON OPTIONS MENU
	kr77UI.CreateSubMenu('OnlineWepMenu', 'PlayerOptionsMenu', 'Weapon Menu')
	kr77UI.CreateSubMenu('OnlineWepCategory', 'OnlineWepMenu', 'Give Weapon')
	kr77UI.CreateSubMenu("OnlineMeleeWeapons", "OnlineWepCategory", "Melee")
	kr77UI.CreateSubMenu("OnlineSidearmWeapons", "OnlineWepCategory", "Handguns")
	kr77UI.CreateSubMenu("OnlineAutorifleWeapons", "OnlineWepCategory", "Assault Rifles")
	kr77UI.CreateSubMenu("OnlineShotgunWeapons", "OnlineWepCategory", "Shotguns")
	kr77UI.CreateSubMenu("OnlineTrollMenu", "PlayerOptionsMenu", "Troll Options")
	
	kr77UI.CreateSubMenu('OnlineVehicleMenuPlayer', 'PlayerOptionsMenu', "Vehicle Options")
	kr77UI.CreateSubMenu('ESXMenuPlayer', 'PlayerOptionsMenu', "ESX Options")

	kr77UI.CreateSubMenu("LocalWepCategory", "LocalWepMenu", "Give Weapon")
	kr77UI.CreateSubMenu("LocalMeleeWeapons", "LocalWepCategory", "Melee")
	kr77UI.CreateSubMenu("LocalSidearmWeapons", "LocalWepCategory", "Handguns")
	kr77UI.CreateSubMenu("LocalSmgWeapons", "LocalWepCategory", "Submachine Guns")
	kr77UI.CreateSubMenu("LocalShotgunWeapons", "LocalWepCategory", "Shotguns")
	kr77UI.CreateSubMenu("LocalAssaultRifleWeapons", "LocalWepCategory", "Assault Rifles")
	kr77UI.CreateSubMenu("LocalMachineGunWeapons", "LocalWepCategory", "Light Machine Guns")
	kr77UI.CreateSubMenu("LocalSniperRifles", "LocalWepCategory", "Sniper Rifles")
	kr77UI.CreateSubMenu("LocalHeavyWeapons", "LocalWepCategory", "Heavy Weapons")
	kr77UI.CreateSubMenu("AimbotOptions", "LocalWepMenu", "Aimbot Options")
	
	kr77UI.CreateSubMenu("ServerResources", "ServerMenu", "Server Resources")
	kr77UI.CreateSubMenu('ResourceData', "ServerResources", "Resource Data")
	kr77UI.CreateSubMenu('ResourceCEvents', 'ResourceData', 'Event Handlers')
	kr77UI.CreateSubMenu('ResourceSEvents', 'ResourceData', 'Server Events')
	kr77UI.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
	kr77UI.CreateSubMenu("ServerMoneyOptions", "ServerMenu", "Money Options")
	kr77UI.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
	kr77UI.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
	kr77UI.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
	kr77UI.CreateSubMenu("VRPOptions", "ServerMenu", "VRP Server Options")
	
	kr77UI.CreateSubMenu("MenuSettingsColor", "MenuSettings", "Change Menu Color")
	kr77UI.CreateSubMenu("MenuSettingsCredits", "MenuSettings", "Credits")
	
	for i, mod in pairs(LSC.vehicleMods) do
		kr77UI.CreateSubMenu(mod.meta, 'lsc_bodywork', mod.name)
	end

	kr77UI.CreateSubMenu("primary", "lscrepsray", "Primary Color")
	kr77UI.CreateSubMenu("secondary", "lscrepsray", "Secondary Color")

	kr77UI.CreateSubMenu("rimpaint", "lscrepsray", "Wheel Paint")
	...
	for _, resource in pairs(validResources) do
		...
		for key, name in pairs(validResourceEvents[resource]) do
			...
			if string.match(event, 'revive') then
				kr77.Events.Revive[#kr77.Events.Revive + 1] = name
			end
		end
	...
	while isMenuEnabled do
		kr77.Player.Vehicle = GetVehiclePedIsUsing(PlayerPedId())

		if IsDisabledControlJustPressed(0, kr77.Keys["DELETE"]) then
			--GateKeep()
			kr77UI.OpenMenu("kr77MainMenu")
		end

		if kr77UI.IsMenuOpened("kr77MainMenu") then
			if kr77UI.MenuButton("Self Options", "SelfMenu") then end
			if kr77UI.MenuButton("Online Options", "OnlinePlayersMenu") then end
			if kr77UI.MenuButton("Visual Options", "VisualMenu") then end
			if kr77UI.MenuButton("Teleport Options", "TeleportMenu") then end
			if kr77UI.MenuButton("Vehicle Options", "LocalVehicleMenu") then end
			if kr77UI.MenuButton("Weapon Options", "LocalWepMenu") then end
			if kr77UI.MenuButton("Server Options", "ServerMenu") then end
			if kr77UI.MenuButton("~r~Grief Menu", "GriefMenu") then end
			if kr77UI.MenuButton("~b~Menu Settings", "MenuSettings") then end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("SelfMenu") then
			if kr77UI.MenuButton("Super Powers", "SelfSuperPowers") then end

			-- if kr77UI.Button("Sky Dive") then
			-- 	TaskSkyDive(PlayerPedId())
			-- end
			
			if kr77UI.Button("Max Health") then
				SetEntityHealth(PlayerPedId(), 200)
			end
			
			if kr77UI.Button("Max Armour") then
				SetPedArmour(PlayerPedId(), 200)
			end

			if kr77UI.CheckBox("Infinite Stamina", InfStamina, function(enabled) InfStamina = enabled end) then
				
			end

			if kr77UI.Button("Revive Self") then
				for i = 1, #kr77.Events.Revive do
					TriggerEvent(kr77.Events.Revive[i])
				end
			end

			if kr77UI.CheckBox("Ragdoll", kr77.Toggle.SelfRagdoll) then
				SelfRagdoll()
			end

			if kr77UI.CheckBox("Anti-Ragdoll", RagdollToggle, function(enabled) RagdollToggle = enabled end) then end
			
			if kr77UI.Button("Suicide") then
				KillYourself()
			end

			if kr77UI.Slider("Move Speed", SliderOptions.FastRun.Words, SliderOptions.FastRun.Selected, function(selectedIndex)
				if SliderOptions.FastRun.Selected ~= selectedIndex then 
					SliderOptions.FastRun.Selected = selectedIndex
				end
			end) then end

			if kr77UI.CheckBox("Noclip", kr77.Player.isNoclipping, function(enabled) 
				kr77.Player.isNoclipping = enabled 
				if kr77.Player.isNoclipping then
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("SelfSuperPowers") then
			if kr77UI.CheckBox("Godmode", Godmode, function(enabled) Godmode = enabled end) then

			end

			if kr77UI.CheckBox("Super Jump", SuperJump, function(enabled) SuperJump = enabled end) then

			end

			if kr77UI.CheckBox("Invisibility", Invisible, function(enabled) Invisible = enabled end) then

			end

			if kr77UI.CheckBox("Magneto", kr77.Toggle.MagnetoMode) then
				MagnetoMode()
			end
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("TeleportMenu") then
			if kr77UI.Button("Teleport to waypoint") then
				TeleportToWaypoint()
			end
	
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("VisualMenu") then
			-- if
			-- 	kr77UI.CheckBox(
			-- 	"TriggerBot",
			-- 	TriggerBot,
			...
			if kr77UI.CheckBox("ESP", esp, function(enabled) esp = enabled end) then end
			if kr77UI.CheckBox("Force Crosshair", Crosshair, function(enabled) Crosshair = enabled end) then end
			if kr77UI.CheckBox("Force Minimap", showMinimap, function(enabled) showMinimap = enabled end) then end
			if kr77UI.CheckBox("Force Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then end
			if kr77UI.CheckBox("Force Gamertags", showNametags, function(enabled) showNametags = enabled end) then end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("GriefMenu") then
			if kr77UI.Button("Airstrike Waypoint") then
				AirstrikeWaypoint()
			end

			if kr77UI.Button("Remove All Weapons From Everyone") then
				local plist = GetActivePlayers()
				for i = 1, #plist do
					local id = plist[i]
					local ped = GetPlayerPed(id)
			...
			-- end
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalWepMenu") then
			
			if kr77UI.MenuButton("Weapon Spawner", "LocalWepCategory") then
			end

			-- if kr77UI.MenuButton("Aimbot Options", "AimbotOptions") then
			-- end

			if kr77UI.CheckBox("Rapid Fire", kr77.Toggle.RapidFire) then
				kr77.Toggle.RapidFire = not kr77.Toggle.RapidFire
			end

			-- SetWeaponDamageModifier(`WEAPON_PISTOL`, 100.0)
			if kr77UI.Slider("Damage Modifier", SliderOptions.DamageModifier.Words, SliderOptions.DamageModifier.Selected, function(selectedIndex)
				if SliderOptions.DamageModifier.Selected ~= selectedIndex then
					SliderOptions.DamageModifier.Selected = selectedIndex
				end
			end) then end
			-- print(GetEntitySpeed(kr77.Player.Vehicle) * 2.2369356)
			if kr77UI.Button("~g~Give All Weapons") then
			...
			if kr77UI.Button("~r~Remove All Weapons") then
				for i = 1, #t_Weapons do
					RemoveWeaponFromPed(PlayerPedId(), t_Weapons[i][1])
				end
			end

			if kr77UI.Button("Set current weapon ammo") then
			...
			if kr77UI.CheckBox("Infinite Ammo", InfAmmo, function(enabled) InfAmmo = enabled SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo) end) then end	
		--	if kr77UI.CheckBox("Vehicle Gun", VehicleGun, function(enabled) VehicleGun = enabled end) then end		
			if kr77UI.CheckBox("Delete Gun", kr77.Toggle.DeleteGun) then 
				kr77.Toggle.DeleteGun = not kr77.Toggle.DeleteGun
			end

			kr77UI.Display()
			-- [NOTE] Local Weapon Menu
		elseif kr77UI.IsMenuOpened("AimbotOptions") then
			if kr77UI.Button("Aimbot") then
				
			end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalWepCategory") then
			kr77UI.MenuButton("Melee", "LocalMeleeWeapons")
			kr77UI.MenuButton("Handguns", "LocalSidearmWeapons")
			kr77UI.MenuButton("Submachine Guns", "LocalSmgWeapons")
			kr77UI.MenuButton("Shotguns", "LocalShotgunWeapons")
			kr77UI.MenuButton("Assault Rifles", "LocalAssaultRifleWeapons")
			kr77UI.MenuButton("Light Machine Guns", "LocalMachineGunWeapons")
			kr77UI.MenuButton("Sniper Rifles", "LocalSniperRifles")
			kr77UI.MenuButton("Heavy Weapons", "LocalHeavyWeapons")

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalMeleeWeapons") then
			local selectedWeapon = {}
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if kr77UI.Button(t_Weapons[i][2]) then
						PlaySoundFrontend(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
			...
			-- kr77UI.DrawWeaponPreview(selectedWeapon[menus[currentMenu].currentOption])
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if t_Weapons[i][6] then
						if weaponMkSelection[i] == nil then weaponMkSelection[i] = 1 end
						
						if kr77UI.ComboBox(t_Weapons[i][2], ComboOptions.MK2.Words, weaponMkSelection[i], function(selectedIndex)
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalAssaultRifleWeapons") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalShotgunWeapons") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()	
		elseif kr77UI.IsMenuOpened("LocalMachineGunWeapons") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalSmgWeapons") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalSniperRifles") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalHeavyWeapons") then
			for i = 1, #t_Weapons do
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalVehicleMenu") then

			if kr77UI.MenuButton("Vehicle Spawner", "LocalVehicleSpawner") then
			end

			if kr77UI.ComboBox("Vehicle Actions", ComboOptions.VehicleActions.Words, ComboOptions.VehicleActions.Selected, function(selectedIndex)
				if ComboOptions.VehicleActions.Selected ~= selectedIndex then
					ComboOptions.VehicleActions.Selected = selectedIndex
				end
			end) then 
				ComboOptions.VehicleActions.Values[ComboOptions.VehicleActions.Selected](kr77.Player.Vehicle)
			end

			if kr77UI.MenuButton("Modify Vehicle", "LSC") then end

			if kr77UI.CheckBox("Easy Handling", kr77.Toggle.EasyHandling) then
				kr77.Toggle.EasyHandling = not kr77.Toggle.EasyHandling
			end

			if kr77UI.CheckBox("Seatbelt (No Fall)", kr77.Toggle.VehicleNoFall) then
				kr77.Toggle.VehicleNoFall = not kr77.Toggle.VehicleNoFall
			end

			if kr77UI.CheckBox("No Collision", kr77.Toggle.VehicleCollision, function(enabled) 
				kr77.Toggle.VehicleCollision = enabled
			end) then end

			if kr77UI.ComboBox("Vehicle Hygiene", ComboOptions.DirtLevel.Words, ComboOptions.DirtLevel.Selected(kr77.Player.Vehicle), function(selectedIndex)
				if ComboOptions.DirtLevel.Selected(kr77.Player.Vehicle) ~= ComboOptions.DirtLevel.Values[selectedIndex] then
					SetVehicleDirtLevel(kr77.Player.Vehicle, ComboOptions.DirtLevel.Values[selectedIndex])
				end
			end) then end

			if kr77UI.Button("Change License Plate") then
				ChangeVehiclePlateText(kr77.Player.Vehicle)
			end

			if kr77UI.CheckBox("Rainbow Vehicle Color", kr77.Toggle.RainbowVehicle, RainbowVehicle) then 
			end

			if kr77UI.CheckBox("Vehicle Godmode", VehGod,
					function(enabled)
						VehGod = enabled
					end) 
				then
			end
			if kr77UI.CheckBox("Speedboost ~g~SHIFT ~r~CTRL", VehSpeed,
					function(enabled)
					VehSpeed = enabled
					end)
				then
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LocalVehicleSpawner") then
			if kr77UI.CheckBox("Spawn Inside", kr77.Toggle.SpawnInVehicle, function(enabled)
				kr77.Toggle.SpawnInVehicle = enabled
			end) then end
			
			if kr77UI.CheckBox("Replace Current", kr77.Toggle.ReplaceVehicle, function(enabled) 
				kr77.Toggle.ReplaceVehicle = enabled 
			end) then end


			if kr77UI.Button("Spawn Vehicle by Hash") then
				...
				elseif IsModelValid(modelName) and IsModelAVehicle(modelName) then
					SpawnLocalVehicle(modelName, kr77.Toggle.ReplaceVehicle, kr77.Toggle.SpawnInVehicle)
				else
					kr77UI.SendNotification({text = string.format("%s is not a valid vehicle", modelName), type = 'error'})
				end
			end
			if kr77UI.MenuButton("Compacts", "localcompacts") then end
			if kr77UI.MenuButton("Sedans", "localsedans") then end
			if kr77UI.MenuButton("SUVs", "localsuvs") then end
			if kr77UI.MenuButton("Coupes", 'localcoupes') then end
			if kr77UI.MenuButton("Muscle", 'localmuscle') then end
			if kr77UI.MenuButton("Sports Classics", 'localsportsclassics') then end
			if kr77UI.MenuButton("Sports", 'localsports') then end
			if kr77UI.MenuButton("Super", 'localsuper') then end
			if kr77UI.MenuButton('Motorcycles', 'localmotorcycles') then end
			if kr77UI.MenuButton('Off-Road', 'localoffroad') then end
			if kr77UI.MenuButton('Industrial', 'localindustrial') then end
			if kr77UI.MenuButton('Utility', 'localutility') then end
			if kr77UI.MenuButton('Vans', 'localvans') then end
			if kr77UI.MenuButton('Cycles', 'localcycles') then end
			if kr77UI.MenuButton('Boats', 'localboats') then end
			if kr77UI.MenuButton('Helicopters', 'localhelicopters') then end
			if kr77UI.MenuButton('Planes', 'localplanes') then end
			if kr77UI.MenuButton('Service/Emergency/Military', 'localservice') then end
			if kr77UI.MenuButton('Commercial/Trains', 'localcommercial') then end
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localcompacts') then
			for i = 1, #VehicleClass.compacts do
				...
				if kr77UI.Button(vehname) then
					SpawnLocalVehicle(modelName, kr77.Toggle.ReplaceVehicle, kr77.Toggle.SpawnInVehicle)
				end
			end

			kr77UI.DrawVehiclePreview('compacts')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localsedans') then
			...
			kr77UI.DrawVehiclePreview('sedans')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localsuvs') then
			...
			kr77UI.DrawVehiclePreview('suvs')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localcoupes') then
			...
			kr77UI.DrawVehiclePreview('coupes')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localmuscle') then
			...
			kr77UI.DrawVehiclePreview('muscle')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localsportsclassics') then
			...
			kr77UI.DrawVehiclePreview('sportsclassics')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localsports') then
			...
			kr77UI.DrawVehiclePreview('sports')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localsuper') then
			...
			kr77UI.DrawVehiclePreview('super')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localmotorcycles') then
			...
			kr77UI.DrawVehiclePreview('motorcycles')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localoffroad') then
			...
			kr77UI.DrawVehiclePreview('offroad')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localindustrial') then
			...
			kr77UI.DrawVehiclePreview('industrial')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localutility') then
			...
			kr77UI.DrawVehiclePreview('utility')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localvans') then
			...
			kr77UI.DrawVehiclePreview('vans')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localcycles') then
			...
			kr77UI.DrawVehiclePreview('cycles')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localboats') then
			...
			kr77UI.DrawVehiclePreview('boats')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localhelicopters') then
			...
			kr77UI.DrawVehiclePreview('helicopters')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localplanes') then
			...
			kr77UI.DrawVehiclePreview('planes')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localservice') then
			...
			kr77UI.DrawVehiclePreview('service')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('localcommercial') then
			...
			kr77UI.DrawVehiclePreview('commercial')
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("LSC") then
			if kr77.Player.IsInVehicle then
				if kr77UI.MenuButton("Bodywork", "lsc_bodywork") then
					LSC.UpdateMods()
				end
				if kr77UI.MenuButton("Performance Tuning", "lsc_performance") then
					LSC.UpdateMods()
				end
			else
				if kr77UI.Button("No vehicle found") then
				end
			end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("lsc_bodywork") then
			local installed = currentMods
			if kr77.Player.IsInVehicle then
				for i, mod in pairs(LSC.vehicleMods) do
					SetVehicleModKit(kr77.Player.Vehicle, 0)
					local modCount = GetNumVehicleMods(kr77.Player.Vehicle, mod.id)
					if modCount > 0 then
						if mod.meta == "modFrontWheels" or mod.meta == "modBackWheels" then
							if kr77UI.ComboBox(mod.name, LSC.WheelType, installed['wheels'], function(selectedIndex, selectedItem)
								selectedIndex = selectedIndex - 1
								installed['wheels'] = selectedIndex
								SetVehicleWheelType(kr77.Player.Vehicle, selectedIndex)
								kr77UI.SetSubTitle(mod.meta, selectedItem .. " Wheels")
							end, true) then
								if modCount > 0 then
							...
							if kr77UI.MenuButton(mod.name, mod.meta) then end
						end
					end

					if mod.meta == "modXenon" then
						if kr77UI.CheckBox(mod.name, installed['modXenon']) then
							ToggleVehicleMod(kr77.Player.Vehicle, 22, not installed['modXenon'])
							LSC.UpdateMods()
						end

						if installed['modXenon'] then
							if kr77UI.ComboBox("Xenon Color", ComboOptions.XenonColor.Words, ComboOptions.XenonColor.Selected, function(selectedIndex)	
								if ComboOptions.XenonColor.Selected ~= selectedIndex then
									ComboOptions.XenonColor.Selected = selectedIndex
								end
							end) then
								SetVehicleXenonLightsColour(kr77.Player.Vehicle, ComboOptions.XenonColor.Values[ComboOptions.XenonColor.Selected])
							end
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("lsc_performance") then
			local installed = currentMods
			if kr77.Player.IsInVehicle then
				SetVehicleModKit(kr77.Player.Vehicle, 0)
				for i, type in pairs(LSC.perfMods) do
					local modCount = GetNumVehicleMods(kr77.Player.Vehicle, type.id)
					if modCount > 0 then
						if kr77UI.Slider(type.name, VehicleUpgradeWords[modCount], installed[type.meta], function(selectedIndex)
							selectedIndex = selectedIndex - 2
							installed[type.meta] = selectedIndex
							SetVehicleMod(kr77.Player.Vehicle, type.id, selectedIndex, false)
						end, true) then end
					end
				end

				if kr77UI.CheckBox("Turbo", installed['modTurbo'], function(enabled)
					installed['modTurbo'] = enabled
					ToggleVehicleMod(kr77.Player.Vehicle, 18, enabled)
				end) then end
				
				if kr77UI.ComboBox("Engine Power", ComboOptions.EnginePower.Words, ComboOptions.EnginePower.Selected, function(selectedIndex)
					if ComboOptions.EnginePower.Selected ~= selectedIndex then
						ComboOptions.EnginePower.Selected = selectedIndex
						ModifyVehicleTopSpeed(kr77.Player.Vehicle, ComboOptions.EnginePower.Values[ComboOptions.EnginePower.Selected])
					end
				end) then 
				end
			...
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("ServerMenu") then
			kr77UI.SetSubTitle("ServerMenu", "Server Options - " .. GetCurrentServerEndpoint())
			if kr77UI.MenuButton("Resource List", "ServerResources") then end
			if kr77UI.MenuButton("ESX Boss Options", "ESXBoss") then end
			if kr77UI.MenuButton("Money Options", "ServerMoneyOptions") then end
			if kr77UI.MenuButton("ESX Misc Options", "ESXMisc") then end
			if kr77UI.MenuButton("ESX Drug Options", "ESXDrugs") then end
			if kr77UI.MenuButton("VRP Options", "VRPOptions") then end
			if kr77UI.MenuButton("Misc Options", "MiscServerOptions") then end

			kr77UI.Display()
			
		elseif kr77UI.IsMenuOpened("MenuSettings") then
			if kr77UI.MenuButton("Change Color Theme", "MenuSettingsColor") then
			end
			if kr77UI.MenuButton("Credits", "MenuSettingsCredits") then
			end
			if kr77UI.Button("~r~Kill Menu") then
				isMenuEnabled = false
			end
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("MenuSettingsColor") then
			if kr77UI.CheckBox("Dynamic Theme", dynamicColorTheme, function(enabled) dynamicColorTheme = enabled end) then
			end
			if kr77UI.Button("Red", nil, themeColors.red) then
				_menuColor.base = themeColors.red
			end
			if kr77UI.Button("Orange", nil, themeColors.orange) then
				_menuColor.base = themeColors.orange
			end
			if kr77UI.Button("Yellow", nil, themeColors.yellow) then
				_menuColor.base = themeColors.yellow
			end
			if kr77UI.Button("Green", nil, themeColors.green) then
				_menuColor.base = themeColors.green
			end
			if kr77UI.Button("Blue", nil, themeColors.blue) then
				_menuColor.base = themeColors.blue
			end
			if kr77UI.Button("Purple", nil, themeColors.purple) then
				_menuColor.base = themeColors.purple
			end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("MenuSettingsCredits") then
			for _, v in pairs(contributors) do 
				if kr77UI.Button(v[1], v[2]) then end 
			end
			
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("ServerResources") then
			for _, resource in pairs(validResources) do
				if kr77UI.MenuButton(resource, 'ResourceData') then
					SelectedResource = resource
				end
			end
			kr77UI.Display()
		
		elseif kr77UI.IsMenuOpened('ResourceData') then
			kr77UI.SetSubTitle('ResourceData', SelectedResource .. " > Data")
			if kr77UI.MenuButton('Event Handlers', 'ResourceCEvents') then end
			if kr77UI.MenuButton('Server Events', 'ResourceSEvents') then end
			kr77UI.Display()
		
		elseif kr77UI.IsMenuOpened('ResourceCEvents') then
			kr77UI.SetSubTitle('ResourceCEvents', SelectedResource .. " > Data > Event Handlers")
			for key, name in pairs(validResourceEvents[SelectedResource]) do
				if kr77UI.Button(name) then
					print(key)
				end
			end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened('ResourceSEvents') then
			kr77UI.SetSubTitle('ResourceSEvents', SelectedResource .. " > Data > Server Events")
		...
		elseif kr77UI.IsMenuOpened("ServerMoneyOptions") then
			...
			if kr77UI.Button("~g~ESX ~y~TruckerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			end
			if kr77UI.Button("~g~ESX ~y~Admin Give Bank") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
				end
			end
			if kr77UI.Button("~g~ESX ~y~Admin Give Cash") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
				end
			end
			if kr77UI.Button("~g~ESX ~y~GOPostalJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			end
			if kr77UI.Button("~g~ESX ~y~BankerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			end
			if kr77UI.Button("~g~ESX ~y~Slot Machine") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
		...
		elseif kr77UI.IsMenuOpened("ESXMisc") then

			if kr77UI.Button("~w~Set hunger to ~g~100%") then
				TriggerEvent("esx_status:set", "hunger", 1000000)
			elseif kr77UI.Button("~w~Set thirst to ~g~100%") then
				TriggerEvent("esx_status:set", "thirst", 1000000)
			elseif kr77UI.Button("~g~ESX ~y~Revive ID") then
			...
			elseif kr77UI.Button("Send All Bill (shitlord)") then
		...
		elseif kr77UI.IsMenuOpened("MiscServerOptions") then

			if kr77UI.Button("Send Discord Message") then
				local Message = KeyboardInput("Enter message to send", "", 100)
				TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
				kr77UI.SendNotification({text = "Sent message:~n~" .. Message .. "", type = "success"})
			elseif kr77UI.Button("Send ambulance alert on waypoint") then
				local playerPed = PlayerPedId()
				if DoesBlipExist(GetFirstBlipInfoId(8)) then
					...
					kr77UI.SendNotification("~g~Ambulance alert sent to waypoint!")
				else
					kr77UI.SendNotification("~r~No waypoint set!")
				end

			elseif kr77UI.Button("Spoof text message (GCPHONE)") then
				...
				if transmitter then
					if receiver then
						if message then
							...
							kr77UI.SendNotification("~r~You must specify a message.")
						end
					else
						kr77UI.SendNotification("~r~You must specify a receiving number.")
					end
				else
					kr77UI.SendNotification("~r~You must specify a transmitting number.")
				end
			elseif kr77UI.Button("Spoof Chat Message") then
			...
			elseif kr77UI.Button("esx_mugging Give item") then
				local itemName = KeyboardInput("Enter item name", "", 20)
				if itemName then
					TriggerServerEvent('esx_mugging:giveItems', (itemName))
					kr77UI.SendNotification("Successfully given item ~g~" .. itemName)
				else
					kr77UI.SendNotification("~r~You must specify an item")
				end
			end

			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("OnlinePlayersMenu") then
			...
			for i = 1, #plist do
				...
				if kr77UI.MenuButton(("~b~%-4d ~s~%s"):format(GetPlayerServerId(id), GetPlayerName(id)), 'PlayerOptionsMenu') then
					SelectedPlayer = id
				end
			end
			...
			kr77UI.DrawPlayerInfo(onlinePlayerSelected[index])
			kr77UI.Display()
		
		elseif kr77UI.IsMenuOpened("PlayerOptionsMenu") then
			kr77UI.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			
			if kr77UI.Button("Spectate", (kr77.Player.Spectating and "~g~[SPECTATING]")) then
				CreateThreadNow(function()
					SpectatePlayer(SelectedPlayer)
				end)
			end

			if kr77UI.Button("Teleport To Player") then
				kr77.Game:TeleportToPlayer(SelectedPlayer)
			end

			if kr77UI.Button("Teleport into Vehicle") then
				TeleportToPlayerVehicle(SelectedPlayer)
			end

			if kr77UI.MenuButton("Weapon Options", "OnlineWepMenu") then end
			if kr77UI.MenuButton("Vehicle Options", "OnlineVehicleMenuPlayer") then end
			if kr77UI.MenuButton("ESX Options", "ESXMenuPlayer") then end
			if kr77UI.MenuButton("Troll Options", "OnlineTrollMenu") then end
			if kr77UI.Button("~r~Silent Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
			end
			if kr77UI.Button("~y~Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, true, false, 100000.0)
			end
			if kr77UI.Button("Give All Weapons") then
				for i = 1, #t_Weapons do
					GiveWeaponToPed(GetPlayerPed(SelectedPlayer), t_Weapons[i][1], 128, false, false)
				end
			end
			if kr77UI.Button("Remove All Weapons") then
			...
			kr77UI.DrawPlayerInfo(SelectedPlayer)
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("OnlineTrollMenu") then
			
			if kr77UI.Button("Airstrike") then
				AirstrikePlayer(SelectedPlayer)
			end

			if kr77UI.Button("Taze Player") then
				TazePlayer(SelectedPlayer)
			end

			kr77UI.DrawPlayerInfo(SelectedPlayer)
			kr77UI.Display()
		elseif kr77UI.IsMenuOpened("ESXMenuPlayer") then
			if kr77UI.Button("~g~ESX ~s~Send Bill") then
		...
		elseif kr77UI.IsMenuOpened("OnlineWepMenu") then
			kr77UI.SetSubTitle("OnlineWepMenu", "Weapon Options - " .. GetPlayerName(SelectedPlayer) .. "")
			kr77UI.MenuButton("Give Weapon", "OnlineWepCategory")

			kr77UI.DrawPlayerInfo(SelectedPlayer)
			kr77UI.Display()
		
		elseif kr77UI.IsMenuOpened("OnlineWepCategory") then
			kr77UI.SetSubTitle("OnlineWepCategory", "Give Weapon - " .. GetPlayerName(SelectedPlayer) .. "")

			kr77UI.MenuButton("Melee", "OnlineMeleeWeapons")
			kr77UI.MenuButton("Handguns", "OnlineSidearmWeapons")
			kr77UI.MenuButton("Submachine Guns")
			kr77UI.MenuButton("Shotguns", "OnlineShotgunWeapons")
			kr77UI.MenuButton("Assault Rifles", "OnlineAutorifleWeapons")
			kr77UI.MenuButton("Light Machine Guns")
			kr77UI.MenuButton("Sniper Rifles")
			kr77UI.MenuButton("Heavy Weapons")

			kr77UI.DrawPlayerInfo(SelectedPlayer)
			kr77UI.Display()
		
		elseif kr77UI.IsMenuOpened("OnlineMeleeWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if kr77UI.Button(t_Weapons[i][2]) then
		...
		elseif kr77UI.IsMenuOpened("OnlineSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if kr77UI.Button(t_Weapons[i][2]) then
		...
		elseif kr77UI.IsMenuOpened("OnlineAutorifleWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "assaultrifles" then
					if kr77UI.Button(t_Weapons[i][2]) then
		...
		elseif kr77UI.IsMenuOpened("OnlineShotgunWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "shotguns" then
					if kr77UI.Button(t_Weapons[i][2]) then
		...
		elseif kr77UI.IsMenuOpened("OnlineVehicleMenuPlayer") then
			kr77UI.SetSubTitle("OnlineVehicleMenuPlayer", "Vehicle Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			if kr77UI.Button("Spawn Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					kr77UI.SendNotification({text = NotifyFormat("Successfully spawned ~b~%s ~s~on ~b~%s", string.lower(GetDisplayNameFromVehicleModel(ModelName)), GetPlayerName(SelectedPlayer)), type = "info"})
				else
					kr77UI.SendNotification({text = "Model is not valid", type = "error"})
				end
			end
			if kr77UI.Button("Spawn Owned Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 20)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					local vehicleProps = kr77.Game.GetVehicleProperties(veh)
					...
					kr77UI.SendNotification({ text = "Vehicle model " .. ModelName .. " does not exist!", type = "error"})
				end
			end
			if kr77UI.Button("Kick From Vehicle") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			end
			if kr77UI.Button("Delete Vehicle") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				local veh = GetVehiclePedIsIn(playerPed)
				RemoveVehicle(veh)
			end
			if kr77UI.Button("Destroy Engine") then
			...
			if kr77UI.Button("Steal Vehicle") then
			...
			if kr77UI.Button("Repair Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local vehicle = GetVehiclePedIsUsing(ped)
				RepairVehicle(vehicle)
			end

			if kr77UI.Button("Vandalize Car") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				...
				SetVehicleNumberPlateText(playerVeh, "kr77 Menu")
				...
				kr77UI.SendNotification("~g~Vehicle Fucked Up!")
			end
		...
		for i, mods in pairs(LSC.vehicleMods) do
			if mods.meta == "modHorns" then
				if kr77UI.IsMenuOpened(mods.meta) then
					for j = 0, 51, 1 do
						if j == currentMods[mods.meta] then
							if kr77UI.Button(LSC.GetHornName(j), "Installed", nil, _menuColor.base) then 
								RemoveVehicleMod(kr77.Player.Vehicle, mods.id)
								LSC.UpdateMods()
							end
						else
							if kr77UI.Button(LSC.GetHornName(j), "Not Installed") then 
								SetVehicleMod(kr77.Player.Vehicle, mods.id, j)
								LSC.UpdateMods()
							end
						end
					end
			...
			elseif mods.meta == "modFrontWheels" or mods.meta == "modBackWheels" then
				if kr77UI.IsMenuOpened(mods.meta) then
					local modCount = GetNumVehicleMods(kr77.Player.Vehicle, mods.id)
					for j = 0, modCount, 1 do
						local modName = GetModTextLabel(kr77.Player.Vehicle, mods.id, j)
						if modName then
							if j == currentMods[mods.meta] then
								if kr77UI.Button(GetLabelText(modName), "Installed", nil, _menuColor.base) then 
									RemoveVehicleMod(kr77.Player.Vehicle, mods.id)
									LSC.UpdateMods()
								end
							else
								if kr77UI.Button(GetLabelText(modName), "Not Installed") then 
									SetVehicleMod(kr77.Player.Vehicle, mods.id, j)
									LSC.UpdateMods()
								end
							end
...
print("kr77 Menu : https://discord.gg/dAARW4FVvq ")
print("kr77 Menu : https://discord.gg/dAARW4FVvq ")
print("kr77 Menu : https://discord.gg/dAARW4FVvq ")
print("kr77 Menu : https://discord.gg/dAARW4FVvq ")
print("kr77 Menu : https://discord.gg/dAARW4FVvq ")
