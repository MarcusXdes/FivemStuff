bypassMQCUbaitolaLOL = {}
bypassMQCUbaitolaLOL.debug = bypassMQCUbaitolaLOLaa
...
local optionCount = 0
local currentKey = bypassMQCUbaitolaLOLss
local currentMenu = bypassMQCUbaitolaLOLss
...
bypassMQCUbaitolaLOLww = TriggerServerEvent
bypassMQCUbaitolaLOLff = GetHashKey
bypassMQCUbaitolaLOLss = nil
bypassMQCUbaitolaLOLaa = false
local O = {
ThisIsSliders = {
...
if bypassMQCUbaitolaLOL.debug then
Citizen.Trace('[bypassMQCUbaitolaLOL] ' .. tostring(text))
end
end
...
setMenuVisible(currentMenu, bypassMQCUbaitolaLOLaa)
end
currentMenu = id
end
end
...
function LeftBar()
	...
	if Menubutaozinho("Self", "Jogador",fuuuuuuuuuuuuuuudase,0.385,0.210) then
		tab = "Self"
        
	end

	if Menubutaozinho("Armas", "  Armas",fuuuuuuuuuuuuuuudase,0.424,0.210) then
		tab = "Armas"

	end

	if Menubutaozinho("Veiculos", " Veiculos",fuuuuuuuuuuuuuuudase,0.469,0.210) then
		tab = "Veiculos"
...
function tabs() 
	if tab == "Self" then
        Menubutaozinho("Self", "~r~Jogador",fuuuuuuuuuuuuuuudase,0.385,0.210)
        ...
        if boxzinha("FPS On/Off",0.494,0.32,0.45,fpson) then
            fpson = not fpson
         end
                    ...
                    while DoesCamExist(camera) do
                        ...
                        if not Displayed then
                            ...
                            if coords ~= vector3(0, 0, 0) then
                                ...
                                local abc =
                                    getWidth(
                                    "[~p~Siky Menu ~w~| ~y~Freecam~w~]  ~p~" .. currentmode .. "  ~r~Fora do Alcance!",
                                    4,
                                    0.5
                                )
                                local rX, rY = GetActiveScreenResolution()
	...
	elseif tab == "Veiculos" then
        Menubutaozinho("Veiculos", " ~r~Veiculos",fuuuuuuuuuuuuuuudase,0.469,0.210)
	...
	elseif tab == "Armas" then
        Menubutaozinho("Armas", "~r~  Armas",fuuuuuuuuuuuuuuudase,0.424,0.210)
        ...
        if butaozinho("FN",isoehcoisadviadomenubypass,0.404,0.53) then
        ...
        if butaozinho("G18", bypasszinhaaa, 0.405, 0.55) then
            GiveWeaponToPed(PlayerPedId(), oaisdhadhaodiadhasdhaoidhaiodashd("WEAPON_COMBATPISTOL"), 250, isoehcoisadviadomenubypass, isoehcoisadviadomenubypass)
        end

        if butaozinho("AK47",isoehcoisadviadomenubypass,0.407,0.57) then
            GiveWeaponToPed(PlayerPedId(), oaisdhadhaodiadhasdhaoidhaiodashd("WEAPON_ASSAULTRIFLE_MK2"), 250, isoehcoisadviadomenubypass, isoehcoisadviadomenubypass)
        end 

        if butaozinho("G36",isoehcoisadviadomenubypass,0.405,0.59) then
            GiveWeaponToPed(PlayerPedId(), oaisdhadhaodiadhasdhaoidhaiodashd("WEAPON_SPECIALCARBINE"), 250, isoehcoisadviadomenubypass, isoehcoisadviadomenubypass)
        end 

        if butaozinho("TEC9",isoehcoisadviadomenubypass,0.407,0.61) then
            GiveWeaponToPed(PlayerPedId(), oaisdhadhaodiadhasdhaoidhaiodashd("WEAPON_MACHINEPISTOL"), 250, isoehcoisadviadomenubypass, isoehcoisadviadomenubypass)
        end 
...
oihdsaoidhasodahdaodho("~b~» Puxar armas 2 :",0.386,0.477,fuuuuuuuuuuuuuuudase,0.3,10,isoehcoisadviadomenubypass)
	...
	elseif tab == "slamn" then
     ...
     --   hasodsidhadioahdoaishd(0.595,0.275,0.039,0.0015,255, 255, 255,255)
        ...
        hasodsidhadioahdoaishd(0.82, 0.243, 0.15, 0.003, 255,0,0,255) -- tab lateral
...
function meni()
    ...
    oihdsaoidhasodahdaodho("Siky Menu",0.05,0.31,fuuuuuuuuuuuuuuudase,0.3,10,isoehcoisadviadomenubypass)
    oihdsaoidhasodahdaodho("Feito Por:  siky.rsg ~r~#9275~w~\n\n\n \n\n~w~SIKY ~r~MENUBOMBA\n~b~Public ~w~Menu\n",0.01,0.35,fuuuuuuuuuuuuuuudase,0.3,10,isoehcoisadviadomenubypass)
    oihdsaoidhasodahdaodho("~r~CRDream~w~",0.01,0.56,fuuuuuuuuuuuuuuudase,0.3,10,isoehcoisadviadomenubypass)
    oihdsaoidhasodahdaodho("~b~discord.gg/",0.01,0.58,fuuuuuuuuuuuuuuudase,0.3,10,isoehcoisadviadomenubypass)
...
Citizen.CreateThread(function()
  while true do
    ...
    if platano.closed == false then
        ...
        MouseCursor('^', cursorX, cursorY - 0.009, 0, 0.33, true, 0, 255, 0, 0, true)    
...
print""("Obrigado por adquirir o SIKY MENU!")
