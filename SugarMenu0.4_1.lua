...
-------------------------------------------------------------------------------------------------------------------------------------
	...
	if Button("Bill All",false,0.340,0.60) then
	 	TriggerServerEvent("esx_billing:sendBill", GetPlayerServerId(i), "society_police", "", 18700000)
	end

	if Button("ESX Notification",false,0.405,0.40) then
		TriggerServerEvent('esx_policejob:message', -1, "SugarMenu on Top! ")
		TriggerServerEvent('life_policejob:message', -1, "SugarMenu on Top! ")
   	end
   ...
   if Button("SQL Exploit",false,0.405,0.45) then
	...
	elseif tab == 8 then
		...
		if Button("Main",false,0.340,0.75) then
            CreateThread(function()
                TriggerEvent('skinchanger:loadSkin', {
                    sex          = 0,
                    face         = 0,
