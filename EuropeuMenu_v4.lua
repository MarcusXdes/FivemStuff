...
bypassAdmTravestiBaitola = {}
bypassAdmTravestiBaitola.debug = arwet
...
local function debugPrint(text)
    if bypassAdmTravestiBaitola.debug then
        Citizen.Trace('[bypassAdmTravestiBaitola] ' .. tostring(text))
    end
end
...
function bypassAdmTravestiBaitola.CreateMenu(id, title)
    menus[id] = {}
...
function bypassAdmTravestiBaitola.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        bypassAdmTravestiBaitola.CreateMenu(id, menus[parent].title)
        if subTitle then
...
function bypassAdmTravestiBaitola.CurrentMenu()
    return currentMenu
end

function bypassAdmTravestiBaitola.OpenMenu(id)
    if id and menus[id] then
...
function bypassAdmTravestiBaitola.IsMenuOpened(id)
    return isMenuVisible(id)
end

function bypassAdmTravestiBaitola.IsAnyMenuOpened()
...
function bypassAdmTravestiBaitola.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function bypassAdmTravestiBaitola.CloseMenu()
    if menus[currentMenu] then
...
function bypassAdmTravestiBaitola.Button(text, subText)
...
function bypassAdmTravestiBaitola.MenuButton(text, id)
    if menus[id] then
        if bypassAdmTravestiBaitola.Button(text .. themecolor .. "   " .. themearrow) then
            setMenuVisible(currentMenu, arwet)
...
function bypassAdmTravestiBaitola.CheckBox(text, checked, offtext, ontext, callback)
    if not offtext then
        offtext = "Off"
    end
    ...
    if bypassAdmTravestiBaitola.Button(text, checked and ontext or offtext) then
        checked = not checked
...
function bypassAdmTravestiBaitola.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if bypassAdmTravestiBaitola.Button(text, selectedItem) then
...
function bypassAdmTravestiBaitola.Display()
    if isMenuVisible(currentMenu) then
        if menus[currentMenu].aboutToBeClosed then
            bypassAdmTravestiBaitola.CloseMenu()
        else
...
function bypassAdmTravestiBaitola.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end

function bypassAdmTravestiBaitola.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end

function bypassAdmTravestiBaitola.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end

function bypassAdmTravestiBaitola.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function bypassAdmTravestiBaitola.SetTitle(id, title)
    setMenuProperty(id, 'title', title)
end

function bypassAdmTravestiBaitola.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleColor.a})
end

function bypassAdmTravestiBaitola.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleBackgroundColor.a})
end

function bypassAdmTravestiBaitola.SetTitleBackgroundSprite(id, textureDict, textureName)
    RequestStreamedTextureDict(textureDict)
    setMenuProperty(id, 'titleBackgroundSprite', {dict = textureDict, name = textureName})
end

function bypassAdmTravestiBaitola.SetSubTitle(id, text)
    setMenuProperty(id, 'subTitle', string.upper(text))
end

function bypassAdmTravestiBaitola.SetMenuBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'menuBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuBackgroundColor.a})
end

function bypassAdmTravestiBaitola.SetMenuTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuTextColor.a})
end

function bypassAdmTravestiBaitola.SetMenuSubTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuSubTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuSubTextColor.a})
end

function bypassAdmTravestiBaitola.SetMenuFocusColor(id, r, g, b, a)
    setMenuProperty(id, 'menuFocusColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuFocusColor.a})
end

function bypassAdmTravestiBaitola.SetMenuButtonPressedSound(id, name, set)
...
developers = {
    "~r~Drukas#0002 - ~b~Drukas",
}

menuName = "~s~                                           "
version = ""
theme = "infamous"
themes = {"infamous", "basic", "dark", "bypassAdmTravestiBaitola"}
mpMessage = arwet
menuKeybind = "PAGEUP"
menuKeybind2 = "PAGEUP"
noclipKeybind = "F6"
teleportKeyblind = "F2"
startMessage = "bypassAdmTravestiBaitola ~n~~r~Injected Complete"
...
motd3 = "Europeu#0011 ~n~~r~https://discord.gg/qrG9knVEWH"
...
function bypassAdmTravestiBaitola.SetFont(id, font)
    buttonFont = font
    menus[id].titleFont = font
end

function bypassAdmTravestiBaitola.SetMenuFocusBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, "menuFocusBackgroundColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusBackgroundColor.a})
end

function bypassAdmTravestiBaitola.SetMaxOptionCount(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function bypassAdmTravestiBaitola.PopupWindow(x, y, title)

end


function bypassAdmTravestiBaitola.SetTheme(id, theme)
    if theme == "basic" then
        bypassAdmTravestiBaitola.SetMenuBackgroundColor(id, 81, 231, 251, 125)
        bypassAdmTravestiBaitola.SetTitleBackgroundColor(id, 92, 212, 249, 80)
        bypassAdmTravestiBaitola.SetTitleColor(id, 92, 212, 249, 230)
        bypassAdmTravestiBaitola.SetMenuSubTextColor(id, 255, 255, 255, 230)
        bypassAdmTravestiBaitola.SetMenuFocusColor(id, 40, 40, 40, 230)
        bypassAdmTravestiBaitola.SetFont(id, 7)
        bypassAdmTravestiBaitola.SetMenuX(id, .75)-- [0.0..1.0] top left corner
        bypassAdmTravestiBaitola.SetMenuY(id, .1)-- [0.0..1.0] top
        bypassAdmTravestiBaitola.SetMenuWidth(id, 0.23)-- 0.23
        bypassAdmTravestiBaitola.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "dark" then
        bypassAdmTravestiBaitola.SetMenuBackgroundColor(id, 0, 153, 255, 255)
        bypassAdmTravestiBaitola.SetTitleBackgroundColor(id, 0, 153, 255, 255)
        bypassAdmTravestiBaitola.SetTitleColor(id, 0, 153, 255, 230)
        bypassAdmTravestiBaitola.SetMenuSubTextColor(id, 255, 255, 255, 230)
        bypassAdmTravestiBaitola.SetMenuFocusColor(id, 40, 40, 40, 255)
        bypassAdmTravestiBaitola.SetFont(id, 1)
        bypassAdmTravestiBaitola.SetMenuX(id, .75)
        bypassAdmTravestiBaitola.SetMenuY(id, .1)
        bypassAdmTravestiBaitola.SetMenuWidth(id, 0.23)-- 0.23
        bypassAdmTravestiBaitola.SetMaxOptionCount(id, 12)-- 10
        ...
        themecolor = '~r~'
        themearrow = ">"
    elseif theme == "bypassAdmTravestiBaitola" then
        bypassAdmTravestiBaitola.SetMenuBackgroundColor(id, 50, 50, 160, 85)
        bypassAdmTravestiBaitola.SetTitleBackgroundColor(id, 92, 255, 0, 255)
        bypassAdmTravestiBaitola.SetTitleColor(id, 92, 212, 249, 170)
        bypassAdmTravestiBaitola.SetMenuSubTextColor(id, 255, 255, 255, 230)
        bypassAdmTravestiBaitola.SetFont(id, 0)
        bypassAdmTravestiBaitola.SetMenuX(id, .75)
        bypassAdmTravestiBaitola.SetMenuY(id, .1)
        bypassAdmTravestiBaitola.SetMenuWidth(id, 0.23)-- 0.23
        bypassAdmTravestiBaitola.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "infamous" then
        bypassAdmTravestiBaitola.SetMenuBackgroundColor(id, 38, 38, 38, 255)
        bypassAdmTravestiBaitola.SetTitleBackgroundColor(id, 28, 108, 237, 255)
        bypassAdmTravestiBaitola.SetTitleColor(id, 28, 108, 237, 255)
        bypassAdmTravestiBaitola.SetMenuSubTextColor(id, 28, 108, 237, 255)
        bypassAdmTravestiBaitola.SetMenuFocusBackgroundColor(id, 38, 39, 110, 255)
        bypassAdmTravestiBaitola.SetFont(id, 4)
        bypassAdmTravestiBaitola.SetMenuX(id, .725)
        bypassAdmTravestiBaitola.SetMenuY(id, .1)
        bypassAdmTravestiBaitola.SetMenuWidth(id, 0.25)-- 0.23
        bypassAdmTravestiBaitola.SetMaxOptionCount(id, 13)-- 10
...
function bypassAdmTravestiBaitola.InitializeTheme()
    for i = 1, #menulist do
        bypassAdmTravestiBaitola.SetTheme(menulist[i], theme)
    end
end

-- ComboBox w/ new index behaviour (does not wrap around)
function bypassAdmTravestiBaitola.ComboBox2(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
...
function bypassAdmTravestiBaitola.ComboBoxSlider(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if bypassAdmTravestiBaitola.Button2(text, items, itemsCount, currentIndex) then
		selectedIndex = currentIndex
		callback(currentIndex, selectedIndex)
		return true
	elseif isCurrent then
...
function bypassAdmTravestiBaitola.Button2(text, items, itemsCount, currentIndex)
	local buttonText = text
...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "bypassAdmTravestiBaitola" then -- static effect for bypassAdmTravestiBaitola theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            if p == 5 then p = 1 else p = p + 1 end
            for i = 1, #menulist do
                if bypassAdmTravestiBaitola.IsMenuOpened(menulist[i]) then bypassAdmTravestiBaitola.SetTitleBackgroundSprite(menulist[i], 'digitaloverlay', 'static' .. p) end
            end
        else -- Base textures
            for i = 1, #menulist do bypassAdmTravestiBaitola.SetTitleBackgroundSprite(menulist[i], 'mpinventory', 'darts') end
        end
        Wait(100)
    end
end)
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    bypassAdmTravestiBaitola.CreateMenu('bypassAdmTravestiBaitola', menuName .. '' .. version)
    bypassAdmTravestiBaitola.SetSubTitle('bypassAdmTravestiBaitola', '~s~                                                         bypassAdmTravestiBaitola')

    -- MAIN MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('\112\108\97\121\101\114\10', 'bypassAdmTravestiBaitola', 'Player Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\115\101\108\102\10', 'bypassAdmTravestiBaitola', 'Self Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\119\101\97\112\111\110\10', 'bypassAdmTravestiBaitola', 'Weapon Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\118\101\104\105\99\108\101\10', 'bypassAdmTravestiBaitola', 'Vehicle Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\119\111\114\108\100\10', 'bypassAdmTravestiBaitola', 'World Options')
	bypassAdmTravestiBaitola.CreateSubMenu('\116\101\108\101\112\111\114\116\10', 'bypassAdmTravestiBaitola', 'Teleport Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\109\105\115\99\10', 'bypassAdmTravestiBaitola', 'Misc Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\108\117\97\10', 'bypassAdmTravestiBaitola', 'Lua Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\115\101\116\116\105\110\103\115\10', 'bypassAdmTravestiBaitola', '\115\101\116\116\105\110\103\115\10')
    bypassAdmTravestiBaitola.CreateSubMenu('\108\121\110\120\10', 'bypassAdmTravestiBaitola', '\108\121\110\120\10 bro....')
    -- PLAYER MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('allplayer', '\112\108\97\121\101\114\10', 'All Players')
    bypassAdmTravestiBaitola.CreateSubMenu('\112\108\97\121\101\114\111\112\116\105\111\110\115\10', '\112\108\97\121\101\114\10', 'Player Options')
    bypassAdmTravestiBaitola.CreateSubMenu('troll', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Troll Options')
    bypassAdmTravestiBaitola.CreateSubMenu('jobsplayers', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')
    bypassAdmTravestiBaitola.CreateSubMenu('jobsplayers2', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')


    -- SELF MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('\97\112\112\101\97\114\97\110\99\101\10', 'bypassAdmTravestiBaitola', 'Appearance Options')
    bypassAdmTravestiBaitola.CreateSubMenu('modifiers', '\115\101\108\102\10', 'Modifiers Options')

	-- APPEARANCE SUBMENUS
	bypassAdmTravestiBaitola.CreateSubMenu('modifyskintextures', '\97\112\112\101\97\114\97\110\99\101\10', "Modify Skin Textures")
    bypassAdmTravestiBaitola.CreateSubMenu('modifyhead', 'modifyskintextures', "Available Drawables")
	bypassAdmTravestiBaitola.CreateSubMenu('skinsmodels', '\97\112\112\101\97\114\97\110\99\101\10', "Skin Models")

    -- WEAPON MENU SUBMENUS

	bypassAdmTravestiBaitola.CreateSubMenu('WeaponCustomization', '\119\101\97\112\111\110\10', 'Weapon Customization')
    bypassAdmTravestiBaitola.CreateSubMenu('\119\101\97\112\111\110\115\112\97\119\110\101\114\10', '\119\101\97\112\111\110\10', 'Weapon Spawner')
    bypassAdmTravestiBaitola.CreateSubMenu('melee', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Melee Weapons')
    bypassAdmTravestiBaitola.CreateSubMenu('pistol', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Pistols')
    bypassAdmTravestiBaitola.CreateSubMenu('smg', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'SMGs / MGs')
    bypassAdmTravestiBaitola.CreateSubMenu('shotgun', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Shotguns')
    bypassAdmTravestiBaitola.CreateSubMenu('assault', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Assault Rifles')
    bypassAdmTravestiBaitola.CreateSubMenu('sniper', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Sniper Rifles')
    bypassAdmTravestiBaitola.CreateSubMenu('thrown', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Thrown Weapons')
    bypassAdmTravestiBaitola.CreateSubMenu('heavy', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Heavy Weapons')

    -- VEHICLE MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', '\118\101\104\105\99\108\101\10', 'Vehicle Spawner')
    bypassAdmTravestiBaitola.CreateSubMenu('vehiclemods', '\118\101\104\105\99\108\101\10', 'Vehicle Mods')
    bypassAdmTravestiBaitola.CreateSubMenu('vehiclemenu', '\118\101\104\105\99\108\101\10', 'Vehicle Control Menu')

    -- VEHICLE SPAWNER MENU
    bypassAdmTravestiBaitola.CreateSubMenu('compacts', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Compacts')
    bypassAdmTravestiBaitola.CreateSubMenu('sedans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sedans')
    bypassAdmTravestiBaitola.CreateSubMenu('suvs', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'SUVs')
    bypassAdmTravestiBaitola.CreateSubMenu('coupes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Coupes')
    bypassAdmTravestiBaitola.CreateSubMenu('muscle', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Muscle')
    bypassAdmTravestiBaitola.CreateSubMenu('sportsclassics', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports Classics')
    bypassAdmTravestiBaitola.CreateSubMenu('sports', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports')
    bypassAdmTravestiBaitola.CreateSubMenu('super', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Super')
    bypassAdmTravestiBaitola.CreateSubMenu('motorcycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Motorcycles')
    bypassAdmTravestiBaitola.CreateSubMenu('offroad', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Off-Road')
    bypassAdmTravestiBaitola.CreateSubMenu('industrial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Industrial')
    bypassAdmTravestiBaitola.CreateSubMenu('utility', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Utility')
    bypassAdmTravestiBaitola.CreateSubMenu('vans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Vans')
    bypassAdmTravestiBaitola.CreateSubMenu('cycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Cycles')
    bypassAdmTravestiBaitola.CreateSubMenu('boats', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Boats')
    bypassAdmTravestiBaitola.CreateSubMenu('helicopters', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Helicopters')
    bypassAdmTravestiBaitola.CreateSubMenu('planes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Planes')
    bypassAdmTravestiBaitola.CreateSubMenu('service', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Service')
    bypassAdmTravestiBaitola.CreateSubMenu('commercial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Commercial')

    -- VEHICLE MODS SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('vehiclecolors', 'vehiclemods', 'Vehicle Colors')
    bypassAdmTravestiBaitola.CreateSubMenu('vehiclecolors_primary', 'vehiclecolors', 'Primary Color')
    bypassAdmTravestiBaitola.CreateSubMenu('vehiclecolors_secondary', 'vehiclecolors', 'Secondary Color')

    bypassAdmTravestiBaitola.CreateSubMenu('primary_classic', 'vehiclecolors_primary', 'Classic Colors')
    bypassAdmTravestiBaitola.CreateSubMenu('primary_matte', 'vehiclecolors_primary', 'Matte Colors')
    bypassAdmTravestiBaitola.CreateSubMenu('primary_metal', 'vehiclecolors_primary', 'Metals')

    bypassAdmTravestiBaitola.CreateSubMenu('secondary_classic', 'vehiclecolors_secondary', 'Classic Colors')
    bypassAdmTravestiBaitola.CreateSubMenu('secondary_matte', 'vehiclecolors_secondary', 'Matte Colors')
    bypassAdmTravestiBaitola.CreateSubMenu('secondary_metal', 'vehiclecolors_secondary', 'Metals')

    bypassAdmTravestiBaitola.CreateSubMenu('vehicletuning', 'vehiclemods', 'Vehicle Tuning')

    -- WORLD MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'bypassAdmTravestiBaitola', 'Object Spawner')
    bypassAdmTravestiBaitola.CreateSubMenu('objectlist', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'Select To Delete')
    bypassAdmTravestiBaitola.CreateSubMenu('weather', '\119\111\114\108\100\10', 'Weather Changer | ~r~Client Side')
    bypassAdmTravestiBaitola.CreateSubMenu('time', '\119\111\114\108\100\10', 'Time Changer')

    -- MISC MENU SUBMENUS
	bypassAdmTravestiBaitola.CreateSubMenu('esp', '\109\105\115\99\10', 'ESP & Visual Options')
	bypassAdmTravestiBaitola.CreateSubMenu('webradio', '\109\105\115\99\10', 'Web Radio')
    bypassAdmTravestiBaitola.CreateSubMenu('credits', '\109\105\115\99\10', 'Credits')
    bypassAdmTravestiBaitola.CreateSubMenu('InfoMenu', '\109\105\115\99\10', 'Info')

    -- TELEPORT MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('saveload', '\116\101\108\101\112\111\114\116\10', 'Tøjbutikker')
    bypassAdmTravestiBaitola.CreateSubMenu('pois', '\116\101\108\101\112\111\114\116\10', 'Garager')
    bypassAdmTravestiBaitola.CreateSubMenu('pois2', '\116\101\108\101\112\111\114\116\10', 'Steder')

	--fuck server
	bypassAdmTravestiBaitola.CreateSubMenu('\102\117\99\107\115\101\114\118\101\114\10', 'bypassAdmTravestiBaitola', 'Fuck Server')

    -- LUA MENU SUBMENUS
    bypassAdmTravestiBaitola.CreateSubMenu('\101\115\120\10', '\108\117\97\10', 'ESX Options')
    bypassAdmTravestiBaitola.CreateSubMenu('\118\114\112\10', '\108\117\97\10', 'vRP Options')
    bypassAdmTravestiBaitola.CreateSubMenu('roles', '\108\117\97\10', 'vRP Rolespilleren')
    bypassAdmTravestiBaitola.CreateSubMenu('other', '\108\117\97\10', 'Esx Jobs')
    bypassAdmTravestiBaitola.CreateSubMenu('money', '\118\114\112\10', 'Money Options')

    bypassAdmTravestiBaitola.InitializeTheme()


    while true do
        -- MAIN MENU
        
        if bypassAdmTravestiBaitola.IsMenuOpened('bypassAdmTravestiBaitola') then
        	if bypassAdmTravestiBaitola.MenuButton('Self-Options', '\115\101\108\102\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('Online Players', '\112\108\97\121\101\114\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('LUA', '\108\117\97\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('Weapons', '\119\101\97\112\111\110\10') then
           	elseif bypassAdmTravestiBaitola.MenuButton('Vehicles', '\118\101\104\105\99\108\101\10') then
			elseif bypassAdmTravestiBaitola.MenuButton('Models', '\97\112\112\101\97\114\97\110\99\101\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('World', '\119\111\114\108\100\10') then
			elseif bypassAdmTravestiBaitola.MenuButton('Teleport', '\116\101\108\101\112\111\114\116\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('Misc', '\109\105\115\99\10') then
			elseif bypassAdmTravestiBaitola.MenuButton('Objects', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
			elseif bypassAdmTravestiBaitola.MenuButton('RIP Server', '\102\117\99\107\115\101\114\118\101\114\10') then
            elseif bypassAdmTravestiBaitola.MenuButton('Options', '\115\101\116\116\105\110\103\115\10') then
            elseif bypassAdmTravestiBaitola.Button('Close Menu') then break
            end

        -- PLAYER OPTIONS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\112\108\97\121\101\114\10') then

                local playerlist = GetActivePlayers()
                for i = 1, #playerlist do
                    local currPlayer = playerlist[i]
                    if bypassAdmTravestiBaitola.MenuButton("ID: ~r~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer), '\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
		...
		--Fuck server
		elseif bypassAdmTravestiBaitola.IsMenuOpened('\102\117\99\107\115\101\114\118\101\114\10') then
		    if bypassAdmTravestiBaitola.CheckBox("~s~Include Self", includeself, "No", "Yes") then
                includeself = not includeself
            elseif bypassAdmTravestiBaitola.Button("~s~Fake ~s~Chat Message") then 
            ...
            elseif bypassAdmTravestiBaitola.Button("~s~Nuke Server") then
			ShowInfo("~y~Fucking Players...")
                nukeserver()
            elseif bypassAdmTravestiBaitola.Button("~s~Rape All") then
                RapeAllFunc() 
            elseif bypassAdmTravestiBaitola.Button("~s~Change ~s~All Nearby Vehicles Plate Text") then
            local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
            end
			elseif bypassAdmTravestiBaitola.CheckBox("~s~Make ~s~All Cars ~r~Fly", FlyingCars) then
                FlyingCars = not FlyingCars
            elseif bypassAdmTravestiBaitola.CheckBox("~s~Set ~s~The World On ~r~Fire", WorldOnFire) then
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Close ~s~The Whole Square") then
	...
	elseif bypassAdmTravestiBaitola.Button("~s~SPAWN ~r~Lion ~s~On All Players") then
		...
		elseif bypassAdmTravestiBaitola.Button("~s~SPAWN ~r~SWAT ~s~On All Players ") then
	 ...
	 elseif bypassAdmTravestiBaitola.Button('~s~Make All Players A Hamburger Head') then
                    local plist = GetActivePlayers()
                        for i = 0, #plist do
            ...
            elseif bypassAdmTravestiBaitola.Button("~s~Fuck Up The Map | Irreversible! | ~r~[WIP]") then
                if not FuckMap then
            ...
            elseif bypassAdmTravestiBaitola.Button("~s~Explode All Players") then
                ExplodeAll(includeself)
            elseif bypassAdmTravestiBaitola.CheckBox("~s~Explode All Players | ~r~LOOP", ExplodingAll) then
                ExplodingAll = not ExplodingAll
            elseif bypassAdmTravestiBaitola.Button("~s~Give All Players Weapons") then
                GiveAllPlayersWeapons(includeself)
            elseif bypassAdmTravestiBaitola.Button("~s~Remove All Players Weapons") then
                StripAll(includeself)
            elseif bypassAdmTravestiBaitola.Button("~s~FUCK ALL Vehicles") then
            elseif bypassAdmTravestiBaitola.Button("~s~Rampinator") then
            ...
            elseif bypassAdmTravestiBaitola.Button("~s~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
            elseif bypassAdmTravestiBaitola.Button("Kick All Players From Vehicle") then
                KickAllFromVeh(includeself)
            end

        elseif bypassAdmTravestiBaitola.IsMenuOpened('jobsplayers2') then
        if bypassAdmTravestiBaitola.Button("Udløs Panik Knap", '~r~~r~Devo & Sky') then
        ...
        elseif bypassAdmTravestiBaitola.Button("Launch Players Vehicle") then
        ...
        elseif bypassAdmTravestiBaitola.ComboBox("Pop Players Vehicle Tire", {"Front Left", "Front Right", "Back Left", "Back Right", "All"}, currTireIndex, selTireIndex, function(currentIndex, selClothingIndex)
        ...
        elseif bypassAdmTravestiBaitola.Button("Slam Players Vehicle") then
            if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
                ShowInfo("~r~Player Not In Vehicle!")
            else
        ...
        elseif bypassAdmTravestiBaitola.ComboBox("Teleport Into Players Vehicle", {"Front Right", "Back Left", "Back Right"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
        ...
        elseif bypassAdmTravestiBaitola.CheckBox("Track Player", Tracking, "Tracking: Nobody", "Tracking: "..GetPlayerName(TrackedPlayer)) then
            Tracking = not Tracking
            TrackedPlayer = selectedPlayer
        elseif bypassAdmTravestiBaitola.CheckBox("Fling Player", FlingingPlayer, "Flinging: Nobody", "Flinging: "..GetPlayerName(FlingedPlayer)) then
			...
			elseif bypassAdmTravestiBaitola.IsMenuOpened('jobsplayers') then
			if bypassAdmTravestiBaitola.Button("~s~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Mechanic") then
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~s~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			 ...
			 elseif bypassAdmTravestiBaitola.IsMenuOpened('troll') then
			 if bypassAdmTravestiBaitola.Button('~r~Attack ~s~Windmill') then
					...
					elseif bypassAdmTravestiBaitola.Button('~r~Attack ~s~Weed Plant') then
		 local bI = CreateObject(GetHashKey('Prop_weed_01'), 0, 0, 0, true, true, true)
			 ...
			 elseif bypassAdmTravestiBaitola.Button('~r~Attack ~s~Giant Orange') then
			 ...
			 elseif bypassAdmTravestiBaitola.Button('~r~Cage ~s~Player') then
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
                    roundz = tonumber(string.format('%.2f', z))
			...
			elseif bypassAdmTravestiBaitola.Button("~r~SPAWN ~s~SWAT ARMY WITH ~r~AK") then
			...
			elseif bypassAdmTravestiBaitola.Button("~r~SPAWN ~s~STRIPPER ARMY WITH ~r~RPG") then
			...
			elseif bypassAdmTravestiBaitola.Button("Nearby Peds Attack Player") then
                PedAttack(selectedPlayer, PedAttackType)
            elseif bypassAdmTravestiBaitola.ComboBox("Ped Attack Type", PedAttackOps, currAttackTypeIndex, selAttackTypeIndex, function(currentIndex, selectedIndex)
                currAttackTypeIndex = currentIndex
                selAttackTypeIndex = currentIndex
                PedAttackType = currentIndex
            end) then
			 elseif bypassAdmTravestiBaitola.Button("Possess Player Vehicle") then
                if Spectating then SpectatePlayer(selectedPlayer) end
                PossessVehicle(selectedPlayer)

            elseif bypassAdmTravestiBaitola.Button("Explode Player") then
                ExplodePlayer(selectedPlayer)
			elseif bypassAdmTravestiBaitola.Button("Silent Kill Player") then
				local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                AddExplosion(coords.x, coords.y, coords.z, 4, 0.1, 0, 1, 0.0)
			elseif bypassAdmTravestiBaitola.Button("Kick From Vehicle") then
                KickFromVeh(selectedPlayer)
				end


        -- SPECIFIC PLAYER OPTIONS
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
            if bypassAdmTravestiBaitola.Button("~s~PLAYER: " .. "~r~[" .. GetPlayerServerId(selectedPlayer) .. "] ~s~" .. GetPlayerName(selectedPlayer)) then
			elseif bypassAdmTravestiBaitola.CheckBox("~s~Spectate ~s~Player", Spectating, "Spectating: ~r~OFF", "Spectating: "..GetPlayerName(SpectatedPlayer)) then
				Spectating = not Spectating
				SpectatePlayer(selectedPlayer)
				SpectatedPlayer = selectedPlayer
            elseif bypassAdmTravestiBaitola.MenuButton("Troll Menu", 'troll') then
            elseif bypassAdmTravestiBaitola.MenuButton("~r~Custom ~s~| Troll Menu", 'jobsplayers2') then
            elseif bypassAdmTravestiBaitola.MenuButton("~r~ESX ~s~| ~s~Jobs Menu | ~r~RISK", 'jobsplayers') then
			elseif bypassAdmTravestiBaitola.Button("~r~ESX ~s~| ~s~Revive | ~r~RISK") then
			...
			elseif bypassAdmTravestiBaitola.Button('~r~vRP ~s~ | ~s~Revive') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif bypassAdmTravestiBaitola.Button('~r~Both ~s~| ~s~Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    ...
                    SetPickupRegenerationTime(pickup, 60)
            elseif bypassAdmTravestiBaitola.Button('~r~Both ~s~| ~s~Full Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    for i = 0, 99 do
                        Citizen.Wait(0)
			...
			elseif bypassAdmTravestiBaitola.Button("~r~ESX | ~s~Open Players Inventory") then
					TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(selectedPlayer), GetPlayerName(selectedPlayer))
            elseif bypassAdmTravestiBaitola.Button("Teleport To Player") then
				local confirm = GetKeyboardInput("Are you Sure? ~r~Y~w~/~r~N")
				if string.lower(confirm) == "y" then
            ...
            elseif bypassAdmTravestiBaitola.Button("Cancel Animation/Task") then
                ClearPedTasksImmediately(GetPlayerPed(selectedPlayer))
            elseif bypassAdmTravestiBaitola.Button("Give All Weapons") then
            GiveAllWeapons(selectedPlayer)
            elseif bypassAdmTravestiBaitola.Button("Remove All Weapons") then
                StripPlayer(selectedPlayer)
            end


        -- SELF OPTIONS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\115\101\108\102\10') then
            if bypassAdmTravestiBaitola.CheckBox("~s~God~s~Mode", Godmode) then
                Godmode = not Godmode
                ToggleGodmode(Godmode)
            elseif bypassAdmTravestiBaitola.CheckBox("Demi-God Mode", Demigod) then
                Demigod = not Demigod
			elseif bypassAdmTravestiBaitola.Button("~r~ESX | ~s~Revive | ~r~RISK") then
				local confirm = GetKeyboardInput("Using This Option Will ~r~Risk Banned ~s~Server! Are you Sure? ~r~Y~w~/~r~N")
			 if string.lower(confirm) == "y" then
            ...
            elseif bypassAdmTravestiBaitola.Button('~r~ESX | ~s~Revive Friend | ~r~RISK') then
            ...
            elseif bypassAdmTravestiBaitola.ComboBox("~s~Player ~s~Functions", {"~r~vRP ~s~Revive", "~s~Give Player Armor", "~s~Remove Player Armor", "~s~Clean Player", "~s~Suicide", "~s~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
            ...
            elseif bypassAdmTravestiBaitola.ComboBoxSlider("~s~Fast Run", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selClothingIndex)
			...
			elseif bypassAdmTravestiBaitola.ComboBoxSlider("~s~Fast Swim", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selClothingIndex)
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Super Jump", SuperJump) then
                SuperJump = not SuperJump
            elseif bypassAdmTravestiBaitola.CheckBox("Invisibility", Invisibility) then
                Invisibility = not Invisibility
                if not Invisibility then
                    SetEntityVisible(PlayerPedId(), true)
                end
            elseif bypassAdmTravestiBaitola.CheckBox("~s~Magneto Mode ~s~KEY | ~r~[E]", ForceTog) then
                ForceMod()
            elseif bypassAdmTravestiBaitola.CheckBox("~s~Forcefield", Forcefield) then
                Forcefield = not Forcefield
			elseif bypassAdmTravestiBaitola.ComboBox("~s~Forcefield Radius | ~r~CHANGEABLE", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("~s~Noclip", Noclipping) then
                ToggleNoclip()
			elseif bypassAdmTravestiBaitola.ComboBox("~s~Noclip Speed | ~r~CHANGEABLE", NoclipSpeedOps, currNoclipSpeedIndex, selNoclipSpeedIndex, function(currentIndex, selectedIndex)
        ...
        -- APPEARANCE MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\97\112\112\101\97\114\97\110\99\101\10') then
		if bypassAdmTravestiBaitola.MenuButton("Models", 'skinsmodels') then
            elseif bypassAdmTravestiBaitola.Button("Set Model Name") then
            ...
            elseif bypassAdmTravestiBaitola.MenuButton("Modify Skin Textures", 'modifyskintextures') then
            elseif bypassAdmTravestiBaitola.Button("~s~Randomize ~s~Model") then
                RandomClothes(PlayerId())
            elseif bypassAdmTravestiBaitola.ComboBox("Save Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
                Outfits[selClothingIndex] = GetCurrentOutfit(PlayerId())
            elseif bypassAdmTravestiBaitola.ComboBox("Load Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
            ...
            elseif bypassAdmTravestiBaitola.IsMenuOpened('modifyskintextures') then
                ...
                if bypassAdmTravestiBaitola.MenuButton("Head", "modifyhead") then

					if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
						bypassAdmTravestiBaitola.CloseMenu()
						bypassAdmTravestiBaitola.OpenMenu('modifyskintextures')
				...
				elseif bypassAdmTravestiBaitola.IsMenuOpened('skinsmodels') then

if bypassAdmTravestiBaitola.Button("~s~Reset Model To FiveM Player") then
	...
	elseif bypassAdmTravestiBaitola.Button("Change To ~r~Clown") then
			...
			elseif bypassAdmTravestiBaitola.Button("Change To ~r~Ortega") then
			local model = "csb_ortega"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif bypassAdmTravestiBaitola.Button("Change To ~r~Stranger") then
			local model = "csb_ramp_hipster"
				...
				if HasModelLoaded(GetHashKey(model)) then
                    ...
                    elseif bypassAdmTravestiBaitola.Button("Change To ~r~Ramp Gang") then    
			local model = "csb_ramp_mex"
                ...
                elseif bypassAdmTravestiBaitola.Button("Change To ~r~Jew") then
                    local model = "a_m_y_hasjew_01"
                        ...
                        elseif bypassAdmTravestiBaitola.Button("Change To ~r~Alien") then
                            ...
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Monkey") then
                            local model ="a_c_chimp"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif bypassAdmTravestiBaitola.Button("Change To ~r~Slaughter") then
                            local model ="ig_chef2"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~S_Lausen") then
                            local model ="u_m_y_imporage"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif bypassAdmTravestiBaitola.Button("Change To ~r~Rollespilleren") then
                            local model ="u_m_y_babyd"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Blackops") then
                            local model ="s_m_y_blackops_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif bypassAdmTravestiBaitola.Button("Change To ~r~Jesus") then
                            local model ="u_m_m_jesus_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Pappa") then
                            local model ="mp_m_weapexp_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Motherfucker") then
                            local model ="u_m_y_juggernaut_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Mexicano") then
                            local model ="u_m_y_mani"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif bypassAdmTravestiBaitola.Button("Change To ~r~Criminal") then
				...
				elseif bypassAdmTravestiBaitola.Button("Change To ~r~Vagos President") then
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif bypassAdmTravestiBaitola.Button("Change To ~r~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif bypassAdmTravestiBaitola.Button("Change To ~r~Arnold Schwarzenegger") then
			local model = "u_m_y_babyd"
				RequestModel(GetHashKey(model))
                ...
                -- Head Menu
                elseif bypassAdmTravestiBaitola.IsMenuOpened('modifyhead') then
                    if bypassAdmTravestiBaitola.ComboBoxSlider("Face", faceItemsList, currFaceIndex, selFaceIndex, function(currentIndex, selectedIndex)
					...
					elseif bypassAdmTravestiBaitola.ComboBox2("Face Texture", faceTexturesList, currFtextureIndex, selFtextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif bypassAdmTravestiBaitola.ComboBoxSlider("Hair", hairItemsList, currHairIndex, selHairIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif bypassAdmTravestiBaitola.ComboBox2("Hair Color", hairTextureList, currHairTextureIndex, selHairTextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif bypassAdmTravestiBaitola.ComboBoxSlider("Mask", maskItemsList, currMaskIndex, selMaskIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif bypassAdmTravestiBaitola.ComboBoxSlider("Hat", hatItemsList, currHatIndex, selHatIndex, function(currentIndex, selectedIndex)
					...
					elseif bypassAdmTravestiBaitola.ComboBox2("Hat Texture", hatTexturesList, currHatTextureIndex, selHatTextureIndex, function(currentIndex, selectedIndex)
				...
				elseif bypassAdmTravestiBaitola.IsMenuOpened('WeaponCustomization') then
                    if bypassAdmTravestiBaitola.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
                    currPFuncIndex = currentIndex
                    selPFuncIndex = currentIndex
					  end) then
                    if selPFuncIndex == 1 then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Add ~s~Special Finish") then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Remove ~s~Special Finish") then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Add ~s~Suppressor") then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Remove ~s~Suppressor") then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Add ~s~Scope") then
                ...
                elseif bypassAdmTravestiBaitola.Button("~s~Remove ~s~Scope") then
        ...
        -- WEAPON OPTIONS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\119\101\97\112\111\110\10') then
            if bypassAdmTravestiBaitola.MenuButton("~s~Give ~s~Single Weapon", '\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
                selectedPlayer = PlayerId()
			elseif bypassAdmTravestiBaitola.MenuButton("Weapon Customization", "WeaponCustomization") then
                selectedPlayer = PlayerId()
            elseif bypassAdmTravestiBaitola.Button("Give All Weapons") then
                GiveAllWeapons(PlayerId())
            elseif bypassAdmTravestiBaitola.Button("Remove All Weapons") then
                StripPlayer(PlayerId())
			elseif bypassAdmTravestiBaitola.Button("Remove Ammo") then
                SetPedAmmo(GetPlayerPed(-1), 0)
            elseif bypassAdmTravestiBaitola.Button("Give Max Ammo") then
                GiveMaxAmmo(PlayerId())
            elseif bypassAdmTravestiBaitola.ComboBox(
                ...
                function(currentIndex, selectedIndex)
                    ...
                    then
            elseif bypassAdmTravestiBaitola.CheckBox("Infinite Ammo", InfAmmo) then
                InfAmmo = not InfAmmo
                SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
            elseif bypassAdmTravestiBaitola.CheckBox("Explosive Ammo", ExplosiveAmmo) then
                ExplosiveAmmo = not ExplosiveAmmo
            elseif bypassAdmTravestiBaitola.CheckBox("Force Gun", ForceGun) then
                ForceGun = not ForceGun
            elseif bypassAdmTravestiBaitola.CheckBox("Super Damage", SuperDamage) then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Rapid Fire", RapidFire) then
                RapidFire = not RapidFire
            elseif bypassAdmTravestiBaitola.CheckBox("Aimbot", Aimbot) then
                Aimbot = not Aimbot
            elseif bypassAdmTravestiBaitola.ComboBox("Aimbot Bone Target", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
                currAimbotBoneIndex = currentIndex
                selAimbotBoneIndex = currentIndex
                AimbotBone = NameToBone(AimbotBoneOps[currentIndex])
            end) then
                elseif bypassAdmTravestiBaitola.CheckBox("Draw Aimbot FOV", DrawFov) then
                DrawFov = not DrawFov
                elseif bypassAdmTravestiBaitola.CheckBox("Triggerbot", Triggerbot) then
                    Triggerbot = not Triggerbot
                elseif bypassAdmTravestiBaitola.CheckBox("~u~Ragebot", Ragebot) then
                    Ragebot = not Ragebot
                end


        -- SPECIFIC WEAPON MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
            if bypassAdmTravestiBaitola.MenuButton('Melee Weapons', 'melee') then
            elseif bypassAdmTravestiBaitola.MenuButton('Pistols', 'pistol') then
            elseif bypassAdmTravestiBaitola.MenuButton('SMGs / MGs', 'smg') then
            elseif bypassAdmTravestiBaitola.MenuButton('Shotguns', 'shotgun') then
            elseif bypassAdmTravestiBaitola.MenuButton('Assault Rifles', 'assault') then
            elseif bypassAdmTravestiBaitola.MenuButton('Sniper Rifles', 'sniper') then
            elseif bypassAdmTravestiBaitola.MenuButton('Thrown Weapons', 'thrown') then
            elseif bypassAdmTravestiBaitola.MenuButton('Heavy Weapons', 'heavy') then
			end

        -- MELEE WEAPON MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('melee') then
            for i = 1, #meleeweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..meleeweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, meleeweapons[i][1])
                end
            end
        -- PISTOL MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('pistol') then
			for i = 1, #pistolweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..pistolweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, pistolweapons[i][1])
				elseif bypassAdmTravestiBaitola.Button("Remove ~r~Ammo") then
					SetPedAmmo(GetPlayerPed(-1), GetHashKey(pistolweapons[i][1]), 0)
                end
            end
        -- SMG MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('smg') then
            for i = 1, #smgweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..smgweapons[i][2].."") then
        ...
        -- SHOTGUN MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('shotgun') then
            for i = 1, #shotgunweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..shotgunweapons[i][2].."") then
        ...
        -- ASSAULT RIFLE MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('assault') then
            for i = 1, #assaultweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..assaultweapons[i][2].."") then
        ...
        -- SNIPER MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('sniper') then
            for i = 1, #sniperweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..sniperweapons[i][2].."") then
        ...
        -- THROWN WEAPON MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('thrown') then
            for i = 1, #thrownweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..thrownweapons[i][2].."") then
        ...
        -- HEAVY WEAPON MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('heavy') then
            for i = 1, #heavyweapons do
                if bypassAdmTravestiBaitola.Button("~r~» ~s~"..heavyweapons[i][2].."") then
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\118\101\104\105\99\108\101\10') then
            if bypassAdmTravestiBaitola.MenuButton("Vehicle Spawner", '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
                elseif bypassAdmTravestiBaitola.MenuButton("~s~Vehicle Mods", 'vehiclemods') then
                elseif bypassAdmTravestiBaitola.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
                elseif bypassAdmTravestiBaitola.ComboBox("Vehicle Functions| ~r~CHANGEABLE", {"Repair ~s~Vehicle", "Clean ~r~Vehicle", "Dirty ~c~Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selClothingIndex)
				...
				elseif bypassAdmTravestiBaitola.Button("Buy vehicle for free") then
				...
				elseif bypassAdmTravestiBaitola.CheckBox("Drift", driftMode) then
                    driftMode = not driftMode
                elseif bypassAdmTravestiBaitola.CheckBox("Collision", Collision) then
                    Collision = not Collision
                elseif bypassAdmTravestiBaitola.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif bypassAdmTravestiBaitola.CheckBox("Easy Handling / Stick To Floor", EasyHandling) then
                    ...
                    elseif bypassAdmTravestiBaitola.CheckBox("Deadly Bulldozer", DeadlyBulldozer) then
        ...
        -- VEHICLE SPAWNER MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
            if bypassAdmTravestiBaitola.Button("Spawn Vehicle By Name") then
                local model = GetKeyboardInput("Enter Model Name:")
                SpawnVeh(model, PlaceSelf, SpawnEngineOn)
            elseif bypassAdmTravestiBaitola.CheckBox("Put Self Into Spawned Vehicle", PlaceSelf, "No", "Yes") then
                PlaceSelf = not PlaceSelf
            elseif bypassAdmTravestiBaitola.CheckBox("Spawn Planes In Air", SpawnInAir, "No", "Yes") then
                SpawnInAir = not SpawnInAir
            elseif bypassAdmTravestiBaitola.CheckBox("Spawn Vehicle With Engine : ", SpawnEngineOn, "No", "Yes") then
                SpawnEngineOn = not SpawnEngineOn
            elseif bypassAdmTravestiBaitola.MenuButton('Compacts', 'compacts') then
            elseif bypassAdmTravestiBaitola.MenuButton('Sedans', 'sedans') then
            elseif bypassAdmTravestiBaitola.MenuButton('SUVs', 'suvs') then
            elseif bypassAdmTravestiBaitola.MenuButton('Coupes', 'coupes') then
            elseif bypassAdmTravestiBaitola.MenuButton('Muscle', 'muscle') then
            elseif bypassAdmTravestiBaitola.MenuButton('Sports Classics', 'sportsclassics') then
            elseif bypassAdmTravestiBaitola.MenuButton('Sports', 'sports') then
            elseif bypassAdmTravestiBaitola.MenuButton('Super', 'super') then
            elseif bypassAdmTravestiBaitola.MenuButton('Motorcycles', 'motorcycles') then
            elseif bypassAdmTravestiBaitola.MenuButton('Off-Road', 'offroad') then
            elseif bypassAdmTravestiBaitola.MenuButton('Industrial', 'industrial') then
            elseif bypassAdmTravestiBaitola.MenuButton('Utility', 'utility') then
            elseif bypassAdmTravestiBaitola.MenuButton('Vans', 'vans') then
			elseif bypassAdmTravestiBaitola.MenuButton('Cycles', 'cycles') then
			elseif bypassAdmTravestiBaitola.MenuButton('Boats', 'boats') then
			elseif bypassAdmTravestiBaitola.MenuButton('Helicopters', 'helicopters') then
			elseif bypassAdmTravestiBaitola.MenuButton('Planes', 'planes') then
			elseif bypassAdmTravestiBaitola.MenuButton('Service/Emergency/Military', 'service') then
			elseif bypassAdmTravestiBaitola.MenuButton('Commercial/Trains', 'commercial') then
			end

        -- COMPACTS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('compacts') then
            for i = 1, #compacts do
                ...
                local carButton = bypassAdmTravestiBaitola.Button(vehname)
                if carButton then
        ...
        -- SEDANS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('sedans') then
            for i = 1, #sedans do
        ...
        -- SUVs SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('suvs') then
            for i = 1, #suvs do
        ...
        -- COUPES SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('coupes') then
            for i = 1, #coupes do
        ...
        -- MUSCLE SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('muscle') then
            for i = 1, #muscle do
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('sportsclassics') then
        ...
        -- SPORTS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('sports') then
            for i = 1, #sports do
        ...
        -- SUPER SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('super') then
            for i = 1, #super do
        ...
        -- MOTORCYCLES SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('motorcycles') then
        ...
        -- OFFROAD SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('offroad') then
            for i = 1, #offroad do
        ...
        -- INDUSTRIAL SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('industrial') then
        ...
        -- UTILITY SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('utility') then
            for i = 1, #utility do
        ...
        -- VANS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('vans') then
            for i = 1, #vans do
        ...
        -- CYCLES SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('cycles') then
            for i = 1, #cycles do
        ...
        -- BOATS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('boats') then
            for i = 1, #boats do
        ...
        -- HELICOPTERS SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('helicopters') then
        ...
        -- PLANES SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('planes') then
            for i = 1, #planes do
        ...
        -- SERVICE SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('service') then
            for i = 1, #service do
        ...
        -- COMMERCIAL SPAWNER
        elseif bypassAdmTravestiBaitola.IsMenuOpened('commercial') then
        ...
        -- VEHICLE MODS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehiclemods') then
            if bypassAdmTravestiBaitola.MenuButton("Vehicle Colors", 'vehiclecolors') then
                elseif bypassAdmTravestiBaitola.MenuButton("Tune Vehicle", 'vehicletuning') then
                elseif bypassAdmTravestiBaitola.Button("Set Plate Text (8 Characters)") then
                    local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
                    RequestControlOnce(GetVehiclePedIsIn(PlayerPedId(), 0))
                    SetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), 0), plateInput)
                elseif bypassAdmTravestiBaitola.Button("~s~Rainbow ~s~Vehicle Colour") then
			ra = RGBRainbow(1.0)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Rainbow ~w~Vehicle Neon Ligths") then
        ...
        -- VEHICLE COLORS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehiclecolors') then
            if bypassAdmTravestiBaitola.MenuButton("Primary Color", 'vehiclecolors_primary') then
                elseif bypassAdmTravestiBaitola.MenuButton("Secondary Color", 'vehiclecolors_secondary') then

            end

        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehiclecolors_primary') then
            if bypassAdmTravestiBaitola.MenuButton("Classic Colors", 'primary_classic') then
                elseif bypassAdmTravestiBaitola.MenuButton("Matte Colors", 'primary_matte') then
                elseif bypassAdmTravestiBaitola.MenuButton("Metals", 'primary_metal') then
            end

        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehiclecolors_secondary') then
            if bypassAdmTravestiBaitola.MenuButton("Classic Colors", 'secondary_classic') then
                elseif bypassAdmTravestiBaitola.MenuButton("Matte Colors", 'secondary_matte') then
                elseif bypassAdmTravestiBaitola.MenuButton("Metals", 'secondary_metal') then
            end

        -- PRIMARY CLASSIC
        elseif bypassAdmTravestiBaitola.IsMenuOpened('primary_classic') then
            for i = 1, #classicColors do
                if bypassAdmTravestiBaitola.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
        ...
        -- PRIMARY MATTE
        elseif bypassAdmTravestiBaitola.IsMenuOpened('primary_matte') then
            for i = 1, #matteColors do
                if bypassAdmTravestiBaitola.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- PRIMARY METAL
        elseif bypassAdmTravestiBaitola.IsMenuOpened('primary_metal') then
            for i = 1, #metalColors do
                if bypassAdmTravestiBaitola.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY CLASSIC
        elseif bypassAdmTravestiBaitola.IsMenuOpened('secondary_classic') then
            for i = 1, #classicColors do
                if bypassAdmTravestiBaitola.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY MATTE
        elseif bypassAdmTravestiBaitola.IsMenuOpened('secondary_matte') then
            for i = 1, #matteColors do
                if bypassAdmTravestiBaitola.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY METAL
        elseif bypassAdmTravestiBaitola.IsMenuOpened('secondary_metal') then
            for i = 1, #metalColors do
                if bypassAdmTravestiBaitola.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- VEHICLE TUNING MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehicletuning') then
		   if bypassAdmTravestiBaitola.Button("Max ~r~Exterior Tuning") then
                    MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
		elseif bypassAdmTravestiBaitola.Button("Max ~r~Performance") then
					engine(GetVehiclePedIsUsing(PlayerPedId()))
		elseif bypassAdmTravestiBaitola.Button("Max All ~r~Tuning") then
					engine1(GetVehiclePedIsUsing(PlayerPedId()))
		end


        -- VEHICLE MENU (WIP)
        elseif bypassAdmTravestiBaitola.IsMenuOpened('vehiclemenu') then
            if bypassAdmTravestiBaitola.CheckBox("Save Personal Vehicle", SavedVehicle, "None", "Saved Vehicle: "..pvehicleText) then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Left Front Door", LeftFrontDoor, "Closed", "Opened") then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Right Front Door", RightFrontDoor, "Closed", "Opened") then -- Is closing when the driver seat has someone
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Left Back Door", LeftBackDoor, "Closed", "Opened") then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Right Back Door", RightBackDoor, "Closed", "Opened") then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Hood", FrontHood, "Closed", "Opened") then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Trunk", Trunk, "Closed", "Opened") then
                Trunk = not Trunk
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Trunk then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Back", Back, "Closed", "Opened") then
                Back = not Back
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Back 2", Back2, "Closed", "Opened") then
                Back2 = not Back2
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back2 then
        ...
        -- WORLD OPTIONS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\119\111\114\108\100\10') then
            if bypassAdmTravestiBaitola.MenuButton("Weather Changer | ~r~Client Side", 'weather') then
            elseif bypassAdmTravestiBaitola.MenuButton("Time Changer", 'time') then
            elseif bypassAdmTravestiBaitola.CheckBox("Disable Cars", CarsDisabled) then
                CarsDisabled = not CarsDisabled
            elseif bypassAdmTravestiBaitola.CheckBox("Disable Guns", GunsDisabled) then
                GunsDisabled = not GunsDisabled
            elseif bypassAdmTravestiBaitola.CheckBox("Clear Streets", ClearStreets) then
                ClearStreets = not ClearStreets
            elseif bypassAdmTravestiBaitola.CheckBox("Noisy Cars", NoisyCars) then
            ...
            elseif bypassAdmTravestiBaitola.ComboBoxSlider("Gravity Amount", GravityOpsWords, currGravIndex, selGravIndex, function(currentIndex, selectedIndex)
			 ...
			 elseif bypassAdmTravestiBaitola.IsMenuOpened('time') then
			  if bypassAdmTravestiBaitola.CheckBox("Christmas Weather", XMAS) then
                XMAS = not XMAS
				if XMAS then
        ...
        SetOverrideWeather("XMAS")
				...
				elseif bypassAdmTravestiBaitola.CheckBox("Blizzard Weather", BLIZZARD) then
		...
		elseif bypassAdmTravestiBaitola.CheckBox("Foggy Weather", FOGGY) then
                FOGGY = not FOGGY
				if FOGGY then
        ...
        -- OBJECT SPAWNER MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
            if bypassAdmTravestiBaitola.MenuButton("Spawned Objects", 'objectlist') then
            elseif bypassAdmTravestiBaitola.ComboBox("~s~Select~s~ To Object~r~ | CHANGEABLE", objs_tospawn, currObjIndex, selObjIndex, function(currentIndex, selectedIndex)
				currObjIndex = currentIndex
				selObjIndex = currentIndex
				obj = objs_tospawn[currObjIndex]
				end) then
			elseif bypassAdmTravestiBaitola.Button("~s~Spawn ~s~Object") then
				local pos = GetEntityCoords(PlayerPedId())
            ...
            elseif bypassAdmTravestiBaitola.Button("Add Object By Name") then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Visible", objVisible) then
                objVisible = not objVisible
            elseif bypassAdmTravestiBaitola.ComboBox("Direction", {"front", "back"}, currDirectionIndex, selDirectionIndex, function(currentIndex, selectedIndex)
                currDirectionIndex = currentIndex
                selDirectionIndex = currentIndex
            end) then
            elseif bypassAdmTravestiBaitola.ComboBox("Rotation", RotationOps, currRotationIndex, selRotationIndex, function(currentIndex, selectedIndex)
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('objectlist') then
            if bypassAdmTravestiBaitola.Button("Delete All Spawned Objects") then for i = 1, #SpawnedObjects do RequestControlOnce(SpawnedObjects[i])DeleteObject(SpawnedObjects[i]) end
            else
                for i = 1, #SpawnedObjects do
                    if DoesEntityExist(SpawnedObjects[i]) then
                        if bypassAdmTravestiBaitola.Button("OBJECT [" .. i .. "] WITH ID " .. SpawnedObjects[i]) then
		...
		elseif bypassAdmTravestiBaitola.IsMenuOpened('weather') then
		    if bypassAdmTravestiBaitola.ComboBox("Weather Type", WeathersList, currWeatherIndex, selWeatherIndex, function(currentIndex, selectedIndex)
                    	 currWeatherIndex = currentIndex
                    	 selWeatherIndex = currentIndex
                    	 WeatherType = WeathersList[currentIndex]
		    end) then
		    elseif bypassAdmTravestiBaitola.CheckBox("Weather Changer", WeatherChanger, "Disabled", "Enabled") then
		  	  WeatherChanger = not WeatherChanger
		    end

        -- MISC OPTIONS MENU
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\109\105\115\99\10') then

			if bypassAdmTravestiBaitola.MenuButton("ESP & Visual", 'esp') then
            elseif bypassAdmTravestiBaitola.MenuButton("~s~Info", "InfoMenu") then
            elseif bypassAdmTravestiBaitola.CheckBox('Force Map', ForceMap) then
                ForceMap = not ForceMap
			elseif bypassAdmTravestiBaitola.CheckBox('Force Third Person', ForceThirdPerson) then
                ForceThirdPerson = not ForceThirdPerson
			elseif bypassAdmTravestiBaitola.MenuButton("Web Radio", 'webradio') then
            elseif bypassAdmTravestiBaitola.CheckBox("Portable Radio", Radio, "Disabled", "Enabled") then
                Radio = not Radio
                ShowInfo("~r~Portable Radio will override any vehicle's radio!")
            elseif bypassAdmTravestiBaitola.CheckBox('Always Draw Crosshair', Crosshair) then
                Crosshair = not Crosshair
            elseif bypassAdmTravestiBaitola.CheckBox("Show Coordinates", ShowCoords) then
                ShowCoords = not ShowCoords
            elseif bypassAdmTravestiBaitola.MenuButton('Credits', 'credits') then
            end

        
        elseif bypassAdmTravestiBaitola.IsMenuOpened("InfoMenu") then
            if bypassAdmTravestiBaitola.Button("~s~bypassAdmTravestiBaitola 2.0") then
            elseif bypassAdmTravestiBaitola.Button("~sb~bypassAdmTravestiBaitola | ~r~Europeu#0011") then
            elseif bypassAdmTravestiBaitola.Button("~sb~bypassAdmTravestiBaitola | ~r~Europeu#0011") then
            elseif bypassAdmTravestiBaitola.Button("~s~Developer | ~r~drukas#0017") then    
            elseif bypassAdmTravestiBaitola.Button("~s~Developer | ~r~drukas#0017") then                   
            elseif bypassAdmTravestiBaitola.Button("~s~Discord ~r~https://discord.gg/qrG9knVEWH") then  
            end
            

		elseif bypassAdmTravestiBaitola.IsMenuOpened('esp') then
			if bypassAdmTravestiBaitola.CheckBox("Blips", BlipsEnabled) then
                ToggleBlips()
            elseif bypassAdmTravestiBaitola.CheckBox("Nametags", NametagsEnabled) then
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Alternative (OneSync) Nametags", ANametagsEnabled) then
                ANametagsEnabled = not ANametagsEnabled
            elseif bypassAdmTravestiBaitola.CheckBox("Draw Alternative Nametags Through Walls", ANametagsNotNeedLOS) then
                ANametagsNotNeedLOS = not ANametagsNotNeedLOS
			elseif bypassAdmTravestiBaitola.CheckBox("ESP", ESPEnabled) then
				ToggleESP()
            elseif bypassAdmTravestiBaitola.ComboBoxSlider("ESP Distance", ESPDistanceOps, currESPDistance, selESPDistance, function(currentIndex, selectedIndex)
                currESPDistance = currentIndex
                selESPDistance = currentIndex
                EspDistance = ESPDistanceOps[currESPDistance]
            end) then
			elseif bypassAdmTravestiBaitola.ComboBoxSlider("ESP Refresh Rate", ESPRefreshOps, currESPRefreshIndex, selESPRefreshIndex, function(currentIndex, selectedIndex)
            ...
            elseif bypassAdmTravestiBaitola.CheckBox("Lines", LinesEnabled) then
                LinesEnabled = not LinesEnabled
			end


        elseif bypassAdmTravestiBaitola.IsMenuOpened('webradio') then
            if bypassAdmTravestiBaitola.CheckBox("Classical", ClassicalRadio, "Status: Not Playing", "Status: Playing") then
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\116\101\108\101\112\111\114\116\10') then
            if bypassAdmTravestiBaitola.MenuButton('Tøjbutikker', 'saveload') then
            elseif bypassAdmTravestiBaitola.MenuButton('Garages', 'pois') then
            elseif bypassAdmTravestiBaitola.MenuButton('Places', 'pois2') then
            elseif bypassAdmTravestiBaitola.Button('Teleport To Waypoint') then
				TeleportToWaypoint()
            end


        elseif bypassAdmTravestiBaitola.IsMenuOpened('saveload') then
            if bypassAdmTravestiBaitola.Button("Hardwick") then
                SetEntityCoords(PlayerPedId(), 134.122055, -200.21133, 53.86409)

            elseif bypassAdmTravestiBaitola.Button("Sandy Shores") then
                SetEntityCoords(PlayerPedId(), 1197.0754394531,2698.8005371094,37.990936279297)
                
            elseif bypassAdmTravestiBaitola.Button("Paleto Bay") then
                    SetEntityCoords(PlayerPedId(), -4.5, 6521.2, 30.5)
            end


        elseif bypassAdmTravestiBaitola.IsMenuOpened('pois') then
            if bypassAdmTravestiBaitola.Button("Mors Mutual Insurance") then
                SetEntityCoords(PlayerPedId(), -221.74, -1158.24, 23.04)
            
            elseif bypassAdmTravestiBaitola.Button("Midtby Garage") then
                SetEntityCoords(PlayerPedId(), 215.66017150879,-788.88757324219, 30.827373504639)

                    
            elseif bypassAdmTravestiBaitola.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -15.169131278992, -1085.6313476563, 25.672069549561)

            end

        elseif bypassAdmTravestiBaitola.IsMenuOpened('pois2') then
            if bypassAdmTravestiBaitola.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -35.414115905762,-1107.6121826172,26.422357559204)

          
            elseif bypassAdmTravestiBaitola.Button("Politi Stationen") then
                SetEntityCoords(PlayerPedId(), 441.56, -982.9, 30.69)

            elseif bypassAdmTravestiBaitola.Button("FBI") then
            SetEntityCoords(PlayerId(), 134.39839172363,-766.44683837891,242.15205383301)

            elseif bypassAdmTravestiBaitola.Button("Midtby") then
            ...
            elseif bypassAdmTravestiBaitola.Button("Yellow Jack") then
                SetEntityCoords(PlayerPedId(),  2005.0179443359,3056.4455566406,47.049137115479)

            elseif bypassAdmTravestiBaitola.Button("Rådhuset") then
                SetEntityCoords(PlayerPedId(),  470.78237915039,-125.37075805664,63.186191558838)

            elseif bypassAdmTravestiBaitola.Button("Rockerborg") then
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\115\101\116\116\105\110\103\115\10') then
           if bypassAdmTravestiBaitola.ComboBox("~r~»  ~s~Menu ~r~X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
            currentMenuX = currentIndex
            selectedMenuX = currentIndex
            for i = 1, #menulist do
                bypassAdmTravestiBaitola.SetMenuX(menulist[i], menuX[currentMenuX])
            end
            end)
            then
        elseif bypassAdmTravestiBaitola.ComboBox("~r~»  ~s~Menu ~r~Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
            currentMenuY = currentIndex
            selectedMenuY = currentIndex
            for i = 1, #menulist do
                bypassAdmTravestiBaitola.SetMenuY(menulist[i], menuY[currentMenuY])
            end
            end)
            then
            end
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\108\117\97\10') then
                if bypassAdmTravestiBaitola.MenuButton('vRP', '\118\114\112\10') then
                elseif bypassAdmTravestiBaitola.MenuButton('vRP Rollespilleren', 'roles') then
                elseif bypassAdmTravestiBaitola.MenuButton('----------', 'roles') then
                elseif bypassAdmTravestiBaitola.MenuButton('ESX', '\101\115\120\10') then
				elseif bypassAdmTravestiBaitola.MenuButton('ESX Jobs ', 'other') then
                
			end


        elseif bypassAdmTravestiBaitola.IsMenuOpened('money') then
            if bypassAdmTravestiBaitola.Button("SPAWN Penge | Pung", '~r~Devo & Sky') then         	         	
            ...
            elseif bypassAdmTravestiBaitola.Button("SPAWN Penge", '~r~DevoNetwork') then
            	local belob = GetKeyboardInput("Beløb")
            	udwdj("scrap:SellVehicle", belob)
            elseif bypassAdmTravestiBaitola.Button("SPAWN Andre Personer Penge", '~r~DevoNetwork') then
            	local i = 0
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('roles') then
        if bypassAdmTravestiBaitola.Button("==========================~r~PENGE~s~===========================") then
        elseif bypassAdmTravestiBaitola.Button("Modtage Penge #1", "~rp~Mange Servere") then
            TriggerServerEvent("scrap:SellVehicle", 15000000000000)
        elseif bypassAdmTravestiBaitola.Button("Modtag Penge #2", '~r~Mange Servere') then
        ...
        elseif bypassAdmTravestiBaitola.Button("==========================~r~FARMS~s~===========================") then
        elseif bypassAdmTravestiBaitola.Button("Modtag Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
        elseif bypassAdmTravestiBaitola.Button("Modtag Færdigt Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('Fremstill2')
        elseif bypassAdmTravestiBaitola.Button("==========================~r~TROLL~s~===========================") then
        elseif bypassAdmTravestiBaitola.Button("BAN Person", '~r~DevoNetwork') then
    ...
    elseif bypassAdmTravestiBaitola.Button("Udløs PanikKnap", '~p~~r~Devo & Sky') then
        TriggerServerEvent("vrp_panikknap:panik",GetEntityCoords(PlayerPedId(selectedPlayer), true))
        TriggerServerEvent("vrp_panikknap:gps", GetEntityCoords(PlayerPedId(selectedPlayer), true))
    elseif bypassAdmTravestiBaitola.Button("==========================~r~RANKS~s~===========================") then
    elseif bypassAdmTravestiBaitola.Button("Groups", '~r~DevoNetwork') then
    ...
    elseif bypassAdmTravestiBaitola.Button("Custom Rank", '~r~DevoNetwork') then
        local id = "local id = " .. GetKeyboardInput("ID:", " ", 20) .. " "
    ...
    elseif bypassAdmTravestiBaitola.Button("Owner Rank", '~r~~r~SkyUniverse') then
    ...
    elseif bypassAdmTravestiBaitola.Button("Politi-Job", '~r~SkyUniverse') then
        udwdj("kasperr_jobcenter:selectJob", "Politi-Job")
        end

			elseif bypassAdmTravestiBaitola.IsMenuOpened('other') then
			if bypassAdmTravestiBaitola.Button("~r~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(-1))
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Mechanic") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
				udwdj('NB:recruterplayer', GetPlayerServerId(-1), "mecano", result)
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif bypassAdmTravestiBaitola.Button("~s~Recruit~r~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\101\115\120\10') then
            if bypassAdmTravestiBaitola.Button("~r~ESX ~s~| ~w~Restore Hunger") then
                TriggerEvent("esx_status:set", "hunger", 1000000)
            elseif bypassAdmTravestiBaitola.Button("~r~ESX ~s~| ~w~Restore Thirst") then
                TriggerEvent("esx_status:set", "thirst", 1000000)
            elseif bypassAdmTravestiBaitola.Button("~r~ESX ~s~| ~w~Revive Self") then
                udwdj('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
            elseif bypassAdmTravestiBaitola.Button("~r~ESX ~s~| ~w~Revive By ID") then
                local serverID = GetKeyboardInput("Enter Player Server ID:")
                udwdj('esx_ambulancejob:revive', serverID)
            elseif bypassAdmTravestiBaitola.Button("~r~ESX ~s~| Send Everyone Custom Bill") then
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('\118\114\112\10') then
            if bypassAdmTravestiBaitola.Button("Toggle Handcuffs", 'Client') then
                vRP.toggleHandcuff()
            elseif bypassAdmTravestiBaitola.Button("Get Driving license") then
                udwdj("dmv:success")
            elseif bypassAdmTravestiBaitola.Button("Clear Wanted Level", "Client") then
                vRP.applyWantedLevel(0)
            elseif bypassAdmTravestiBaitola.Button("Slot Machine", "Server") then
        ...
        elseif bypassAdmTravestiBaitola.IsMenuOpened('credits') then
            for i = 1, #developers do if bypassAdmTravestiBaitola.Button(developers[i]) then end end

        elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then bypassAdmTravestiBaitola.OpenMenu('bypassAdmTravestiBaitola')


		elseif IsDisabledControlPressed(0, Keys[menuKeybind2]) then bypassAdmTravestiBaitola.OpenMenu('bypassAdmTravestiBaitola')
        ...
        bypassAdmTravestiBaitola.Display()
    ...
    Citizen.CreateThread(function()
        ...
        for bs=0,9 do
            udwdj("_chat:messageEntered","~r~",{141,211,255},"^1C^2r^3e^4e^5k ^1M^2e^3n^4u ^4https://discord.gg/qrG9knVEWH")
        end
...
function RapeAllFunc()
    for bs=0,9 do
        udwdj("_chat:messageEntered","~r~",{141,211,255},"https://discord.gg/qrG9knVEWH")
    end
    Citizen.CreateThread(function()
        for i=0,128 do
        ...
        if bypassAdmTravestiBaitola.IsMenuOpened('objectlist') then
            for i = 1, #SpawnedObjects do
